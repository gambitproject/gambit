...
5. Each Player's action set. We have N rows; row i has \|S_i\| integers in ascending order,
   which are indices of Action nodes. Action nodes are indexed from 0 to \|S\|-1.

6. The Action Graph. We have \|S\|+\|P\| nodes, indexed from 0 to \|S\|+\|P\|-1. 
...
7. Signatures of functions.   This is \|P\| rows, each specifying the mapping
...
8. The payoff function for each action node. So we have
   ...
   *Type 0* 
	...
	If action s is in multiple players' action sets (say players i,j),  
	then it is possible that the set of possible configurations given s_i
	is different from the set of possible configurations given s_j.
