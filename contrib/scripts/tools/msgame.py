...
class MSGame(object):
    ...
    def __init__(self, N, contribs):
        ...
        self.statistics = xrange(
            (self.N - 1) * min(self.contribs), (self.N - 1) * max(self.contribs) + 1
        )

    def AsTable(self):
        return [
            [own, others, self.Payoff(own, others)]
            for (own, others) in cartesian(self.contribs, self.statistics)
        ]

    def AsMatrix(self):
        return [[""] + [other for other in self.statistics]] + [
            [own] + [self.Payoff(own, other) for other in self.statistics]
    ...
    def AsNfg(self):
        ...
        for cont in gambit.StrategyIterator(gambit.StrategySupport(game)):
            ...
            choices = [
                int(cont.GetStrategy(pl + 1).GetLabel()) for pl in xrange(self.N)
            ]
            for pl in xrange(self.N):
                outcome.SetPayoff(
                    pl + 1, self.Payoff(choices[pl], sum(choices) - choices[pl])
                )
...
class MSTableGame(MSGame):
    ...
    def __setitem__(self, key, value):
        if (
            len(key) != 2
            or key[0] not in self.contribs
            or key[1] not in self.statistics
        ):
            raise IndexError
