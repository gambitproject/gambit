...
void DropDownPopup::OnTimer(wxTimerEvent &WXUNUSED(event))
{
  ...
  // m_popped_handler, m_child, m_handlerPopup); fflush(stdout);
...
void DropDownPopup::SetChild(wxWindow *win) { m_childWin = win; }
...
void DropDownPopup::Popup(wxWindow *focus)
{
  ...
  Connect(wxID_ANY, wxEVT_IDLE,
          (wxObjectEventFunction)(wxEventFunction)(wxIdleEventFunction)&DropDownPopup::OnIdle, 0,
          this);
#endif // USE_POPUP_IDLE
...
void DropDownPopup::Dismiss()
{
  ...
  Disconnect(wxID_ANY, wxEVT_IDLE,
             (wxObjectEventFunction)(wxEventFunction)(wxIdleEventFunction)&DropDownPopup::OnIdle,
             0, this);
#endif // USE_POPUP_IDLE
...
bool DropDownBase::Create(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size,
                          long style, const wxValidator &val, const wxString &name)
{
  if (!wxControl::Create(parent, id, pos, size, wxNO_BORDER | wxCLIP_CHILDREN | style, val,
                         name)) {
    return false;
  }
  ...
  m_dropdownButton =
      new wxCustomButton(this, IDD_DROPDOWN_BUTTON, s_dropdownBitmap, wxDefaultPosition,
...
wxSize DropDownBase::DoGetBestSize() const { return wxSize(95, DROPDOWN_DROP_HEIGHT); }
...
bool DropDownBase::ShowPopup()
{
  ...
  // wxPrintf(wxT("ShowPopup %d %d, %d %d -- %d\n"), width, height, m_popupWin->GetSize().x,
  // m_popupWin->GetSize().y, m_popupWin->GetMinHeight());
...
bool DropDownBase::IsPopupShown() { return m_popupWin && m_popupWin->IsShown(); }
