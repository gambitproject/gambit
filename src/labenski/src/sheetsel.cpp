...
#define PRINT_BLOCK(s, b)                                                                         \
  wxPrintf(wxT("%s %d %d %d %d - w%d h%d\n"), s, b.GetTop(), b.GetLeft(), b.GetBottom(),          \
...
static int wxCMPFUNC_CONV wxsheetcellblock_sort_topleft_bottomright(wxSheetBlock **a,
                                                                    wxSheetBlock **b)
{
...
static int wxCMPFUNC_CONV wxsheetcellblock_sort_bottomright_topleft(wxSheetBlock **a,
                                                                    wxSheetBlock **b)
{
...
wxSheetCoords wxSheetCoords::GetCellCoords(wxSheetCell_Type type) const
{
  ...
  case wxSHEET_CELL_ROWLABEL:
    return wxSheetCoords(m_row, -1 - m_col);
  case wxSHEET_CELL_COLLABEL:
    return wxSheetCoords(-1 - m_row, m_col);
  case wxSHEET_CELL_CORNERLABEL:
    return wxSheetCoords(-1 - m_row, -1 - m_col);
  default:
    break;
  }
...
wxSheetBlock wxSheetBlock::GetAligned() const
{
  return wxSheetBlock(m_height < 0 ? m_row + m_height - 1 : m_row,
                      ...
                      m_height < 0 ? 2 - m_height : m_height, m_width < 0 ? 2 - m_width : m_width);
}
...
{
  if (IsEmpty()) {
    return other; // preserve other block
  }
  if (other.IsEmpty()) {
    return *this; // preserve this
  }
...
bool wxSheetBlock::Combine(const wxSheetBlock &block)
{
  ...
  /*
      ...
      if (((uBlock.GetLeftTop() == GetLeftTop()) || (uBlock.GetLeftTop() == block.GetLeftTop())) &&
          ((uBlock.GetRightTop() == GetRightTop()) || (uBlock.GetRightTop() ==
     block.GetRightTop())) &&
          ((uBlock.GetLeftBottom() == GetLeftBottom()) || (uBlock.GetLeftBottom() ==
     block.GetLeftBottom())) &&
          ((uBlock.GetRightBottom() == GetRightBottom()) || (uBlock.GetRightBottom() ==
     block.GetRightBottom())) )
      {
          *this = uBlock;
          return true;
      }
...
int wxSheetBlock::Combine(const wxSheetBlock &block, wxSheetBlock &top, wxSheetBlock &bottom,
                          wxSheetBlock &left, wxSheetBlock &right) const
{
  wxSheetBlock iBlock(Intersect(block));
  if (iBlock.IsEmpty()) {
    return wxSHEET_BLOCK_NONE; // nothing to combine
  }
  if (Contains(block)) {
    return wxSHEET_BLOCK_ALL; // can combine all of block, no leftover
  }
...
int wxSheetBlock::Delete(const wxSheetBlock &block, wxSheetBlock &top, wxSheetBlock &bottom,
                         wxSheetBlock &left, wxSheetBlock &right) const
{
  wxSheetBlock iBlock(Intersect(block));
  if (iBlock.IsEmpty()) {
    return wxSHEET_BLOCK_NONE; // nothing to delete
  }
  if (block.Contains(*this)) {
    return wxSHEET_BLOCK_ALL; // can delete all of this, no leftover
  }
...
int wxSheetBlock::CmpTopLeft(const wxSheetBlock &b) const
{
  ...
  cmp = m_height - b.m_height;
  if (cmp != 0) {
    return cmp;
  }
  cmp = m_width - b.m_width;
  return cmp;
}
int wxSheetBlock::CmpRightBottom(const wxSheetBlock &b) const
{
  int cmp = GetRight() - b.GetRight();
  if (cmp != 0) {
    return cmp;
  }
  cmp = GetBottom() - b.GetBottom();
  if (cmp != 0) {
    return cmp;
  }
  cmp = m_col - b.m_col;
...
wxSheetSelection::wxSheetSelection(int options) : m_minimized(true), m_options(options) {}
...
int wxSheetSelection::FindInsertIndex(const wxSheetBlock &block) const
{
  ...
  if ((hi == 0) || (block < m_blocks[0])) {
    return 0;
  }
  if (block > m_blocks[hi - 1]) {
    return hi;
  }
...
{
  size_t n, lo = 0, hi = m_blocks.GetCount();
  if ((hi == 0) || (row >= m_blocks[0].GetTop())) {
    return 0;
  }
  if (row < m_blocks[hi - 1u].GetTop()) {
    return hi;
  }
                                   ...
                                   wxArraySheetBlock *addedBlocks)
{
  ...
  for (int m = 1; m < m_blocks.GetCount(); m++) {
    if (m_blocks[m] < m_blocks[m - 1]) {
      PRINT_BLOCK("Not sorted", m_blocks[m]);
    }
  }
...
wxSheetSelectionIterGet_Type wxSheetSelectionIterator::GetNextForward(wxSheetCoords &coord)
{
  ...
  if (m_coords.GetCol() == m_blocks[m_block_index].GetRight()) {
    ...
    return (coord == m_blocks[m_block_index].GetRightBottom())
               ? wxSHEET_SELECTIONITER_GET_RIGHTBOTTOM
               : wxSHEET_SELECTIONITER_GET_NEXTROW;
  }
...
wxSheetSelectionIterGet_Type wxSheetSelectionIterator::GetNextReverse(wxSheetCoords &coord)
{
  ...
  if (m_coords.GetCol() == m_blocks[m_block_index].GetLeft()) {
    ...
    coord = m_coords;
    return (coord == m_blocks[m_block_index].GetLeftTop()) ? wxSHEET_SELECTIONITER_GET_LEFTTOP
                                                           : wxSHEET_SELECTIONITER_GET_NEXTROW;
  }
  // increment the col
  m_coords.m_col--;
...
public:
  wxSheetVariant(const wxSheetVariant &sheetVariant) : m_data(NULL) { Copy(sheetVariant); }
  wxSheetVariant(const wxVariant &variant) : m_data(NULL) { Copy(variant); }
  wxSheetVariant(const wxVariantData &data) : m_data(NULL) { Copy(data); }
  wxSheetVariant(const wxVariantData *data) : m_data(NULL) { Copy(data); }
  ...
  wxSheetVariant &operator=(const wxSheetVariant &other)
  {
    Copy(other);
    return *this;
  }
  wxSheetVariant &operator=(const wxVariant &other)
  {
    Copy(other);
    return *this;
  }
  wxSheetVariant &operator=(const wxVariantData &other)
  {
    Copy(other);
    return *this;
  }
...
void wxSheetVariant::Copy(const wxSheetVariant &other) { Copy(other.GetData()); }
void wxSheetVariant::Copy(const wxVariant &other) { Copy(other.GetData()); }
...
bool wxSheetRowColVariantContainer::UpdateRows(size_t row, int numRows) { return false; }
bool wxSheetRowColVariantContainer::UpdateCols(size_t col, int numCols) { return false; }
