...
#define LONG_TO_WXCOLOUR(c)                                                                       \
  wxColour((unsigned char)((c >> 16) & 0xFF), (unsigned char)((c >> 8) & 0xFF),                   \
...
#define wxPLOT_DRAW_ELLIPSE(dc, win, pen, x0, y0, w, h) dc->DrawEllipse(x0, y0, w, h);
...
{
  if (!wxFinite(x0) || !wxFinite(y0) || !wxFinite(x1) || !wxFinite(y1)) {
    return ClippedOut;
  }
  ...
  if ((out0 & out1) != wxInside) {
    return ClippedOut; // both outside on same side
  }
  if ((out0 | out1) == wxInside) {
    return ClippedNeither; // both inside
  }
...
public:
  ...
  void Create(wxDC *dc, const wxPen &curPen, const wxPen &selPen, const wxRect2DDouble &rect,
...
void SplineDrawer::DrawSpline(double x, double y)
{
  ...
  while (m_stack_count > 0) {
    ...
    if ((fabs(xx1 - xmid) < THRESHOLD) && (fabs(yy1 - ymid) < THRESHOLD) &&
      ...
      SPLINE_PUSH(xmid, ymid, (xmid + xx3) / 2.0, (ymid + yy3) / 2.0, (xx3 + xx4) / 2.0,
                  (yy3 + yy4) / 2.0, xx4, yy4);
      SPLINE_PUSH(xx1, yy1, (xx1 + xx2) / 2.0, (yy1 + yy2) / 2.0, (xx2 + xmid) / 2.0,
                  (yy2 + ymid) / 2.0, xmid, ymid);
    }
  }
...
void wxPlotDrawerCurve::Draw(wxDC *dc, wxPlotCurve *curve, int curve_index)
{
  ...
  // printf("curve subviewrect %lf %lf %lf %lf\n", subViewRect.m_x, subViewRect.m_y,
  // subViewRect.m_width, subViewRect.m_height); fflush(stdout);

  int right = dcRect.GetRight();

  wxPen currentPen = (curve_index == m_owner->GetActiveIndex())
                         ? curve->GetPen(wxPLOTPEN_ACTIVE).GetPen()
...
void wxPlotDrawerDataCurve::Draw(wxDC *dc, wxPlotData *curve, int curve_index)
{
  ...
  if (!wxPlotRect2DDoubleIntersects(curveRect, subViewRect)) {
    return;
  }
  ...
  */
  ...
  if (draw_spline) {
    ...
    sd.Create(dc, currentPen, selectedPen,
              wxRect2DDouble(dcRect.x, dcRect.y, dcRect.width, dcRect.height), &dblRangeSel,
  ...
  for (n = n_start; n < n_end; n++) {
    ...
    if (draw_spline) {
      sd.DrawSpline(m_owner->GetClientCoordFromPlotX(x1), m_owner->GetClientCoordFromPlotY(y1));
    }
  ...
  for (n = 0; n < count; n++) {
    ...
    else // figure out the rest
    {
      ...
      else // rectangle
      {
        ...
        if (r.m_x < subViewRect.m_x) {
          i0 -= pen_width;
        }
        if (r.m_y < subViewRect.m_y) {
          j0 -= pen_width;
        }
        if (r.GetRight() > subViewRect.GetRight()) {
          i1 += pen_width;
        }
        if (r.GetBottom() > subViewRect.GetBottom()) {
          j1 += pen_width;
        }
