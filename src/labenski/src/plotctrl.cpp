...
// #define LONG_TO_WXCOLOUR(c) wxColour((unsigned char)((c>>16)&0xFF), (unsigned char)((c>>8
// )&0xFF), (unsigned char)((c)&0xFF)) #define WXCOLOUR_TO_LONG(c)
...
{
  return (r1.m_x == r1.m_x) && (r1.m_y == r1.m_y) && (r1.m_width == r1.m_width) &&
         (r1.m_height == r1.m_height);
}
#else
...
// inline void wxPLOT_DRAW_LINE(wxDC *dc, GdkWindow *win, GdkGC *pen, int x0, int y0, int x1, int
// y1)
...
//        gdk_draw_arc( win, pen, false, x0-2, y0-2, 4, 4, 0, 360*64 ); // false for outline, true
//        for inside
...
#define wxPLOT_DRAW_LINE(dc, win, pen, x0, y0, x1, y1) dc->DrawLine(x0, y0, x1, y1);

// inline void wxPLOT_DRAW_CIRCLE(wxDC *dc, int win, int pen, int x0, int y0)
#define wxPLOT_DRAW_CIRCLE(dc, win, pen, x0, y0) dc->DrawCircle(x0, y0, 2);
...
#define wxPlotRect2DDoubleOutCode(x, y, rect)                                                     \
  wxOutCode((x < rect.m_x ? wxOutLeft : (x > rect.GetRight() ? wxOutRight : wxInside)) +          \
            (y < rect.m_y ? wxOutTop : (y > rect.GetBottom() ? wxOutBottom : wxInside)))
...
// in plotdraw.cpp
extern int ClipLineToRect(double &x0, double &y0, double &x1, double &y1,
...
#define aDEFINE_LOCAL_EVENT_TYPE(t)                                                               \
  if (event.GetEventType() == t)                                                                  \
    return wxT(#t);

wxString GetEventName(const wxPlotEvent &event)
{
  aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_ADD_CURVE) aDEFINE_LOCAL_EVENT_TYPE(
      wxEVT_PLOT_DELETING_CURVE) aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_DELETED_CURVE)

      aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CURVE_SEL_CHANGING) aDEFINE_LOCAL_EVENT_TYPE(
          wxEVT_PLOT_CURVE_SEL_CHANGED)

          aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_MOUSE_MOTION) aDEFINE_LOCAL_EVENT_TYPE(
              wxEVT_PLOT_CLICKED) aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_DOUBLECLICKED)
              aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_POINT_CLICKED) aDEFINE_LOCAL_EVENT_TYPE(
                  wxEVT_PLOT_POINT_DOUBLECLICKED)

                  aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CREATING) aDEFINE_LOCAL_EVENT_TYPE(
                      wxEVT_PLOT_AREA_SEL_CHANGING) aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CREATED)

                      aDEFINE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VIEW_CHANGING) aDEFINE_LOCAL_EVENT_TYPE(
                          wxEVT_PLOT_VIEW_CHANGED)
                                                  ...
                                                  aDEFINE_LOCAL_EVENT_TYPE(
                                                      ...
                                                      aDEFINE_LOCAL_EVENT_TYPE(
                                                          ...
                                                          aDEFINE_LOCAL_EVENT_TYPE(
                                                              ...
                                                              aDEFINE_LOCAL_EVENT_TYPE(
                                                                  wxEVT_PLOT_MOUSE_FUNC_CHANGING)
                                                                  aDEFINE_LOCAL_EVENT_TYPE(
                                                                      wxEVT_PLOT_MOUSE_FUNC_CHANGED)
...
wxPlotEvent::wxPlotEvent(wxEventType commandType, int id, wxPlotCtrl *window)
  : wxNotifyEvent(commandType, id), m_curve(NULL), m_curveIndex(-1), m_curveData_index(-1), m_x(0),
    m_y(0)
{
...
wxPlotSelectionEvent::wxPlotSelectionEvent(wxEventType commandType, int id, wxPlotCtrl *window)
...
void wxPlotArea::Init() { m_owner = NULL; }
...
void wxPlotArea::OnPaint(wxPaintEvent &WXUNUSED(event))
{
  ...
  /*
     ...
     ); fflush(stdout); Paint( &dc, upd.GetRect() ); upd++;
      }
  */
...
void wxPlotAxis::CreateBitmap()
{
  ...
  if ((clientSize.x < 2) || (clientSize.y < 2)) {
    return;
  }
...
void wxPlotCtrl::Init()
{
  ...
  m_cursorMarker.CreateEllipseMarker(wxPoint2DDouble(0, 0), wxSize(2, 2),
...
bool wxPlotCtrl::Create(wxWindow *parent, wxWindowID win_id, const wxPoint &pos,
                        const wxSize &size, wxPlotAxis_Type WXUNUSED(flag), const wxString &name)
{
  m_redraw_type = wxPLOT_REDRAW_BLOCKER; // no paints until finished

  if (!wxWindow::Create(parent, win_id, pos, size, wxWANTS_CHARS | wxCLIP_CHILDREN, name)) {
    return false;
  }
  ...
  m_xAxisScrollbar = new wxScrollBar(this, ID_PLOTCTRL_X_SCROLLBAR, wxDefaultPosition,
                                     wxDefaultSize, wxSB_HORIZONTAL);
  m_yAxisScrollbar = new wxScrollBar(this, ID_PLOTCTRL_Y_SCROLLBAR, wxDefaultPosition,
...
void wxPlotCtrl::DrawActiveBitmap(wxDC *dc)
{
  if (m_xAxisScrollbar && m_yAxisScrollbar) {
    ...
    dc->DrawBitmap(m_focused ? *m_activeBitmap : *m_inactiveBitmap, pt.x, pt.y, true);
  }
}
...
void wxPlotCtrl::ShowTextCtrl(wxPlotCtrlTextCtrl_Type type, bool send_event)
{
  ...
  case wxPLOT_EDIT_TITLE: {
    ...
    m_textCtrl = new wxTextCtrl(
        this, wxEVT_PLOT_END_TITLE_EDIT, GetPlotTitle(), wxPoint(m_areaRect.x, 0),
        wxSize(m_areaRect.width, m_titleRect.height + 2 * m_border), wxTE_PROCESS_ENTER);
  ...
  case wxPLOT_EDIT_YAXIS: {
    ...
    m_textCtrl = new wxTextCtrl(
        this, wxEVT_PLOT_END_Y_LABEL_EDIT, GetYAxisLabel(),
...
wxColour wxPlotCtrl::GetCursorColour() const { return m_cursorMarker.GetPen().GetColour(); }
int wxPlotCtrl::GetCursorSize() const { return m_cursorMarker.GetSize().x; }
void wxPlotCtrl::SetCursorSize(int size) { m_cursorMarker.SetSize(wxSize(size, size)); }
...
void wxPlotCtrl::SetAxisFont(const wxFont &font)
{
  ...
  if (m_xAxisDrawer) {
    m_xAxisDrawer->SetTickFont(font);
  }
  if (m_yAxisDrawer) {
    m_yAxisDrawer->SetTickFont(font);
  }
...
void wxPlotCtrl::SetAxisColour(const wxColour &colour)
{
  wxCHECK_RET(colour.Ok(), wxT("invalid colour"));
  if (m_xAxisDrawer) {
    m_xAxisDrawer->SetTickColour(colour);
  }
  if (m_yAxisDrawer) {
    m_yAxisDrawer->SetTickColour(colour);
  }
  Redraw(wxPLOT_REDRAW_XAXIS | wxPLOT_REDRAW_YAXIS);
}
...
void wxPlotCtrl::SetAxisLabelFont(const wxFont &font)
{
  wxCHECK_RET(font.Ok(), wxT("invalid font"));
  if (m_xAxisDrawer) {
    m_xAxisDrawer->SetLabelFont(font);
  }
  if (m_yAxisDrawer) {
    m_yAxisDrawer->SetLabelFont(font);
  }
...
void wxPlotCtrl::SetAxisLabelColour(const wxColour &colour)
{
  wxCHECK_RET(colour.Ok(), wxT("invalid colour"));
  if (m_xAxisDrawer) {
    m_xAxisDrawer->SetLabelColour(colour);
  }
  if (m_yAxisDrawer) {
    m_yAxisDrawer->SetLabelColour(colour);
  }
  SetXAxisLabel(GetXAxisLabel()); // FIXME - lazy hack
  SetYAxisLabel(GetYAxisLabel());
}
...
wxPoint wxPlotCtrl::GetKeyPosition() const { return m_keyDrawer->m_keyPosition; }
bool wxPlotCtrl::GetKeyInside() const { return m_keyDrawer->m_key_inside; }
...
bool wxPlotCtrl::AddCurve(wxPlotCurve *curve, bool select, bool send_event)
{
  ...
  m_batch_count++;
  if (select) {
    SetActiveCurve(curve, send_event);
  }
  m_batch_count--;
...
bool wxPlotCtrl::AddCurve(const wxPlotCurve &curve, bool select, bool send_event)
{
  ...
  if (wxDynamicCast(&curve, wxPlotFunction)) {
    return AddCurve(new wxPlotFunction(*wxDynamicCast(&curve, wxPlotFunction)), select,
                    send_event);
  }
...
{
  wxCHECK_RET((marker >= 0) && (marker < (int)m_plotMarkers.GetCount()),
              wxT("Invalid marker number"));
  m_plotMarkers.RemoveAt(marker);
}

void wxPlotCtrl::ClearMarkers() { m_plotMarkers.Clear(); }
...
{
  wxCHECK_MSG(CurveIndexOk(curve_index) && GetDataCurve(curve_index), false,
              wxT("invalid curve index"));
...
bool wxPlotCtrl::SetCursorXPoint(int curve_index, double x, bool send_event)
{
  ...
  if (GetDataCurve(curve_index)) {
    return SetCursorDataIndex(curve_index, GetDataCurve(curve_index)->GetIndexFromX(x),
                              send_event);
  }
...
void wxPlotCtrl::MakeCursorVisible(bool center, bool send_event)
{
  ...
  if (center) {
    wxPoint2DDouble origin = m_viewRect.GetLeftTop() - m_viewRect.GetCentre() + GetCursorPoint();
...
bool wxPlotCtrl::HasSelection(int curve_index) const
{
  if (curve_index == -1) {
    ...
    for (n = 0; n < count; n++) {
      if ((m_curveSelections[n].GetCount() > 0) || (m_dataSelections[n].GetCount() > 0)) {
        return true;
      }
    }
    return false;
...
bool wxPlotCtrl::DoSelectRectangle(int curve_index, const wxRect2DDouble &rect, bool select,
                                   bool send_event)
{
  wxCHECK_MSG((curve_index >= -1) && (curve_index < int(m_curves.GetCount())), false,
              wxT("invalid plotcurve index"));
  ...
  if (plotData) {
    ...
    if (done) {
      RedrawCurve(curve_index, xRange.m_min, xRange.m_max);
    }
...
bool wxPlotCtrl::DoSelectDataRange(int curve_index, const wxRangeInt &range, bool select,
                                   bool send_event)
{
  ...
  wxPlotData *plotData = GetDataCurve(curve_index);
  wxCHECK_MSG(plotData && (range.m_min >= 0) && (range.m_max < (int)plotData->GetCount()), false,
              wxT("invalid index"));
  ...
  if (done) {
    RedrawDataCurve(curve_index, range.m_min, range.m_max);
  }
...
void wxPlotCtrl::ShiftOrigin( int dx, int dy, bool send_event )
{
    ...
    if (dx != 0)
    {
        rx = wxRect((dx>0 ? -5 : m_areaClientRect.width+dx-5), 0, labs(dx)+10,
m_areaClientRect.height); RedrawXAxis(false);
    }
    if (dy != 0)
    {
        ry = wxRect(0, (dy>0 ? -5 : m_areaClientRect.height+dy-5), m_areaClientRect.width,
labs(dy)+10); RedrawYAxis(false);
    }

    printf("Shift %d %d rx %d %d %d %d, ry %d %d %d %d\n", dx, dy, rx.x, rx.y, rx.width, rx.height,
ry.x, ry.y, ry.width, ry.height); fflush(stdout);
...
bool wxPlotCtrl::SetZoom(double zoom_x, double zoom_y, double origin_x, double origin_y,
                         bool send_event)
{
  ...
  if (!IsFinite(zoom_x, wxT("X zoom is NaN"))) {
    return false;
  }
  if (!IsFinite(zoom_y, wxT("Y zoom is NaN"))) {
    return false;
  }
  if (!IsFinite(origin_x, wxT("X origin is not finite"))) {
    return false;
  }
  if (!IsFinite(origin_y, wxT("Y origin is not finite"))) {
    return false;
  }
  if (!IsFinite(view_width, wxT("Plot width is NaN"))) {
    return false;
  }
  if (!IsFinite(view_height, wxT("Plot height is NaN"))) {
    return false;
  }
...
{
  wxCHECK_RET(wxFinite(rect.m_x) && wxFinite(rect.m_y) && wxFinite(rect.GetRight()) &&
                  wxFinite(rect.GetBottom()),
              wxT("bounding rect is NaN"));
  wxCHECK_RET((rect.m_width > 0) && (rect.m_height > 0), wxT("Plot Size < 0"));
  m_defaultPlotRect = rect;
  CalcBoundingPlotRect();
  SetZoom(m_areaClientRect.width / rect.m_width, m_areaClientRect.height / rect.m_height, rect.m_x,
          rect.m_y, send_event);
}

void wxPlotCtrl::AddHistoryView()
{
  if (!(wxFinite(m_viewRect.GetLeft()) && wxFinite(m_viewRect.GetRight()) &&
        wxFinite(m_viewRect.GetTop()) && wxFinite(m_viewRect.GetBottom()))) {
    return;
  }

  if ((m_history_views_index >= 0) && (m_history_views_index < int(m_historyViews.GetCount())) &&
...
void wxPlotCtrl::OnSize(wxSizeEvent &) { DoSize(); }

void wxPlotCtrl::DoSize(const wxRect &boundingRect)
{
  if (!m_yAxisScrollbar) {
    return; // we're not created yet
  }
  ...
  int area_height =
      m_clientRect.height - titleRect.GetBottom() - xaxis_height - xLabelRect.height - area_border;

  m_yAxisRect = wxRect(yLabelRect.GetRight(), titleRect.GetBottom(), yaxis_width,
  ...
  m_areaRect = wxRect(m_yAxisRect.GetRight() + area_border, m_yAxisRect.GetTop() + area_border,
  ...
  if (!IsFinite(zoom_x, wxT("Plot zoom is NaN"))) {
    return;
  }
  if (!IsFinite(zoom_y, wxT("Plot zoom is NaN"))) {
    return;
  }
...
void wxPlotCtrl::CalcBoundingPlotRect()
{
  ...
  if (count > 0) {
    ...
    if (IsFinite(rect.m_x, wxT("left curve boundary is NaN")) &&
        ...
        IsFinite(rect.GetBottom(), wxT("top curve boundary is NaN")) && (rect.m_width >= 0) &&
        (rect.m_height >= 0)) {
      valid_rect = true;
    }
    else {
...
void wxPlotCtrl::RedrawDataCurve(int index, int min_index, int max_index)
{
  ...
  int count = plotData->GetCount();
  wxCHECK_RET((min_index <= max_index) && (min_index >= 0) && (max_index >= 0) &&
                  (min_index < count) && (max_index < count),
...
void wxPlotCtrl::RedrawCurve(int index, double min_x, double max_x)
{
  ...
  wxCHECK_RET((min_x <= max_x) && (index >= 0) && (index < (int)m_curves.GetCount()),
              wxT("invalid curve index"));
...
void wxPlotCtrl::ProcessAreaEVT_MOUSE_EVENTS(wxMouseEvent &event)
{
  ...
  if (event.GetWheelRotation() != 0) {
    ...
    SetOrigin(m_viewRect.GetLeft(), m_viewRect.GetTop() + dir * m_viewRect.m_height, true);
  }
  ...
  if (event.LeftDown() || event.LeftDClick()) {
    ...
    if (FindCurve(wxPoint2DDouble(plotX, plotY), dpt, curve_index, data_index, &curvePt)) {
      ...
      if (plotCurve) {
        wxPlotEvent pt_click_event(event.ButtonDClick() ? wxEVT_PLOT_POINT_DOUBLECLICKED
                                                        : wxEVT_PLOT_POINT_CLICKED,
                                   GetId(), this);
  ...
  // Finished marking rectangle or scrolling, perhaps
    ...
    if (m_area_mouse_func == wxPLOT_MOUSE_ZOOM) {
      ...
      else if ((m_area_mouse_marker == wxPLOT_MARKER_VERT) && (rightedRect.width > 10)) {
        SetZoom(wxRect(rightedRect.x, 0, rightedRect.width, m_areaClientRect.height), true);
      }
      else if ((m_area_mouse_marker == wxPLOT_MARKER_HORIZ) && (rightedRect.height > 10)) {
  ...
  // Marking the rectangle or panning around
    ...
    if (m_area_mouse_func == wxPLOT_MOUSE_PAN) {
      ...
      SetOrigin(m_viewRect.GetLeft() - dx / m_zoom.m_x, m_viewRect.GetTop() + dy / m_zoom.m_y,
                true);
      return;
    }
    else {
...
void wxPlotCtrl::ProcessAxisEVT_MOUSE_EVENTS(wxMouseEvent &event)
{
  ...
  if (wheel != 0) {
    ...
    SetOrigin(m_viewRect.GetLeft() + dx, m_viewRect.GetTop() + dy, true);
  }
...
void wxPlotCtrl::ProcessAreaEVT_KEY_DOWN(wxKeyEvent &event)
{
  ...
  // int(event.GetKeyCode()), (wxChar)event.GetKeyCode(), event.ShiftDown(), event.ControlDown(),
  // event.AltDown());
  event.Skip(true);
...
void wxPlotCtrl::OnChar(wxKeyEvent &event)
{
    ...
    if (event.ShiftDown() || event.ControlDown()) {
      if (IsCursorValid()) {
        if (GetDataCurve(m_cursor_curve)) {
          DoSelectDataRange(m_cursor_curve, wxRangeInt(m_cursor_index, m_cursor_index),
                            !event.ControlDown(), true);
        }
        else {
          wxPoint2DDouble pt(m_cursorMarker.GetPlotPosition());
          DoSelectRectangle(m_cursor_curve, wxRect2DDouble(pt.m_x, 0, pt.m_x, 0),
                            !event.ControlDown(), true);
        }
      }
    }
      ...
      if ((count < 1) || ((count == 1) && (m_active_index == 0))) {
        return;
      }
  ...
  case WXK_LEFT:
    SetOrigin(m_viewRect.GetLeft() - m_viewRect.m_width / 10.0, m_viewRect.GetTop());
    return;
  case WXK_RIGHT:
    SetOrigin(m_viewRect.GetLeft() + m_viewRect.m_width / 10.0, m_viewRect.GetTop());
    return;
  case WXK_UP:
    SetOrigin(m_viewRect.GetLeft(), m_viewRect.GetTop() + m_viewRect.m_height / 10.0);
    return;
  case WXK_DOWN:
    SetOrigin(m_viewRect.GetLeft(), m_viewRect.GetTop() - m_viewRect.m_height / 10.0);
    return;
  case WXK_PAGEUP:
    SetOrigin(m_viewRect.GetLeft(), m_viewRect.GetTop() + m_viewRect.m_height / 2.0);
    return;
  case WXK_PAGEDOWN:
    SetOrigin(m_viewRect.GetLeft(), m_viewRect.GetTop() - m_viewRect.m_height / 2.0);
    return;
  ...
  // zoom in and out
  case wxT('a'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x / 1.5, m_zoom.m_y), true);
    return;
  case wxT('d'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x * 1.5, m_zoom.m_y), true);
    return;
  case wxT('w'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x, m_zoom.m_y * 1.5), true);
    return;
  case wxT('x'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x, m_zoom.m_y / 1.5), true);
    return;
  ...
  case wxT('q'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x / 1.5, m_zoom.m_y * 1.5), true);
    return;
  case wxT('e'):
    SetZoom(wxPoint2DDouble(m_zoom.m_x * 1.5, m_zoom.m_y * 1.5), true);
    return;
  ...
  case wxT(','): {
    ...
    if (plotData) {
      if (!IsCursorValid()) {
        SetCursorDataIndex(m_active_index, plotData->GetIndexFromX(x, wxPlotData::index_floor),
                           true);
      }
      ...
      if (!IsCursorValid()) {
        ...
        x = GetPlotCoordFromClientX(
            (GetClientCoordFromPlotX(m_cursorMarker.GetPlotRect().m_x) - 1));
  ...
  case wxT('.'): {
    ...
    if (plotData) {
      ...
      if (!IsCursorValid()) {
        SetCursorDataIndex(m_active_index, plotData->GetIndexFromX(x, wxPlotData::index_ceil),
                           true);
      }
  ...
  case WXK_DELETE: {
    if (m_activeCurve) {
      DeleteCurve(m_activeCurve, true);
    }
    return;
  }
...
void wxPlotCtrl::UpdateWindowSize()
{
  ...
  if (m_areaClientRect.width < 10) {
    m_areaClientRect.width = 10;
  }
  if (m_areaClientRect.height < 10) {
    m_areaClientRect.height = 10;
  }
}
...
void wxPlotCtrl::AdjustScrollBars()
{
  ...
  if (!IsFinite(range, wxT("plot's x range is NaN"))) {
    return;
  }
  if (range > 32000) {
    range = 32000;
  ...
  if (!IsFinite(thumbsize, wxT("plot's x range is NaN"))) {
    return;
  }
  if (thumbsize > range) {
    thumbsize = range;
  ...
  position = (range * ((m_viewRect.GetLeft() - m_curveBoundingRect.GetLeft()) /
                       m_curveBoundingRect.m_width));
  if (!IsFinite(position, wxT("plot's x range is NaN"))) {
    return;
  }
  ...
  if (!IsFinite(range, wxT("plot's y range is NaN"))) {
    return;
  }
  if (range > 32000) {
    range = 32000;
  ...
  position = (range -
              range * ((m_viewRect.GetTop() - m_curveBoundingRect.GetTop()) /
                       m_curveBoundingRect.m_height) -
              thumbsize);
  if (!IsFinite(position, wxT("plot's x range is NaN"))) {
    return;
  }
...
void wxPlotCtrl::OnScroll(wxScrollEvent &event)
{
  ...
  if (event.GetId() == wxPlotCtrl::ID_PLOTCTRL_X_SCROLLBAR) {
    ...
    double origin_x =
        m_curveBoundingRect.GetLeft() + m_curveBoundingRect.m_width * (position / range);
    ...
    double origin_y = m_curveBoundingRect.GetTop() +
                      m_curveBoundingRect.m_height * ((range - position - thumbsize) / range);
...
bool wxPlotCtrl::FindCurve(const wxPoint2DDouble &pt, const wxPoint2DDouble &dpt, int &curve_index,
                           int &data_index, wxPoint2DDouble *curvePt) const
{
  curve_index = data_index = -1;

  if (!IsFinite(pt.m_x, wxT("point is not finite"))) {
    return false;
  }
  if (!IsFinite(pt.m_y, wxT("point is not finite"))) {
    return false;
  }
  if (!IsFinite(dpt.m_x, wxT("point is not finite"))) {
    return false;
  }
  if (!IsFinite(dpt.m_y, wxT("point is not finite"))) {
    return false;
  }
  ...
  for (int n = -1; n < curve_count; n++) {
    ...
    else // not a data curve, just find y at this x pos
    {
      ...
      if ((rect.m_width <= 0) ||
          ((pt.m_x + dpt.m_x >= rect.GetLeft()) && (pt.m_x - dpt.m_x <= rect.GetRight()))) {
        if ((rect.m_height <= 0) ||
            ((pt.m_y >= rect.GetTop()) && (pt.m_y - dpt.m_y <= rect.GetBottom()))) {
          double y = plotCurve->GetY(pt.m_x);
          if (fabs(y - pt.m_y) <= dpt.m_y) {
            curve_index = n;
            if (curvePt) {
              *curvePt = wxPoint2DDouble(pt.m_x, y);
            }
            return true;
          }
        }
...
bool wxPlotCtrl::DoSendEvent(wxPlotEvent &event) const
{
  /*
      ...
      {
          wxLogDebug(wxT("wxPlotEvent '%s' CurveIndex: %d, DataIndex: %d, Pos: %lf %lf, MouseFn
     %d"), GetEventName(event).c_str(), event.GetCurveIndex(), event.GetCurveDataIndex(),
...
bool wxPlotCtrl::IsTimerRunning() { return (m_timer && m_timer->IsRunning()); }
...
void wxPlotCtrl::OnTimer(wxTimerEvent &event)
{
  ...
  switch (event.GetId()) {
  case ID_AREA_TIMER:
    mousePt = m_area->m_mousePt;
    break;
  case ID_XAXIS_TIMER:
    mousePt = m_xAxis->m_mousePt;
    break;
  case ID_YAXIS_TIMER:
    mousePt = m_yAxis->m_mousePt;
    break;
  default: {
