...
// LM_LeastSquare
//=============================================================================
LM_LeastSquare::LM_LeastSquare() { Init(); }
...
bool LM_LeastSquare::Create(const wxPlotData &plotData, const wxPlotFunction &plotFunc)
{
  ...
  wxCHECK_MSG((plotData.GetCount() > 1) &&
                  (plotData.GetCount() >= plotFunc.GetNumberVariables() - 1),
              false, wxT("invalid dimensions"));
  ...
  if (!(m_plotData && m_plotFunc && m_vars && m_x && m_fvec && m_diag && m_fjac && m_qtf &&
        m_ipvt)) {
    Destroy();
...
int LM_LeastSquare::Fit(const double *x0, int init_count)
{
  ...
  lmdif(m_m, m_n, m_x, m_fvec, m_ftol, m_xtol, m_gtol, m_maxfev, m_epsfcn, m_diag, m_mode,
        m_factor, s_nprint, &m_info, &m_nfev, m_fjac, m_ldfjac, m_ipvt, m_qtf, wa1, wa2, wa3, wa4);
...
wxString LM_LeastSquare::GetResultMessage() const
{
  ...
  case 0:
    ...
    break;
  case 1:
    msg.Printf(wxT("Both actual and predicted relative reductions in the sum of squares are at "
                   "most ftol (%lg)."),
               m_ftol);
    break;
  case 2:
    msg.Printf(wxT("Relative error between two consecutive iterates is at most xtol (%lg)."),
               m_xtol);
    break;
  case 3:
    msg.Printf(wxT("Both actual and predicted relative reductions in the sum of squares are at "
               ...
               m_ftol);
    msg += wxString::Format(
        wxT("Relative error between two consecutive iterates is at most xtol (%lg)."), m_xtol);
    break;
  case 4:
    msg.Printf(wxT("The cosine of the angle between fvec and any column of the jacobian is at "
                   "most gtol (%lg) in absolute value."),
               m_gtol);
    break;
  case 5:
    msg.Printf(
        wxT("Number of iterations has reached or exceeded %d, try adjusting initial values."),
        m_maxfev);
    break;
  case 6:
    msg.Printf(
        wxT("ftol (%lg) is too small. no further reduction in the sum of squares is possible"),
        m_ftol);
    break;
  case 7:
    msg.Printf(
        wxT("xtol (%lg) too small. no further improvement in approximate solution x possible"),
        m_xtol);
    break;
  case 8:
    msg.Printf(wxT("gtol (%lg) is too small. fvec is orthogonal to the columns of the jacobian to "
                   "machine precision."),
               m_gtol);
    break;
  default:
...
void LM_LeastSquare::lmdif(int m, int n, double x[], double fvec[], double ftol, double xtol,
...
void LM_LeastSquare::lmpar(int n, double r[], int ldr, int ipvt[], double diag[], double qtb[],
...
void LM_LeastSquare::qrsolv(int n, double r[], int ldr, int ipvt[], double diag[], double qtb[],
