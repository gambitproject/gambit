...
public:
  explicit wxGenericColourRefData(unsigned char r = 0, unsigned char g = 0, unsigned char b = 0,
                                  unsigned char a = 255)
...
//----------------------------------------------------------------------------
wxObjectRefData *wxGenericColour::CreateRefData() const { return new wxGenericColourRefData; }
...
void wxGenericColour::Create(const wxGenericColour &c) { Ref(c); }
...
void wxGenericColour::Create(unsigned char red, unsigned char green, unsigned char blue,
                             unsigned char alpha)
{
  UnRef();
...
void wxGenericColour::Set(unsigned char red, unsigned char green, unsigned char blue,
                          unsigned char alpha)
{
...
bool wxGenericColour::IsSameAs(const wxColour &c) const
{
  ...
  return (M_GCOLOURDATA->m_r == c.Red()) && (M_GCOLOURDATA->m_g == c.Green()) &&
         (M_GCOLOURDATA->m_b == c.Blue());
}
...
public:
  explicit wxGenericPenRefData(int width = 1, wxPenStyle style = wxPENSTYLE_SOLID,
    ...
    : wxObjectRefData(), m_width(width), m_style(style), m_cap(cap), m_join(join), m_dash_count(0),
      m_dash(NULL)
  {
  }

  wxGenericPenRefData(const wxGenericPenRefData &data)
    : wxObjectRefData(), m_colour(data.m_colour), m_width(data.m_width), m_style(data.m_style),
      m_cap(data.m_cap), m_join(data.m_join), m_dash_count(data.m_dash_count), m_dash(NULL)
  {
    if (data.m_dash) {
...
//----------------------------------------------------------------------------
wxObjectRefData *wxGenericPen::CreateRefData() const { return new wxGenericPenRefData; }
...
void wxGenericPen::Create(const wxGenericPen &pen) { Ref(pen); }
...
void wxGenericPen::Create(const wxGenericColour &colour, int width, wxPenStyle style, wxPenCap cap,
                          wxPenJoin join)
{
  UnRef();
...
void wxGenericPen::Create(const wxColour &colour, int width, wxPenStyle style, wxPenCap cap,
                          wxPenJoin join)
{
...
{
  wxCHECK_RET(Ok(), wxT("Invalid generic pen"));
  wxCHECK_RET(((number_of_dashes == 0) && !dash) || ((number_of_dashes > 0) && dash),
              wxT("Invalid dashes for pen"));

  // internal double check to see if somebody's messed with this
  // wxCHECK_RET(((M_GPENDATA->m_dash_count == 0) && !M_GPENDATA->m_dash) ||
  //            ((M_GPENDATA->m_dash_count != 0) &&  M_GPENDATA->m_dash), wxT("Invalid internal
  //            dashes for pen"));
...
bool wxGenericPen::IsSameAs(const wxGenericPen &pen) const
{
  ...
  if (M_GPENDATA->m_dash_count > 0) {
    return memcmp(M_GPENDATA->m_dash, pen.GetDash(), M_GPENDATA->m_dash_count * sizeof(wxDash)) ==
           0;
  }
...
public:
  wxGenericBrushRefData(const wxGenericColour &c = wxNullGenericColour,
    ...
    : wxObjectRefData(), m_colour(c), m_style(style)
  {
  }

  wxGenericBrushRefData(const wxGenericBrushRefData &data)
    : wxObjectRefData(), m_colour(data.m_colour), m_style(data.m_style), m_stipple(data.m_stipple)
  {
  }
...
//----------------------------------------------------------------------------
wxObjectRefData *wxGenericBrush::CreateRefData() const { return new wxGenericBrushRefData; }
...
void wxGenericBrush::Create(const wxGenericBrush &brush) { Ref(brush); }
...
void wxGenericBrush::SetStipple(const wxBitmap &stipple)
{
  wxCHECK_RET(Ok(), wxT("Invalid generic brush"));
  M_GBRUSHDATA->m_stipple = stipple;
  M_GBRUSHDATA->m_style =
      stipple.GetMask() ? wxBRUSHSTYLE_STIPPLE_MASK_OPAQUE : wxBRUSHSTYLE_STIPPLE;
}
