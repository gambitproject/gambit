...
{
  wxCHECK_MSG((index >= 0) && (index < int(m_ranges.GetCount())), wxEmptyRangeInt,
              wxT("Invalid index"));
  ...
  for (i = 1; i < int(m_ranges.GetCount()); i++) {
    if (m_ranges[i - 1].Contains(m_ranges[i])) {
      printf("Error in Selecting ranges %d %d, %d %d count %d\n", m_ranges[i - 1].m_min,
             m_ranges[i - 1].m_max, m_ranges[i].m_min, m_ranges[i].m_max, m_ranges.GetCount());
    }
...
{
  wxCHECK_MSG((index >= 0) && (index < int(m_ranges.GetCount())), wxEmptyRangeDouble,
              wxT("Invalid index"));
  return m_ranges[index];
}
