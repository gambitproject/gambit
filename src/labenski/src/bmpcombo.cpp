...
#include "wx/wxthings/bmpcombo.h"
...
bool wxBitmapComboBox::Create(wxWindow *parent, wxWindowID id, const wxPoint &pos,
...
void wxBitmapComboBox::CalcLabelBitmapPos(int n, const wxSize &area, wxPoint &labelPos,
                                          wxPoint &bitmapPos) const
{
  ...
  else // if ((m_label_style & wxBMPCOMBO_LEFT) != 0)
  {
    labelPos = wxPoint(BORDER, (area.y - lh) / 2);
    bitmapPos =
        wxPoint(BORDER * 2 + m_labelSize.x + (area.x - BORDER * 2 - m_labelSize.x - bw) / 2,
                (area.y - bh) / 2);
  }
}
