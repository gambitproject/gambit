...
public:
  ...
  wxPlotMarkerRefData(const wxPlotMarkerRefData &data)
    : wxObjectRefData(), m_markerType(data.m_markerType), m_rect(data.m_rect), m_size(data.m_size),
      m_bitmap(data.m_bitmap), m_pen(data.m_pen), m_brush(data.m_brush)
  {
  }
...
wxObjectRefData *wxPlotMarker::CreateRefData() const { return new wxPlotMarkerRefData; }
...
void wxPlotMarker::Create(int marker_type, const wxRect2DDouble &rect, const wxSize &size,
                          const wxGenericPen &pen, const wxGenericBrush &brush,
