...
#define PRINT_BLOCK(s, b)                                                                         \
  wxPrintf(wxT("%s %d %d %d %d - w%d h%d\n"), wxT(s), b.GetTop(), b.GetLeft(), b.GetBottom(),     \
           b.GetRight(), b.GetWidth(), b.GetHeight());
#define PRINT_RECT(s, b)                                                                          \
  wxPrintf(wxT("%s %d %d %d %d - w%d h%d\n"), wxT(s), b.GetTop(), b.GetLeft(), b.GetBottom(),     \
           b.GetRight(), b.GetWidth(), b.GetHeight());
...
public:
  wxSheetDataTypeInfo(const wxString &typeName, const wxSheetCellRenderer &renderer,
...
public:
  ...
  int RegisterDataType(const wxString &typeName, const wxSheetCellRenderer &renderer,
                       const wxSheetCellEditor &editor);
...
wxSheetChildWindow::wxSheetChildWindow(wxSheet *parent, wxWindowID id, const wxPoint &pos,
                                       const wxSize &size, long style, const wxString &name)
  : wxWindow(parent, id, pos, size, style, name), m_owner(parent),
...
int wxSheetRefData::FindSheet(wxSheet *sheet) const { return m_sheets.Index(sheet); }
...
wxBEGIN_FLAGS(wxSheetStyle)
    ...
    wxFLAGS_MEMBER(wxBORDER_SIMPLE) wxFLAGS_MEMBER(wxBORDER_SUNKEN) wxFLAGS_MEMBER(wxBORDER_DOUBLE)
        wxFLAGS_MEMBER(wxBORDER_RAISED) wxFLAGS_MEMBER(wxBORDER_STATIC)
            wxFLAGS_MEMBER(wxBORDER_NONE)

    // old style border flags
    wxFLAGS_MEMBER(wxSIMPLE_BORDER) wxFLAGS_MEMBER(wxSUNKEN_BORDER) wxFLAGS_MEMBER(wxDOUBLE_BORDER)
        wxFLAGS_MEMBER(wxRAISED_BORDER) wxFLAGS_MEMBER(wxSTATIC_BORDER) wxFLAGS_MEMBER(wxBORDER)

    // standard window styles
    wxFLAGS_MEMBER(wxTAB_TRAVERSAL) wxFLAGS_MEMBER(wxCLIP_CHILDREN)
        wxFLAGS_MEMBER(wxTRANSPARENT_WINDOW) wxFLAGS_MEMBER(wxWANTS_CHARS)
                ...
                wxFLAGS_MEMBER(wxVSCROLL) wxFLAGS_MEMBER(wxHSCROLL)
                    ...
                    wxEND_FLAGS(wxSheetStyle)
                        ...
                        IMPLEMENT_DYNAMIC_CLASS_XTI(wxSheet, wxWindow, "wx/sheet/sheet.h")
                            ...
                            wxBEGIN_PROPERTIES_TABLE(wxSheet) wxHIDE_PROPERTY(Children)
                                wxPROPERTY_FLAGS(WindowStyle, wxSheetStyle, long,
                                                 SetWindowStyleFlag, GetWindowStyleFlag,
                                                 EMPTY_MACROVALUE, 0 /*flags*/, wxT("Helpstring"),
                                                 wxT("group")) // style
    wxEND_PROPERTIES_TABLE()

        wxBEGIN_HANDLERS_TABLE(wxSheet) wxEND_HANDLERS_TABLE()

            wxCONSTRUCTOR_5(wxSheet, wxWindow *, Parent, wxWindowID, Id, wxPoint, Position, wxSize,
                            Size, long, WindowStyle)

/*
 TODO : Expose more information of a list's layout etc. via appropriate objects (a la
 NotebookPageInfo)
*/
#else
IMPLEMENT_DYNAMIC_CLASS(wxSheet, wxWindow)
                ...
                BEGIN_EVENT_TABLE(wxSheet, wxWindow) EVT_PAINT(wxSheet::OnPaint)
                    EVT_SIZE(wxSheet::OnSize)
    // EVT_ERASE_BACKGROUND( wxSheet::OnEraseBackground )
    EVT_KEY_DOWN(wxSheet::OnKeyDown) EVT_KEY_UP(wxSheet::OnKeyUp) EVT_CHAR(wxSheet::OnChar)
        EVT_MOUSEWHEEL(wxSheet::OnMouseWheel) EVT_MOUSE_EVENTS(wxSheet::OnMouse)
...
bool wxSheet::Create(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size,
                     long style, const wxString &name)
{
  ...
  m_horizScrollBar =
      new wxScrollBar(this, ID_HORIZ_SCROLLBAR, wxDefaultPosition, wxDefaultSize, wxSB_HORIZONTAL);
  m_vertScrollBar =
      new wxScrollBar(this, ID_VERT_SCROLLBAR, wxDefaultPosition, wxDefaultSize, wxSB_VERTICAL);
  ...
  GetTextExtent(wxT("W1!(jJ"), &char_w, &char_h, NULL, NULL, &labelFont);
    ...
    defined(__WXGTK__) // see also text ctrl sizing in ShowCellEditControl()
  char_h += 8;
#else
...
wxSheet::~wxSheet()
{
  ...
  wxPrintf(_T("wxSheet attribute cache statistics: "
              "total: %u, hits: %u (%u%%)\n"),
           total, gs_nAttrCacheHits, total ? (gs_nAttrCacheHits * 100) / total : 0);
#endif
}
...
bool wxSheet::CreateGrid(int numRows, int numCols, int options)
{
  ...
  // table->SetGridCellValueProvider(new wxSheetValueProviderSparseString(numRows, numCols,
  // options), true);
  ...
  table->SetColLabelValueProvider(
      new wxSheetValueProviderString(1, numCols, wxSHEET_ValueProviderColPref), true);
...
{
  wxCHECK_MSG(ContainsCell(coords), wxSize(GetDefaultColWidth(), GetDefaultRowHeight()),
              wxT("Invalid coords"));
...
{
  wxCHECK_MSG(ContainsGridRow(row), GetDefaultRowHeight(),
              wxT("Invalid coords in wxSheet::GetRowBestHeight"));
...
{
  wxCHECK_MSG(ContainsGridCol(col), GetDefaultColWidth(),
              wxT("Invalid col in wxSheet::GetColBestWidth"));
  wxClientDC dc(m_gridWin);
  wxCoord bestWidth = 0;
...
bool wxSheet::HasSpannedCells() const { return GetTable() && GetTable()->HasSpannedCells(); }
...
wxSheetCellAttr wxSheet::GetOrCreateAttr(const wxSheetCoords &coords, wxSheetAttr_Type type) const
{
  ...
  wxCHECK_MSG(type != wxSHEET_AttrAny, attr,
              wxT("Cannot create attribute of type wxSHEET_AttrAny"));

  if (IsGridCell(coords)) {
    switch (type) {
    case wxSHEET_AttrDefault:
      return DoGetDefaultGridAttr();
    case wxSHEET_AttrCell:
    case wxSHEET_AttrRow:
    case wxSHEET_AttrCol: {
      wxCHECK_MSG(GetTable() && ContainsGridCell(coords), attr,
                  wxT("Invalid table or attr coords"));
    ...
    case wxSHEET_AttrCell:
      return DoGetDefaultCornerLabelAttr();
    default: {
      wxFAIL_MSG(wxT("Grid cell attribute kind invalid for GetOrCreateAttr"));
      return attr;
    }
    ...
    switch (type) {
    case wxSHEET_AttrDefault:
      return DoGetDefaultRowLabelAttr();
    case wxSHEET_AttrCell: {
      wxCHECK_MSG(GetTable() && ContainsRowLabelCell(coords), attr,
                  wxT("Invalid table or attr coords"));
      attr = GetTable()->GetAttr(coords, type);
      if (!attr.Ok()) {
        attr.Create();
    ...
    switch (type) {
    case wxSHEET_AttrDefault:
      return DoGetDefaultColLabelAttr();
    case wxSHEET_AttrCell: {
      wxCHECK_MSG(GetTable() && ContainsColLabelCell(coords), attr,
                  wxT("Invalid table or attr coords"));
      attr = GetTable()->GetAttr(coords, type);
      if (!attr.Ok()) {
        attr.Create();
  ...
  wxFAIL_MSG(wxString::Format(wxT("Unable to get or create attribute for cell(%d,%d)"),
...
wxSheetCellAttr wxSheet::GetAttr(const wxSheetCoords &coords, wxSheetAttr_Type type) const
{
  if (IsGridCell(coords)) {
    ...
    case wxSHEET_AttrCell:
    case wxSHEET_AttrAny: {
      wxCHECK_MSG(ContainsRowLabelCell(coords), DoGetDefaultRowLabelAttr(),
                  wxT("Invalid attr coords"));
      wxSheetCellAttr attr;
      if (GetTable()) {
        attr = GetTable()->GetAttr(coords, type);
      }
    ...
    case wxSHEET_AttrCell:
    case wxSHEET_AttrAny: {
      wxCHECK_MSG(ContainsColLabelCell(coords), DoGetDefaultColLabelAttr(),
                  wxT("Invalid attr coords"));
      wxSheetCellAttr attr;
      if (GetTable()) {
        attr = GetTable()->GetAttr(coords, type);
  ...
  wxFAIL_MSG(wxString::Format(wxT("Unable to get attribute for cell(%d,%d)"), coords.m_row,
                              coords.m_col));
  return wxSheetCellAttr(true);
}

void wxSheet::SetAttr(const wxSheetCoords &coords, const wxSheetCellAttr &attr_,
                      wxSheetAttr_Type type)
{
  ...
  wxFAIL_MSG(wxString::Format(wxT("Unable to set attribute for cell(%d,%d)"), coords.m_row,
                              coords.m_col));
}
...
const wxColour &wxSheet::GetAttrBackgroundColour(const wxSheetCoords &coords,
                                                 wxSheetAttr_Type type) const
{
  return GetAttr(coords, type).GetBackgroundColour();
}
const wxColour &wxSheet::GetAttrForegoundColour(const wxSheetCoords &coords,
                                                wxSheetAttr_Type type) const
{
...
wxSheetCellRenderer wxSheet::GetAttrRenderer(const wxSheetCoords &coords,
                                             wxSheetAttr_Type type) const
{
...
void wxSheet::SetAttrBackgroundColour(const wxSheetCoords &coords, const wxColour &colour,
                                      wxSheetAttr_Type type)
{
  GetOrCreateAttr(coords, type).SetBackgroundColour(colour);
  RefreshAttrChange(coords, type);
}
void wxSheet::SetAttrForegroundColour(const wxSheetCoords &coords, const wxColour &colour,
                                      wxSheetAttr_Type type)
{
...
void wxSheet::SetAttrOverflowMarker(const wxSheetCoords &coords, bool draw_marker,
                                    wxSheetAttr_Type type)
{
  GetOrCreateAttr(coords, type).SetOverflowMarker(draw_marker);
  RefreshAttrChange(coords, type);
}
void wxSheet::SetAttrShowEditor(const wxSheetCoords &coords, bool show_editor,
                                wxSheetAttr_Type type)
{
...
void wxSheet::SetAttrRenderer(const wxSheetCoords &coords, const wxSheetCellRenderer &renderer,
                              wxSheetAttr_Type type)
{
  GetOrCreateAttr(coords, type).SetRenderer(renderer);
  RefreshAttrChange(coords, type);
}
void wxSheet::SetAttrEditor(const wxSheetCoords &coords, const wxSheetCellEditor &editor,
                            wxSheetAttr_Type type)
{
...
void wxSheet::RegisterDataType(const wxString &typeName, const wxSheetCellRenderer &renderer,
                               const wxSheetCellEditor &editor)
{
...
wxSheetCellEditor wxSheet::GetDefaultEditorForCell(const wxSheetCoords &coords) const
{
  ...
  int index =
      GetSheetRefData()->m_typeRegistry->FindOrCloneDataType(GetTable()->GetTypeName(coords));
  if (index == wxNOT_FOUND) {
...
void wxSheet::SetGridCursorCell(const wxSheetCoords &coords)
{
  ...
  wxCHECK_RET((coords.GetRow() < GetNumberRows()) && (coords.GetCol() < GetNumberCols()),
...
bool wxSheet::DoMoveCursor(const wxSheetCoords &relCoords, bool expandSelection)
{
  ...
  if ((relCoords == wxSheetCoords(0, 0)) || !ContainsGridCell(cursorCoords) ||
      !ContainsGridCell(coords)) {
    return false;
  }
...
{
  if (selecting) {
    return (!GetSelectingBlock().IsEmpty() || (GetSelection() && GetSelection()->HasSelection()));
  }
...
bool wxSheet::SelectRow(int row, bool addToSelected, bool sendEvt)
{
  ...
  return SelectBlock(wxSheetBlock(row, 0, 1, GetNumberCols() + 1), addToSelected, sendEvt);
}
...
bool wxSheet::SelectCol(int col, bool addToSelected, bool sendEvt)
{
  ...
  return SelectBlock(wxSheetBlock(0, col, GetNumberRows() + 1, 1), addToSelected, sendEvt);
}
...
bool wxSheet::SelectAll(bool sendEvt)
{
  ...
  return SelectBlock(wxSheetBlock(0, 0, GetNumberRows(), GetNumberCols()), false, sendEvt);
}
...
bool wxSheet::ClearSelection(bool send_event)
{
  ...
  if (send_event) {
    ...
    SendRangeEvent(wxEVT_SHEET_RANGE_SELECTED,
                   wxSheetBlock(0, 0, GetNumberRows(), GetNumberCols()), false, false);
  }
...
bool wxSheetDataObject::SetData(size_t len, const void *buf)
{
  ...
  wxString strBuf((const wchar_t *)wxConvertMB2WX((const char *)buf),
                  len); // probably not Unicode safe
#else
...
bool wxSheet::PasteFromClipboard(const wxSheetCoords &topLeft, const wxChar &colSep)
{
  if (!wxTheClipboard->Open()) {
    return false;
  }
...
bool wxSheet::PasteInternalCopiedSelection(const wxSheetCoords &topLeft_)
{
  ...
  wxSheetCoords copiedTopLeft(copiedData.GetItemKey(0), copiedData.GetItemValue(0).GetItemKey(0));
...
{
  return GetEditControl().Ok() && GetEditControl().IsCreated() && GetEditControl().IsShown();
}
...
bool wxSheet::ShowCellEditControl()
{
  ...
  if (!GetEditControl().IsCreated()) {
    GetSheetRefData()->m_cellEditor.CreateEditor(
        win, -1, new wxSheetCellEditorEvtHandler(this, GetEditControl()), this);

    wxSheetEditorCreatedEvent evt(GetId(), wxEVT_SHEET_EDITOR_CREATED, this,
                                  GetEditControlCoords(),
  ...
  if (!value.IsEmpty() && attr.GetOverflow() && (GetEditControlCoords().m_col < GetNumberCols())) {
...
{
  wxCHECK_MSG(IsCellEditControlShown(), false,
              wxT("Edit control not shown in wxSheet::HideCellEditControl"));
...
{
  wxCHECK_RET(IsCellEditControlCreated(),
              wxT("Edit control not shown in wxSheet::HideCellEditControl"));
...
void wxSheet::RefreshRow(int row) { RefreshBlock(wxSheetBlock(row, 0, 1, GetNumberCols())); }
void wxSheet::RefreshCol(int col) { RefreshBlock(wxSheetBlock(0, col, GetNumberRows(), 1)); }
void wxSheet::RefreshGridCellBlock(const wxSheetBlock &block) { RefreshBlock(block); }
...
void wxSheet::DrawSplitterButton(wxDC &dc, const wxRect &rect)
{
  ...
  wxPen highlight(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNHIGHLIGHT), 1, wxPENSTYLE_SOLID);
  wxPen shadow(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNSHADOW), 1, wxPENSTYLE_SOLID);
  wxPen darkShadow(wxSystemSettings::GetColour(wxSYS_COLOUR_3DDKSHADOW), 1, wxPENSTYLE_SOLID);
...
void wxSheet::DrawGridCells(wxDC &dc, const wxSheetSelection &blockSel)
{
  ...
  while (revIter.GetNext(coords) != wxSHEET_SELECTIONITER_GET_END) {
    ...
    // HasCellValue(coords), HasCellValue(lastCoords)); fflush(stdout);
    DrawCell(dc, coords);
    lastCoords = coords;
  }
...
void wxSheet::DrawGridCells( wxDC& dc, const wxSheetSelection& blockSel_ )
{
    ...
    while (selIter.GetNext(coords))
    {
        ...
        if ((bottom > check_overflow_row) && !HasValue()) //GetTable() &&
!GetTable()->HasValue(coords))
        {
            ...
            for ( c.m_row = coords.m_row; c.m_row <= bottom; c.m_row++ )
            {
                ...
                while (c.m_col > 0)
                {
                    ...
                    if (GetTable()->HasValue(c))
                    {
                        ...
                        {
                            // check if this cell actually does overflow into coords
                            int width = attr.GetRenderer(this, c).GetBestSize(*this, attr, dc,
c).GetWidth();
...
void wxSheet::DrawGridSpace(wxDC &dc)
{
  ...
  if ((right > rightCol) || (bottom > bottomRow)) {
    ...
    dc.SetBrush(wxBrush(GetAttrBackgroundColour(wxSheetCoords(0, 0), wxSHEET_AttrDefault),
                        wxBRUSHSTYLE_SOLID));
    dc.SetPen(*wxTRANSPARENT_PEN);
...
void wxSheet::DrawCell(wxDC &dc, const wxSheetCoords &coords)
{
  ...
  if ((coords == GetEditControlCoords()) && IsCellEditControlShown()) {
    GetSheetRefData()->m_cellEditor.PaintBackground(*this, attr, dc, rect, coords,
                                                    IsCellSelected(coords));
  }
  else {
...
void wxSheet::DrawCursorCellHighlight(wxDC &dc, const wxSheetCellAttr &attr)
{
  ...
  int penWidth =
      attr.GetReadOnly() ? GetCursorCellHighlightROPenWidth() : GetCursorCellHighlightPenWidth();
  ...
  if (penWidth > 0) {
    ...
    dc.SetPen(
        wxPen(IsCellSelected(coords) ? GetSelectionForeground() : GetCursorCellHighlightColour(),
...
void wxSheet::DrawTextRectangle(wxDC &dc, const wxString &value, const wxRect &rect, int align,
                                int textOrientation)
{
  wxArrayString lines;
...
void wxSheet::DrawTextRectangle(wxDC &dc, const wxArrayString &lines, const wxRect &rect,
                                int align, int textOrientation)
{
...
bool wxSheet::GetTextBoxSize(wxDC &dc, const wxArrayString &lines, long *width, long *height) const
{
  long w = 0, h = 0;
  wxCoord lineW, lineH;
...
void wxSheet::OnScroll(wxScrollEvent &event)
{
  ...
  // m_horizScrollBar->GetThumbPosition(), m_horizScrollBar->GetThumbSize(),
  // pos*m_horizScrollBar->GetThumbSize(),
  // pos*m_horizScrollBar->GetThumbSize()+GetGridWindow()->GetClientSize().x);
...
void wxSheet::AdjustScrollbars(bool calc_win_sizes)
{
  ...
  */
  ...
  bool need_Xscroll = (m_scrollBarMode & SB_HORIZ_NEVER) != 0
                          ? false
                          : (((m_scrollBarMode & SB_HORIZ_ALWAYS) != 0) ? true : virtSize.x > gw);
  bool need_Yscroll = (m_scrollBarMode & SB_VERT_NEVER) != 0
                          ? false
  ...
  need_Xscroll = (m_scrollBarMode & SB_HORIZ_NEVER) != 0
                     ? false
                     : (((m_scrollBarMode & SB_HORIZ_ALWAYS) != 0) ? true : virtSize.x > gw);
  need_Yscroll = (m_scrollBarMode & SB_VERT_NEVER) != 0
                     ? false
                     : (((m_scrollBarMode & SB_VERT_ALWAYS) != 0) ? true : virtSize.y > gh);
  ...
  //     m_horizScrollBar->GetThumbPosition(), m_horizScrollBar->GetRange(),
  //     m_horizScrollBar->GetThumbSize(), m_horizScrollBar->GetPageSize());
...
void wxSheet::PrepareGridDC(wxDC &dc) { dc.SetDeviceOrigin(-m_gridOrigin.x, -m_gridOrigin.y); }

void wxSheet::PrepareRowLabelDC(wxDC &dc) { dc.SetDeviceOrigin(0, -m_gridOrigin.y); }

void wxSheet::PrepareColLabelDC(wxDC &dc) { dc.SetDeviceOrigin(-m_gridOrigin.x, 0); }
...
static int CheckAlignment(int align)
{
  ...
  int count = 0;
  if ((align & wxSHEET_AttrAlignRight) != 0) {
    count++;
  }
  if ((align & wxSHEET_AttrAlignCenterHoriz) != 0) {
    count++;
  }
  if ((align & wxSHEET_AttrAlignHorizUnset) != 0) {
    count++;
  }
  if (count > 1) {
  ...
  count = 0;
  if ((align & wxSHEET_AttrAlignBottom) != 0) {
    count++;
  }
  if ((align & wxSHEET_AttrAlignCenterVert) != 0) {
    count++;
  }
  if ((align & wxSHEET_AttrAlignVertUnset) != 0) {
    count++;
  }
  if (count > 1) {
  ...
  if (event.Dragging() && event.LeftIsDown()) {
    ...
    if (event.LeftIsDown()) {
      ...
      case WXSHEET_CURSOR_RESIZE_ROW: {
        y = wxMax(y, GetRowTop(m_dragRowOrCol) + GetMinimalRowHeight(m_dragRowOrCol));
  ...
  if (event.LeftDown()) {
    ...
    if (YToEdgeOfGridRow(y) < 0) {
      if (ContainsRowLabelCell(coords) &&
        ...
        if (event.ShiftDown()) {
          ...
          block =
              wxSheetBlock(GetSelectingAnchor(), wxSheetCoords(coords.m_row, GetNumberCols() + 1));
        }
        else {
        ...
        if (!add && HasSelection() &&
            (SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, block, false, false, &event) !=
             EVT_VETOED)) {
          ClearSelection(true);
        }
    ...
    if (HasMouseCursorMode(WXSHEET_CURSOR_RESIZE_ROW)) {
      ...
      if (!GetSelectingBlock().IsEmpty()) {
        ...
        if (SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, GetSelectingBlock(), true, add, &event) !=
            EVT_VETOED) {
  ...
  if (event.Dragging() && event.LeftIsDown()) {
    ...
    if (event.LeftIsDown()) {
      ...
      case WXSHEET_CURSOR_RESIZE_COL: {
        x = wxMax(x, GetColLeft(m_dragRowOrCol) + GetMinimalColWidth(m_dragRowOrCol));
    ...
    if (XToEdgeOfGridCol(x) < 0) {
      if (ContainsColLabelCell(coords) &&
        ...
        if (event.ShiftDown()) {
          ...
          block =
              wxSheetBlock(GetSelectingAnchor(), wxSheetCoords(GetNumberRows() + 1, coords.m_col));
        }
        else {
  ...
  if (event.LeftDown()) {
    ...
    if (SendEvent(wxEVT_SHEET_LABEL_LEFT_DOWN, coords, &event) == EVT_SKIPPED) {
      ...
      if (!can_edit && !HasSelectionMode(wxSHEET_SelectNone) &&
          (SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, block, true, false, &event) !=
           EVT_VETOED)) {
        SelectAll(true);
      }
    }
    ...
    if (!GetSelectingBlock().IsEmpty()) {
      if (SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, GetSelectingBlock(), true, false, &event) !=
          EVT_VETOED) {
        SelectAll(true);
        SetSelectingBlock(wxNullSheetBlock);
      }
  ...
  if (event.LeftDown() && ContainsGridCell(coords)) {
    if (SendEvent(wxEVT_SHEET_CELL_LEFT_DOWN, coords, &event) == EVT_SKIPPED) {
      ...
      if (event.ShiftDown()) {
        ...
        if (!HasSelectionMode(wxSHEET_SelectNone) &&
            SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, block, true, true, &event) != EVT_VETOED) {
        ...
        if (event.ControlDown()) {
          ...
          if (!HasSelectionMode(wxSHEET_SelectNone) &&
              SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, block, !sel, !sel, &event) !=
                  EVT_VETOED) {
    ...
    if (HasMouseCursorMode(WXSHEET_CURSOR_SELECT_CELL)) {
      if ((coords == GetGridCursorCell()) && m_waitForSlowClick &&
        ...
        if (!HasSelectionMode(wxSHEET_SelectNone) &&
            SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, GetSelectingBlock(), true, true, &event) !=
                EVT_VETOED) {
  ...
  if (GetNumberRows() && GetNumberCols()) {
    ...
    case WXK_SPACE: {
      if (event.ShiftDown() && ContainsGridCell(GetGridCursorCell())) {
        ...
        if (!GetSelectingBlock().IsEmpty() && GetSelectingBlock().Contains(GetGridCursorCell())) {
      ...
      if (event.AltDown() && ContainsGridCell(GetGridCursorCell())) {
        ...
        if (!HasSelectionMode(wxSHEET_SelectNone) &&
            SendRangeEvent(wxEVT_SHEET_RANGE_SELECTING, block, !sel, !sel, &event) != EVT_VETOED) {
    ...
    default: {
      ...
      if (!IsCellEditControlCreated() && CanEnableCellControl(GetGridCursorCell())) {
        ...
        wxSheetCellEditor editor(
            GetAttr(GetGridCursorCell()).GetEditor(this, GetGridCursorCell()));
...
wxWindow *wxSheet::GetWindowForCoords(const wxSheetCoords &coords) const
{
  ...
  wxFAIL_MSG(wxString::Format(wxT("Unable to get window for coords (%d,%d)"), coords.m_row,
                              coords.m_col));
  return NULL;
}
...
int wxSheet::SendEvent(const wxEventType type, const wxSheetCoords &coords, wxEvent *mouseOrKeyEvt)
{
...
int wxSheet::SendRangeEvent(const wxEventType type, const wxSheetBlock &block, bool selecting,
                            bool add, wxEvent *mouseOrKeyEvt)
{
...
void wxSheet::CalcWindowSizes(bool adjustScrollBars)
{
  ...
  if (horiz_sb) {
    ...
    if (horiz_splitter) {
      ...
      m_horizSplitRect =
          wxRect(rect.GetRight(), rect.GetTop(), SPLIT_BUTTON_WIDTH, rect.GetHeight());
    }
    else {
...
wxSize wxSheet::DoGetBestSize() const
{
  ...
  if (size.x > displaySize.x / 2) {
    size.x = displaySize.x / 2;
  }
  if (size.y > displaySize.y / 2) {
...
wxSheetEvent::wxSheetEvent(int id, wxEventType type, wxObject *obj, const wxSheetCoords &coords,
                           const wxPoint &pos, bool sel)
  : wxNotifyEvent(type, id), m_coords(coords), m_pos(pos), m_selecting(sel), m_control(false),
    m_shift(false), m_alt(false), m_meta(false)
{
  SetEventObject(obj);
}
...
wxSheetRangeSelectEvent::wxSheetRangeSelectEvent(int id, wxEventType type, wxObject *obj,
                                                 const wxSheetBlock &block, bool sel, bool add)
  : wxSheetEvent(id, type, obj, wxNullSheetCoords, wxPoint(-1, -1), sel), m_block(block),
    m_add(add)
{
}
...
wxSheetEditorCreatedEvent::wxSheetEditorCreatedEvent(int id, wxEventType type, wxObject *obj,
                                                     const wxSheetCoords &coords, wxWindow *ctrl)
...
wxString Joint(const wxArrayString &a)
{
  ...
  for (size_t n = 1u; n < a.GetCount(); n++) {
    s += wxT("'") + a[n] + wxT("'\t ");
  }
  return s;
}
...
#define CSVT1(str, a1)                                                                            \
  {                                                                                               \
    wxArrayString ar;                                                                             \
    ar.Add(wxT(a1));                                                                              \
    CSV_TEST(wxT(str), ar, ParseLine(wxT(str)));                                                  \
  }
#define CSVT2(str, a1, a2)                                                                        \
  {                                                                                               \
    wxArrayString ar;                                                                             \
    ar.Add(wxT(a1));                                                                              \
    ar.Add(wxT(a2));                                                                              \
