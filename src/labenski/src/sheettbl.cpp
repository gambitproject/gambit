    ...
    if (no_error && (int(pos) - num > count)) {
      ...
      wxCHECK_MSG(int(pos) - num <= count, false,
                  wxStrF(wxT("Called wxArrayStringUpdatePos(pos=%d, N=%d)\nPos value is invalid "
                             "for present array with %d elements"),
                         int(pos), num, count));
    }
...
wxString wxSheetValueProviderString::GetValue(const wxSheetCoords &coords_) const
{
  ...
  // wxPrintf(wxT("RC %d %d - NumRC %d %d DataRC %d %d '%s'\n"), coords_.m_row, coords_.m_col,
  // m_numRows, m_numCols,
  //     m_data.GetCount(), int(m_data.GetCount()) > coords.m_col ? m_data[coords.m_col].GetCount()
  //     : 0, wxDateTime::Now().FormatISOTime().c_str());
...
wxSheetValueProviderSparseString::wxSheetValueProviderSparseString(size_t numRows, size_t numCols,
                                                                   int options)
  : wxSheetValueProviderBase(0, 0, options)
{
  m_numRows += numRows;
...
{
  wxSHEET_CHECKUPDATE_MSG(col, numCols, m_numRows, false) m_numCols += numCols;
  return HasOption(wxSHEET_ValueProviderColPref) ? DoUpdateCols(col, numCols)
                                                 : DoUpdateRows(col, numCols);
}
...
wxSheetValueProviderHashString::wxSheetValueProviderHashString(size_t numRows, size_t numCols,
                                                               int options)
  : wxSheetValueProviderBase(0, 0, options)
{
  m_numRows += numRows;
...
void wxSheetValueProviderHashString::SetValue(const wxSheetCoords &coords_, const wxString &value)
{
  wxCHECK_RET(ContainsCell(coords_), wxT("Invalid coords"));
  wxSheetCoords coords(HasOption(wxSHEET_ValueProviderColPref) ? coords_ : coords_.GetSwapped());
...
{
  wxSHEET_CHECKUPDATE_MSG(row, numRows, m_numRows, false) bool append = (int(row) == m_numRows);
  m_numRows += numRows;
...
{
  wxSHEET_CHECKUPDATE_MSG(col, numCols, m_numRows, false) bool append = (int(col) == m_numRows);
  m_numCols += numCols;
  if (!m_numCols || !m_numRows) {
    ClearValues();
    return true;
...
wxSheetTable::wxSheetTable(wxSheet *view)
  : wxObject(), m_view(view), m_attrProvider(NULL), m_own_attr_provider(true),
    m_gridCellValues(NULL), m_own_grid_cell_values(true), m_rowLabelValues(NULL),
    m_own_row_label_values(true), m_colLabelValues(NULL), m_own_col_label_values(true),
...
wxString wxSheetTable::GetValue(const wxSheetCoords &coords)
{
  ...
  case wxSHEET_CELL_GRID: {
    ...
    wxCHECK_MSG(GetGridCellValueProvider(), wxEmptyString,
                wxT("Invalid grid cell data for the table"));
...
wxString wxSheetTable::GetDefaultColLabelValue(int col) const
{
  ...
  */
  ...
  const size_t chars =
      (col > 0) ? size_t(log((double)col) / 3.2580965380) : 0; // log_26(x) = log(x)/log(26.0)
...
long wxSheetTable::GetValueAsLong(const wxSheetCoords &coords)
{
  ...
  long lval = 0;
  val.ToLong(&lval);
  // wxCHECK_MSG(val.ToLong(&lval), 0, wxT("Unable to get cell ") + CS(coords) + wxT(" value as
  // long."));
  return lval;
}
double wxSheetTable::GetValueAsDouble(const wxSheetCoords &coords)
{
  ...
  double dval = 0.0;
  val.ToDouble(&dval);
  // wxCHECK_MSG(val.ToDouble(&dval), 0.0, wxT("Unable to get cell ") + CS(coords) + wxT(" value as
  // double."));
  return dval;
}
...
{
  const wxString val(GetValue(coords));
  if (val.IsEmpty() || (val == wxT("0")) || (val.CmpNoCase(wxT("f")) == 0) ||
      (val.CmpNoCase(wxT("false")) == 0)) {
    return false;
  }
...
void *wxSheetTable::GetValueAsCustom(const wxSheetCoords &coords, const wxString &typeName)
{
  wxFAIL_MSG(wxStrF(
      wxT("Unable to return wxSheetTable::GetValueAsCustom for cell (%d, %d) and type '%s'"),
      coords.m_row, coords.m_col, typeName.c_str()));

  return NULL;
}
void wxSheetTable::SetValueAsCustom(const wxSheetCoords &coords, const wxString &typeName,
                                    void *WXUNUSED(value))
{
  wxFAIL_MSG(
      wxStrF(wxT("Unable to set wxSheetTable::SetValueAsCustom for cell (%d, %d) and type '%s'"),
             coords.m_row, coords.m_col, typeName.c_str()));
}

bool wxSheetTable::CanGetValueAs(const wxSheetCoords &coords, const wxString &typeName)
{
...
void wxSheetTable::SetGridCellValueProvider(wxSheetValueProviderBase *gridCellValues,
                                            bool is_owner)
{
...
void wxSheetTable::SetRowLabelValueProvider(wxSheetValueProviderBase *rowLabelValues,
                                            bool is_owner)
{
...
void wxSheetTable::SetColLabelValueProvider(wxSheetValueProviderBase *colLabelValues,
                                            bool is_owner)
{
...
wxSheetCellAttr wxSheetTable::GetAttr(const wxSheetCoords &coords, wxSheetAttr_Type kind)
{
...
void wxSheetTable::SetAttr(const wxSheetCoords &coords, const wxSheetCellAttr &attr,
...
void wxSheetTable::SetCellSpan(const wxSheetBlock &block)
{
  ...
  wxCHECK_RET((ContainsGridCell(block.GetLeftTop()) && ContainsGridCell(block.GetRightBottom())) ||
                  (ContainsRowLabelCell(block.GetLeftTop()) &&
                   ContainsRowLabelCell(block.GetRightBottom())) ||
                  (ContainsColLabelCell(block.GetLeftTop()) &&
                   ContainsColLabelCell(block.GetRightBottom())),
  ...
  if (GetView()) {
    GetView()->RefreshGridCellBlock(bounds);
  }
}
...
bool wxSheetTable::UpdateSheetRowsCols(int update)
{
  ...
  if (tableRows != sheetRows) {
    done |= UpdateSheetRows(tableRows < sheetRows ? tableRows : sheetRows, tableRows - sheetRows,
                            update);
  }
  if (tableCols != sheetCols) {
    done |= UpdateSheetCols(tableCols < sheetCols ? tableCols : sheetCols, tableCols - sheetCols,
                            update);
  }
