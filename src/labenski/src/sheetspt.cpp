...
bool wxSheetSplitter::Create(wxWindow *parent, wxWindowID id, const wxPoint &pos,
...
wxSheetSplitter::~wxSheetSplitter()
{
  ...
  if (m_tlSheet) {
    m_tlSheet->GetSheetRefData()->RemoveSheet(m_tlSheet);
  }
  if (m_trSheet) {
    m_trSheet->GetSheetRefData()->RemoveSheet(m_trSheet);
  }
  if (m_blSheet) {
    m_blSheet->GetSheetRefData()->RemoveSheet(m_blSheet);
  }
  if (m_brSheet) {
    m_brSheet->GetSheetRefData()->RemoveSheet(m_brSheet);
  }
}
...
{
  wxCHECK_MSG(m_tlSheet, NULL,
              wxT("Unable to create wxSheet child in splitter, splitter not initialized yet."));
...
void wxSheetSplitter::SplitVertically(int y_pos, bool sendEvt)
{
  ...
  wxCHECK_RET((y_pos >= m_minSize.y) || (y_pos < GetClientSize().GetHeight() - m_minSize.y),
...
void wxSheetSplitter::SplitHorizontally(int x_pos, bool sendEvt)
{
  ...
  wxCHECK_RET((x_pos >= m_minSize.x) || (x_pos < GetClientSize().GetWidth() - m_minSize.x),
...
void wxSheetSplitter::LayoutWindows()
{
  ...
  */
  ...
  if (m_brSheet) {
    rect = wxRect(x + sash_size, y + sash_size, clientSize.x - x - sash_size - border_size,
...
void wxSheetSplitter::OnMouse(wxMouseEvent &event)
{
  ...
  if (event.LeftDown()) {
    ...
    if (SendEvent(wxEVT_SHEET_SPLIT_DOUBLECLICKED, m_splitMode == wxSHEET_SPLIT_VERTICAL)) {
...
void wxSheetSplitter::SetMouseCursor(int sheet_split_mode)
{
  ...
  case wxSHEET_SPLIT_VERTICAL:
    SetCursor(wxCURSOR_SIZENS);
    break;
  case wxSHEET_SPLIT_HORIZONTAL:
