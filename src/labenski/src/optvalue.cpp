...
wxObjectRefData *wxOptionValue::CreateRefData() const { return new wxOptionValueRefData; }
...
bool wxOptionValue::Ok() const { return M_OPTVALUDATA != NULL; }

void wxOptionValue::Destroy() { UnRef(); }
...
{
  wxCHECK_MSG(Ok() && (n < M_OPTVALUDATA->m_optionNames.GetCount()), wxEmptyString,
              wxT("Invalid wxOptionValue"));
  return M_OPTVALUDATA->m_optionNames[n];
}

wxString wxOptionValue::GetOptionValue(size_t n) const
{
  wxCHECK_MSG(Ok() && (n < M_OPTVALUDATA->m_optionValues.GetCount()), wxEmptyString,
              wxT("Invalid wxOptionValue"));
...
int wxOptionValue::GetOptionInt(const wxString &name) const { return wxAtoi(GetOption(name)); }
...
int wxOptionValue::GetOption(const wxString &name, wxArrayInt &values, int count,
