...
#define CHECK_INDEX_COUNT_MSG(index, count, max_count, ret)                                       \
  wxCHECK_MSG((int(index) >= 0) && (int(index) + int(count) <= int(max_count)), ret,              \
              wxT("invalid index or count"))
#define CHECK_INDEX_COUNT_RET(index, count, max_count)                                            \
  wxCHECK_RET((int(index) >= 0) && (int(index) + int(count) <= int(max_count)),                   \
              wxT("invalid index or count"))

#define CHECK_START_END_INDEX_MSG(start_index, end_index, max_count, ret)                         \
  wxCHECK_MSG((int(start_index) >= 0) && (int(start_index) < int(max_count)) &&                   \
                  (int(end_index) > int(start_index)) && (int(end_index) < int(max_count)),       \
              ret, wxT("Invalid data index"))
#define CHECK_START_END_INDEX_RET(start_index, end_index, max_count)                              \
  wxCHECK_RET((int(start_index) >= 0) && (int(start_index) < int(max_count)) &&                   \
                  (int(end_index) > int(start_index)) && (int(end_index) < int(max_count)),       \
...
public:
  ...
  wxBitmap m_normalSymbol, m_activeSymbol, m_selectedSymbol;
};
...
wxPlotDataRefData::wxPlotDataRefData(const wxPlotDataRefData &data) : wxPlotCurveRefData()
{
  CopyData(data);
  CopyExtra(data);
}

wxPlotDataRefData::~wxPlotDataRefData() { Destroy(); }
...
wxObjectRefData *wxPlotData::CreateRefData() const { return new wxPlotDataRefData; }
...
bool wxPlotData::Ok() const { return m_refData && (M_PLOTDATA->m_count > 0); }

void wxPlotData::Destroy() { UnRef(); }
...
bool wxPlotData::Create(const wxPlotFunction &plotFunc, double x_start, double dx, int points)
{
  ...
  if (!Create(points, false)) {
    return false; // this checks points
  }
...
wxPlotData wxPlotData::Remove(int index, int count) const
{
  ...
  if (count <= 0) {
    count = M_PLOTDATA->m_count - index;
  }
  wxCHECK_MSG((index >= 0) && (index + count <= M_PLOTDATA->m_count), newCurve,
              wxT("invalid index"));
...
wxPlotData wxPlotData::GetSubPlotData(int index, int count) const
{
  ...
  wxCHECK_MSG((index >= 0) && (index + count <= M_PLOTDATA->m_count), wxPlotData(),
              wxT("invalid index"));
...
void wxPlotData::CalcBoundingRect()
{
  ...
  double *x_data = M_PLOTDATA->m_Xdata, *y_data = M_PLOTDATA->m_Ydata;

  double x = *x_data, y = *y_data, xmin = x, xmax = x, ymin = y, ymax = y, xlast = x;
        ...
        while (select_cols && !stop_load) {
          wxString colStr = wxGetTextFromUser(
              wxString::Format(wxT("%d columns found.\n") wxT(
                                   ...
                                   wxT("If x = y then x values are 0,1,2... and y is that col."),
                               n),
          ...
          if (colStr.IsEmpty()) {
            ...
            long xcol = 0, ycol = 0;
            if ((!leftStr.IsEmpty()) && leftStr.ToLong(&xcol) && rightStr.ToLong(&ycol)) {
              xcol--;
              ycol--;
...
bool wxPlotData::SaveFile(const wxString &filename, bool save_header, const wxString &format)
{
  ...
  wxCHECK_MSG((!format.IsEmpty()) && (format.Find(wxT('%')) != wxNOT_FOUND), false,
              wxT("invalid format"));
...
wxEOL wxPlotData::GetEOLMode() const
{
  ...
  int eol = HasOption(wxPLOTCURVE_OPTION_EOLMODE) ? GetOptionInt(wxPLOTCURVE_OPTION_EOLMODE)
                                                  : wxEOL_NATIVE;
  switch (eol) {
  case wxEOL_NATIVE:
    return wxEOL_NATIVE;
...
{
  wxCHECK_MSG(Ok() && (index < M_PLOTDATA->m_count), wxPoint2DDouble(0, 0),
              wxT("Invalid wxPlotData"));
...
double wxPlotData::GetY(double x)
{
  ...
  return LinearInterpolateY(M_PLOTDATA->m_Xdata[i], y0, M_PLOTDATA->m_Xdata[i1], y1, x);
}
...
void wxPlotData::SetXValues(int start_index, int count, double x)
{
  ...
  if (count < 0) {
    count = M_PLOTDATA->m_count - start_index;
  }
...
int wxPlotData::GetIndexFromX(double x, wxPlotData::Index_Type type) const
{
  ...
  if (type == index_ceil) {
    if ((lo < count - 1) && (x > x_data[lo])) {
      lo++;
    }
    return lo;
  }

  if ((lo > 0) && (fabs(x - x_data[lo - 1]) < fabs(x - x_data[lo]))) {
    lo--;
  }
  if ((lo < count - 1) && (fabs(x - x_data[lo + 1]) < fabs(x - x_data[lo]))) {
    lo++;
  }
...
int wxPlotData::GetIndexFromY(double y, wxPlotData::Index_Type type) const
{
  ...
  // out of bounds so just return the closest
  if ((y < M_PLOTDATA->m_boundingRect.GetLeft()) || (y > M_PLOTDATA->m_boundingRect.GetRight())) {
    return index;
  }
...
double wxPlotData::GetAverage(int start_index, int count) const
{
  ...
  int end_index = start_index + count - 1;
  wxCHECK_MSG((start_index < M_PLOTDATA->m_count) && (end_index < M_PLOTDATA->m_count), 0.0,
              wxT("invalid input"));
...
int wxPlotData::GetMinMaxAve(const wxRangeIntSelection &rangeSel, wxPoint2DDouble *minXY_,
                             wxPoint2DDouble *maxXY_, wxPoint2DDouble *ave_, int *x_min_index_,
                             int *x_max_index_, int *y_min_index_, int *y_max_index_) const
{
  ...
  if (minXY_) {
    *minXY_ = wxPoint2DDouble(x_min_x, y_min_y);
  }
  if (maxXY_) {
    *maxXY_ = wxPoint2DDouble(x_max_x, y_max_y);
  }
  if (x_min_index_) {
    *x_min_index_ = x_min_index;
  }
  if (x_max_index_) {
    *x_max_index_ = x_max_index;
  }
  if (y_min_index_) {
    *y_min_index_ = y_min_index;
  }
  if (y_max_index_) {
    *y_max_index_ = y_max_index;
  }
...
wxArrayInt wxPlotData::GetCrossing(double y_value) const
{
  ...
  for (i = 1; i < M_PLOTDATA->m_count; i++) {
    ...
    if (((last_y >= y_value) && (y <= y_value)) || ((last_y <= y_value) && (y >= y_value))) {
...
wxPlotData wxPlotData::Resample(const wxPlotData &source) const
{
  ...
  int src_index_min =
      source.GetIndexFromX(wxMax(GetBoundingRect().m_x, source.GetBoundingRect().m_x));
  int src_index_max = source.GetIndexFromX(
      wxMin(GetBoundingRect().GetRight(), source.GetBoundingRect().GetRight()));

  int index_min = GetIndexFromX(wxMax(GetBoundingRect().m_x, source.GetBoundingRect().m_x));
  // int index_max = GetIndexFromX(wxMin(GetBoundingRect().GetRight(),
  // source.GetBoundingRect().GetRight()));
  ...
  if (!dst.Create(src_index_max - src_index_min, false)) {
    return dst;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (s_x == x) {
      ...
      y = LinearInterpolateY(GetXValue(index1), GetYValue(index1), GetXValue(index2),
                             GetYValue(index2), x);
    }
    ...
    if ((*x_data > curve2_x_min) && (*x_data < curve2_x_max)) {
      ...
      else // need to interpolate to find data's y value at this x
      {
        ...
        if ((index1 >= 0) && (index1 < curve2_count) && (index2 >= 0) && (index2 < curve2_count)) {
          *dst_y_data = factor1 * (*y_data) +
                        factor2 * LinearInterpolateY(
                                      curve2.GetXValue(index1), curve2.GetYValue(index1),
                                      curve2.GetXValue(index2), curve2.GetYValue(index2), *x_data);
        }
        else {
          *dst_y_data = *x_data;
        }
...
wxPlotData wxPlotData::RunAverage(int points, int start_index, int count) const
{
  ...
  for (i = run_start + 1; i < run_end; i++) {
    run_sum += GetYValue(i + half_width) -
               GetYValue(i - half_width - 1); // src[i+half_width] - src[i-half_width-1];
...
wxPlotData wxPlotData::LinearizeY(int start_index, int count) const
{
  ...
  wxCHECK_MSG(wxFinite(x0) && wxFinite(y0) && wxFinite(x1) && wxFinite(y1), dst,
              wxT("values are NaN"));

  if (!dst.Copy(*this, true)) {
    return dst;
  }
...
wxPlotData wxPlotData::VarianceCurve(int points) const
{
  ...
  if (!dst.Create(data_count, false)) {
    return dst;
  }
  ...
  for (i = min; i < max; i++) {
    ...
    if ((x >= other_x_min) && (x <= other_x_max)) {
      ...
      if (other.GetXValue(index1) == x) {
        ...
        cc += (GetYValue(i) - smooth.GetYValue(i)) *
              (other.GetYValue(index1) - other_smooth.GetYValue(index1));
      }
      else // need to interpolate to find data's y value at this x
      {
        ...
        if ((index1 >= 0) && (index1 < other_count) && (index2 >= 0) && (index2 < other_count)) {
          double y = LinearInterpolateY(other.GetXValue(index1), other.GetYValue(index1),
                                        other.GetXValue(index2), other.GetYValue(index2), x);

          double ys = LinearInterpolateY(
              other_smooth.GetXValue(index1), other_smooth.GetYValue(index1),
              other_smooth.GetXValue(index2), other_smooth.GetYValue(index2), x);
...
wxPlotData wxPlotData::FFT(bool forward)
{
  ...
  if (fft_double(samples, (forward ? 0 : 1), source.GetYData(), source.GetYiData(),
...
wxPlotData wxPlotData::PowerSpectrum()
{
  ...
  for (i = 0; i < count; i++) {
    power.GetYData()[i] = pow(power.GetYData()[i] * power.GetYData()[i] +
                                  power.GetYiData()[i] * power.GetYiData()[i],
                              0.5) /
                          double(count);
  }
...
wxString wxPlotData::FFTHiPassFilterFormat(double hi, wxPlotData::FFTFilter_Type filter, double n)
{
  switch (filter) {
  case FilterStep:
    return wxString::Format(wxT("x>%lg"), hi);
  case FilterButterworth:
    return wxString::Format(wxT("1 - 1/(1+(x/%lg)^(2*%lg))"), hi, n);
  case FilterGaussian:
    return wxString::Format(wxT("exp(-(%lg^2)/(2*x*x))"), hi);
  case FilterFermi:
    return wxString::Format(wxT("1/(1+exp((%lg-x)/%lg))"), hi, n);
  default:
    break;
  }
...
wxString wxPlotData::FFTLoPassFilterFormat(double lo, wxPlotData::FFTFilter_Type filter, double n)
{
  switch (filter) {
  case FilterStep:
    return wxString::Format(wxT("x<%lg"), lo);
  case FilterButterworth:
    return wxString::Format(wxT("1/(1+(x/%lg)^(2*%lg))"), lo, n);
  case FilterGaussian:
    return wxString::Format(wxT("1 - exp(-(%lg^2)/(2*x*x))"), lo);
  case FilterFermi:
    return wxString::Format(wxT("1/(1+exp((%lg-x)/(-%lg)))"), lo, n);
  default:
    break;
  }
  return wxEmptyString;
...
wxString wxPlotData::FFTNotchFilterFormat(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                                          double n)
{
  switch (filter) {
  case FilterStep:
    return wxString::Format(wxT("(x<%lg)|(x>%lg)"), lo, hi);
  case FilterButterworth:
    return wxString::Format(wxT("1/(1+(x/%lg)^(2*%lg)) + (1-1/(1+(x/%lg)^(2*%lg)))"), lo, n, hi,
                            n);
  case FilterGaussian:
    return wxString::Format(wxT("2-(exp(-(%lg^2)/(2*x*x))+(1-exp(-(%lg^2)/(2*x*x))))"), lo, hi);
  case FilterFermi:
    return wxString::Format(wxT("1/(1+exp((%lg-x)/(-%lg))) + 1/(1+exp((%lg-x)/(%lg)))"), lo, n, hi,
                            n);
  default:
    break;
  }
...
wxString wxPlotData::FFTBandPassFilterFormat(double lo, double hi,
                                             wxPlotData::FFTFilter_Type filter, double n)
{
  switch (filter) {
  case FilterStep:
    return wxString::Format(wxT("(x>%lg)&(x<%lg)"), lo, hi);
  case FilterButterworth:
    return wxString::Format(wxT("1 - (1/(1+(x/%lg)^(2*%lg))+(1-1/(1+(x/%lg)^(2*%lg))))"), lo, n,
                            hi, n);
  case FilterGaussian:
    return wxString::Format(wxT("exp(-(%lg^2)/(2*x*x)) - exp(-(%lg^2)/(2*x*x))"), lo, hi);
  case FilterFermi:
    return wxString::Format(wxT("1 - 1/(1+exp((%lg-x)/(-%lg))) - 1/(1+exp((%lg-x)/(%lg)))"), lo, n,
                            hi, n);
  default:
    break;
  }
...
wxPlotData wxPlotData::FFTHiPassFilter(double fc, wxPlotData::FFTFilter_Type filter, double n)
{
  ...
  double f, x;
  double *ydata = xform.GetYData(), *yidata = xform.GetYiData();
...
wxPlotData wxPlotData::FFTNotchFilter(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                                      double n)
{
...
wxPlotData wxPlotData::FFTBandPassFilter(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                                         double n)
{
  wxPlotData xform;
  ...
  double *xform_xdata = xform.GetXData(), *xform_ydata = xform.GetYData(),
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((filter == FilterStep) && ((x <= lo) || (x >= hi))) {
      ...
      *xform_yidata = 0;
      ...
      if (filter == FilterButterworth) {
        f = 1.0 -
            (1.0 / (1.0 + pow(x / lo, 2.0 * n)) + (1.0 - 1.0 / (1.0 + pow(x / hi, 2.0 * n))));
      }
      else if (filter == FilterGaussian) {
...
void wxPlotData::SetSymbol(wxPlotSymbol_Type type, wxPlotPen_Type colour_type, int width,
                           int height, const wxPen *pen, const wxBrush *brush)
{
  wxCHECK_RET(Ok(), wxT("Invalid wxPlotData"));
...
wxBitmap wxPlotData::CreateSymbol(wxPlotSymbol_Type type, wxPlotPen_Type colour_type, int width,
                                  int height, const wxPen *pen, const wxBrush *brush)
{
...
public:
  ...
  void OnExit() { s_clipboardwxPlotData.Destroy(); }
};
...
wxPlotData wxPlotDataObject::GetPlotData() const { return s_clipboardwxPlotData; }
