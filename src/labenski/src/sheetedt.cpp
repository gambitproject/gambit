...
wxSheetCellEditor::wxSheetCellEditor(wxSheetCellEditorRefData *editor) { m_refData = editor; }
...
void wxSheetCellEditor::PaintBackground(wxSheet &sheet, const wxSheetCellAttr &attr, wxDC &dc,
...
bool wxSheetCellEditorRefData::IsShown() const { return GetControl() && GetControl()->IsShown(); }

void wxSheetCellEditorRefData::CreateEditor(wxWindow *WXUNUSED(parent), wxWindowID WXUNUSED(id),
                                            wxEvtHandler *evtHandler, wxSheet *WXUNUSED(sheet))
{
...
void wxSheetCellEditorRefData::PaintBackground(wxSheet &, const wxSheetCellAttr &attr, wxDC &dc,
                                               const wxRect &rect, const wxSheetCoords &, bool)
{
...
void wxSheetCellTextEditorRefData::HandleReturn(wxKeyEvent &WXUNUSED_GTK(WXUNUSED_MOTIF(event)))
{
  wxCHECK_RET(IsCreated(), wxT("The wxSheetCellEditor must be Created first!"));
...
void wxSheetCellTextEditorRefData::SetParameters(const wxString &params)
{
  if (params.IsEmpty()) {
    ...
    if (!params.ToLong(&tmp)) {
      wxLogDebug(_T("Invalid wxSheetCellTextEditor parameter string '%s' ignored"),
                 params.c_str());
    }
    else {
...
void wxSheetCellAutoWrapStringEditorRefData::CreateEditor(wxWindow *parent, wxWindowID id,
                                                          wxEvtHandler *evtHandler, wxSheet *sheet)
{
...
void wxSheetCellNumberEditorRefData::CreateEditor(wxWindow *parent, wxWindowID id,
                                                  wxEvtHandler *evtHandler, wxSheet *sheet)
{
  if (HasRange()) {
    // create a spin ctrl
    SetControl(new wxSpinCtrl(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
...
void wxSheetCellNumberEditorRefData::StartingKey(wxKeyEvent &event)
{
  if (!HasRange()) {
    int keycode = event.GetKeyCode();
    if (wxIsdigit(keycode) || keycode == '+' || keycode == '-' || keycode == WXK_NUMPAD0 ||
        keycode == WXK_NUMPAD1 || keycode == WXK_NUMPAD2 || keycode == WXK_NUMPAD3 ||
        keycode == WXK_NUMPAD4 || keycode == WXK_NUMPAD5 || keycode == WXK_NUMPAD6 ||
        keycode == WXK_NUMPAD7 || keycode == WXK_NUMPAD8 || keycode == WXK_NUMPAD9 ||
        keycode == WXK_ADD || keycode == WXK_NUMPAD_ADD || keycode == WXK_SUBTRACT ||
...
void wxSheetCellNumberEditorRefData::SetParameters(const wxString &params)
{
  if (params.IsEmpty()) {
    ...
    wxLogDebug(_T("Invalid wxSheetCellNumberEditor parameter string '%s' ignored"),
               params.c_str());
  }
}
...
void wxSheetCellFloatEditorRefData::CreateEditor(wxWindow *parent, wxWindowID id,
                                                 wxEvtHandler *evtHandler, wxSheet *sheet)
{
  wxSheetCellTextEditorRefData::CreateEditor(parent, id, evtHandler, sheet);
...
void wxSheetCellFloatEditorRefData::ResetValue() { DoResetValue(GetInitValue()); }
...
void wxSheetCellFloatEditorRefData::StartingKey(wxKeyEvent &event)
{
  ...
  if (wxIsdigit(keycode) || keycode == '+' || keycode == '-' || is_decimal_point ||
      keycode == WXK_NUMPAD0 || keycode == WXK_NUMPAD1 || keycode == WXK_NUMPAD2 ||
      keycode == WXK_NUMPAD3 || keycode == WXK_NUMPAD4 || keycode == WXK_NUMPAD5 ||
      keycode == WXK_NUMPAD6 || keycode == WXK_NUMPAD7 || keycode == WXK_NUMPAD8 ||
      keycode == WXK_NUMPAD9 || keycode == WXK_ADD || keycode == WXK_NUMPAD_ADD ||
      keycode == WXK_SUBTRACT || keycode == WXK_NUMPAD_SUBTRACT) {
    wxSheetCellTextEditorRefData::StartingKey(event);
...
bool wxSheetCellFloatEditorRefData::IsAcceptedKey(wxKeyEvent &event)
{
  if (wxSheetCellEditorRefData::IsAcceptedKey(event)) {
    ...
    default: {
      ...
      bool is_decimal_point =
          (strbuf == wxLocale::GetInfo(wxLOCALE_DECIMAL_POINT, wxLOCALE_CAT_NUMBER));
      if ((keycode < 128) && (wxIsdigit(keycode) || tolower(keycode) == 'e' || is_decimal_point ||
                              keycode == '+' || keycode == '-')) {
        return true;
      }
    }
...
void wxSheetCellBoolEditorRefData::CreateEditor(wxWindow *parent, wxWindowID id,
                                                wxEvtHandler *evtHandler, wxSheet *sheet)
{
  SetControl(
      new wxCheckBox(parent, id, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxNO_BORDER));
...
void wxSheetCellChoiceEditorRefData::CreateEditor(wxWindow *parent, wxWindowID id,
                                                  wxEvtHandler *evtHandler, wxSheet *sheet)
{
#if wxCHECK_VERSION(2, 5, 0)
  SetControl(new wxComboBox(parent, id, wxEmptyString, wxDefaultPosition, wxDefaultSize, m_choices,
  ...
  SetControl(new wxComboBox(parent, id, wxEmptyString, wxDefaultPosition, wxDefaultSize, count,
                            choices, m_allowOthers ? 0 : wxCB_READONLY));
