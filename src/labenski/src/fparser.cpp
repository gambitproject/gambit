    ...
    return z *
           (2.0 +
            y * (0.666666666663366 +
                 y * (0.400000001206045 +
                      y * (0.285714091590488 +
                           y * (0.22223823332791 + y * (0.1811136267967 + y * 0.16948212488))))));
  }
}
...
{
  if (fabs(x) > 1.0E10) {
    return ((x > 0.0) ? 0.69314718055995 + log(fabs(x)) : -0.69314718055995 + log(fabs(x)));
  }
  else {
    double y = x * x;
    return ((x == 0.0) ? 0.0
                       : ((x > 0.0) ? logoneplusx(fabs(x) + y / (1.0 + sqrt(1.0 + y)))
...
double acosh(double x)
{
  return ((x <= 1.0)
              ? 0.0
              : ((x > 1.0E10) ? 0.69314718055995 + log(x) : log(x + sqrt((x - 1.0) * (x + 1.0)))));
}
...
double atanh(double x)
{
  ...
  if (ax >= 1.0) {
    ...
    return ((x == 0) ? 0.0
                     : ((x > 0.0) ? 0.5 * logoneplusx(2.0 * ax / (1.0 - ax))
...
// ----------------------------------------------------------------------------
wxFunctionParser::wxFunctionParser() : m_ok(false) { m_functionParser = new FunctionParser; }
...
wxFunctionParser::~wxFunctionParser() { delete m_functionParser; }
...
double wxFunctionParser::Eval(const double *Vars) { return m_functionParser->Eval(Vars); }
int wxFunctionParser::EvalError() const { return m_functionParser->EvalError(); }
...
void wxFunctionParser::Optimize() { m_functionParser->Optimize(); }
int wxFunctionParser::GetNumberVariables() const { return m_functionParser->GetNumberVariables(); }
bool wxFunctionParser::GetUseDegrees() const { return m_functionParser->GetUseDegrees(); }
...
const FuncDefinition Functions[] = {
    ...
    {"atanh", 5, cAtanh, 1},
#endif
    {"ceil", 4, cCeil, 1},   {"cos", 3, cCos, 1},     {"cosh", 4, cCosh, 1}, {"cot", 3, cCot, 1},
    {"csc", 3, cCsc, 1},
#ifndef DISABLE_EVAL
    {"eval", 4, cEval, 0},
#endif
    {"exp", 3, cExp, 1},     {"floor", 5, cFloor, 1}, {"if", 2, cIf, 0},     {"int", 3, cInt, 1},
    {"log", 3, cLog, 1},     {"log10", 5, cLog10, 1}, {"max", 3, cMax, 2},   {"min", 3, cMin, 2},
    {"sec", 3, cSec, 1},     {"sin", 3, cSin, 1},     {"sinh", 4, cSinh, 1}, {"sqrt", 4, cSqrt, 1},
    {"tan", 3, cTan, 1},     {"tanh", 4, cTanh, 1}};
...
//---------------------------------------------------------------------------
//===========================================================================
FunctionParser::FunctionParser() : parseErrorType(FP_NO_ERROR), evalErrorType(0), data(new Data)
{
  data->referenceCounter = 1;
}
...
FunctionParser::FunctionParser(const FunctionParser &cpy)
  : parseErrorType(cpy.parseErrorType), evalErrorType(cpy.evalErrorType), data(cpy.data)
{
...
FunctionParser::Data::Data()
  : useDegreeConversion(false), ByteCode(0), ByteCodeSize(0), Immed(0), ImmedSize(0), Stack(0),
    StackSize(0)
{
}
...
FunctionParser::Data::Data(const Data &cpy)
  : varAmount(cpy.varAmount), useDegreeConversion(cpy.useDegreeConversion),
    Variables(cpy.Variables), Constants(cpy.Constants), FuncPtrNames(cpy.FuncPtrNames),
    FuncPtrs(cpy.FuncPtrs), FuncParserNames(cpy.FuncParserNames), FuncParsers(cpy.FuncParsers),
    ByteCode(0), ByteCodeSize(cpy.ByteCodeSize), Immed(0), ImmedSize(cpy.ImmedSize), Stack(0),
    StackSize(cpy.StackSize)
{
  ...
  for (unsigned j = 0; j < ImmedSize; ++j) {
    Immed[j] = cpy.Immed[j];
  }
...
inline bool IsOperator(int c) { return strchr("+-*/%^=<>&|", c) != NULL; }
...
int FunctionParser::CheckSyntax(const char *Function)
{
  ...
  while (true) {
    ...
    // Check for operator
    if (!IsOperator(c) &&
        (c != ',' || functionParenthDepth.empty() || functionParenthDepth.back() != ParenthCnt)) {
...
inline void FunctionParser::AddCompiledByte(unsigned c) { tempByteCode->push_back(c); }

inline void FunctionParser::AddImmediate(double i) { tempImmed->push_back(i); }
  ...
  if (isalpha(c) || c == '_') // Function, variable or constant
  {
    ...
    if (func) // is function
    {
      ...
      unsigned requiredParams =
          strcmp(func->name, "eval") == 0 ? data->Variables.size() : func->params;
#else
...
//===========================================================================
namespace {
inline int doubleToInt(double d) { return d < 0 ? -int((-d) + .5) : int(d + .5); }
...
inline double RadiansToDegrees(double radians) { return radians * (180.0 / M_PI); }
} // namespace
...
public:
  ...
  bool operator==(bool b) const { return (state != 0) != !b; }
  bool operator!=(bool b) const { return (state != 0) != b; }
...
public:
  ...
  void AddParam(const SubTree &p) { args.push_back(p); }
...
#define CHECKCONSTNEG(item, op) ((op) == cMul) ? (item).CheckConstInv() : (item).CheckConstNeg()
...
private:
  ...
  void ReplaceWith(unsigned op, const SubTree &p) { ReplaceWith(CodeTree(op, p)); }
    ...
    Redo:
      for (pit a = GetBegin(); a != GetEnd(); ++a) {
        ...
        if (poslist.size() >= min) {
          ...
          CodeTree tmp(GetOp() == cAdd ? cMul : cPow, arvo, factor);
    ...
    */
    ...
    if (GetOp() == cLog) {
      ...
      if (p->GetOp() == cPow) {
        ...
        for (pit b, a = p->GetBegin(); a != p->GetEnd(); a = b) {
          ...
          b = a;
          ++b;
          if (pa->GetOp() == cPow && pa->getp0()->IsImmed() &&
    ...
    if (GetOp() == cPow) {
      ...
      if (p0->IsImmed() && p0->GetImmed() == CONSTANT_E && p1->GetOp() == cLog) {
        ...
        if (p0->IsImmed() && p0->GetImmed() == 10.0 && cl.value == CONSTANT_L10I && foundposlog) {
        ...
        if (p0->IsImmed() && p0->GetImmed() == CONSTANT_E && foundposlog) {
          SubTree base = (*poslogpos)->getp0();
          p1->Erase(poslogpos);
        ...
        if (foundposlog && foundneglog && *((*neglogpos)->getp0()) == *p0) {
          SubTree base = (*poslogpos)->getp0();
          p1->Erase(poslogpos);
          p1->Erase(neglogpos);
  ...
  void OptimizePowMulAdd()
  {
    ...
    if (GetOp() == cPow) {
      ...
      if (exponent->IsImmed()) {
        ...
        else if (exponent->GetImmed() == 0.0 && base->NonZero()) {
...
CodeTree::ConstList CodeTree::BuildConstList()
{
  ...
  if (GetOp() == cMul) {
    /*
      Jos joku niista arvoista on -1 eika se ole ainoa arvo,
      niin joku muu niista arvoista negatoidaan.
    */
...
#define AddConst(v)                                                                               \
  ...
  case cMul: {
    ...
    for (pcit a = GetBegin(); a != GetEnd(); ++a) {
      ...
      bool done = false;
      if (pa->IsImmed()) {
        if (GetOp() == cMul && pa->data->IsInverted() && (pnega || opcount == 2)) {
          CodeTree tmp = *pa;
  ...
  for (unsigned phase = 0; phase <= 2; ++phase) {
    ...
    if (phase == 0 || phase == 2) {
      ...
      /* Optimization paths:
         ...
         linearcombine= conflict * addmulflat(1) redundant(1)
         powmuladd=
         exponents= linearcombine * powmuladd conflict(1)
         logarithm= exponents *
         functioncalls= IDLE
         linearexplode= IDLE
         pascal= IDLE
...
SubTreeDecl((const CodeTree &b), (b)) SubTreeDecl((double value), (value))
...
bool SubTree::operator==(const SubTree &b) const { return sign == b.sign && *tree == *b.tree; }
