...
// ----------------------------------------------------------------------------
wxSheetArrayEdge::wxSheetArrayEdge(size_t count, size_t default_size, size_t min_allowed_size)
  : m_count(count), m_default_size(default_size), m_min_allowed_size(min_allowed_size)
{
}
...
int wxSheetArrayEdge::GetMinSize(size_t index) const
{
  ...
  return (min_index != wxNOT_FOUND) ? m_minSizes.GetItemValue(min_index) : m_min_allowed_size;
}
