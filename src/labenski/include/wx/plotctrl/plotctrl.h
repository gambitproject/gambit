...
#define CURSOR_GRAB wxCURSOR_MAX // for a hand cursor with fingers closed

#include "wx/dynarray.h"
#ifndef WX_DECLARE_OBJARRAY_WITH_DECL // for wx2.4 backwards compatibility
#define WX_DECLARE_OBJARRAY_WITH_DECL(T, name, expmode)                                           \
  WX_DECLARE_USER_EXPORTED_OBJARRAY(T, name, WXDLLIMPEXP_PLOTCTRL)
#endif
...
public:
  ...
  wxPlotCtrl(wxWindow *parent, wxWindowID win_id = wxID_ANY,
             ...
             wxPlotAxis_Type flags = wxPLOT_DEFAULT, const wxString &name = wxT("wxPlotCtrl"))
  {
    Init();
  ...
  bool Create(wxWindow *parent, wxWindowID id = wxID_ANY, const wxPoint &pos = wxDefaultPosition,
              const wxSize &size = wxDefaultSize, wxPlotAxis_Type flags = wxPLOT_DEFAULT,
  ...
  //   this works for all plotcurve classes, for wxPlotData they're converted to the indexes
  //   however if there's nothing to select or already selected it returns false if curve_index ==
  //   -1 then try to select points in all curves
  ...
  bool SelectXRange(int curve_index, const wxRangeDouble &range, bool send_event = false)
  {
    return DoSelectRectangle(
        curve_index,
        wxRect2DDouble(range.m_min, -wxPlot_MAX_DBL, range.GetRange(), wxPlot_MAX_RANGE), true,
        send_event);
  }
  bool DeselectXRange(int curve_index, const wxRangeDouble &range, bool send_event = false)
  {
    return DoSelectRectangle(
        curve_index,
        wxRect2DDouble(range.m_min, -wxPlot_MAX_DBL, range.GetRange(), wxPlot_MAX_RANGE), false,
        send_event);
  }
  bool SelectYRange(int curve_index, const wxRangeDouble &range, bool send_event = false)
  {
    return DoSelectRectangle(
        curve_index,
        wxRect2DDouble(-wxPlot_MAX_DBL, range.m_min, wxPlot_MAX_RANGE, range.GetRange()), true,
        send_event);
  }
  bool DeselectYRange(int curve_index, const wxRangeDouble &range, bool send_event = false)
  {
    return DoSelectRectangle(
        curve_index,
        wxRect2DDouble(-wxPlot_MAX_DBL, range.m_min, wxPlot_MAX_RANGE, range.GetRange()), false,
        send_event);
  }
  ...
  // internal use, or not...
  virtual bool DoSelectRectangle(int curve_index, const wxRect2DDouble &rect, bool select,
                                 bool send_event = false);
  virtual bool DoSelectDataRange(int curve_index, const wxRangeInt &range, bool select,
                                 bool send_event = false);
  ...
  virtual bool SetZoom(double zoom_x, double zoom_y, double origin_x, double origin_y,
                       bool send_event = false);
  ...
  void SetShowYAxisLabel(bool show)
  {
    m_show_ylabel = show;
    DoSize();
  }
  ...
  bool FindCurve(const wxPoint2DDouble &pt, const wxPoint2DDouble &dxdyPt, int &curve_index,
                 int &data_index, wxPoint2DDouble *curvePt = NULL) const;
  ...
  {
    double x = m_zoom.m_x * (plotx - m_viewRect.GetLeft()) + 0.5;
    return x < INT_MAX ? int(x) : INT_MAX;
  }
  inline int GetClientCoordFromPlotY(double ploty) const
  {
    double y = m_areaClientRect.height - m_zoom.m_y * (ploty - m_viewRect.GetTop()) + 0.5;
  ...
  inline wxRect GetClientRectFromPlotRect(const wxRect2DDouble &plotRect) const
  {
    ...
    return wxRect(GetClientCoordFromPlotX(plotRect.m_x),
                  GetClientCoordFromPlotY(plotRect.GetBottom()), w < INT_MAX ? int(w) : INT_MAX,
...
protected:
  ...
  wxStockCursor m_area_mouse_cursorid;

  void SetPlotWinMouseCursor(wxStockCursor cursorid);
  wxStockCursor m_mouse_cursorid;
...
public:
  wxPlotEvent(wxEventType commandType = wxEVT_NULL, int id = wxID_ANY, wxPlotCtrl *window = NULL);

  wxPlotEvent(const wxPlotEvent &event)
    : wxNotifyEvent(event), m_curve(event.m_curve), m_curveIndex(event.m_curveIndex),
...
public:
  ...
  wxPlotSelectionEvent(const wxPlotSelectionEvent &event)
    : wxPlotEvent(event), m_dataRange(event.m_dataRange), m_curveRange(event.m_curveRange),
      m_dataSelection(event.m_dataSelection), m_selecting(event.m_selecting)
  {
  }
...
// wxPlotEvent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_ADD_CURVE,
                            0) // a curve has been added
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_DELETING_CURVE,
                            0) // a curve is about to be deleted, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_DELETED_CURVE,
                            0) // a curve has been deleted

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_CURVE_SEL_CHANGING,
                            0) // curve selection changing, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_CURVE_SEL_CHANGED,
                            0) // curve selection has changed

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_MOUSE_MOTION, 0) // mouse moved
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_CLICKED,
                            0) // mouse left or right clicked
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_DOUBLECLICKED,
                            0) // mouse left or right doubleclicked
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_POINT_CLICKED,
                            0) // clicked on a plot point (+-2pixels)
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_POINT_DOUBLECLICKED,
                            0) // dclicked on a plot point (+-2pixels)

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_AREA_SEL_CREATING,
                            0) // mouse left down and drag begin
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_AREA_SEL_CHANGING,
                            0) // mouse left down and dragging
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_AREA_SEL_CREATED,
                            0) // mouse left down and drag end

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_VIEW_CHANGING,
                            0) // zoom or origin of plotctrl is about to change
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_VIEW_CHANGED,
                            0) // zoom or origin of plotctrl has changed

DECLARE_EXPORTED_EVENT_TYPE(
    WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_CURSOR_CHANGING,
    0) // cursor point/curve is about to change, event.Skip(false) to prevent
       // if the cursor is invalidated since
       // the curve is gone you cannot prevent it
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_CURSOR_CHANGED,
                            0) // cursor point/curve changed

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_ERROR,
                            0) // an error has occured, see event.GetString()
                               // usually nonfatal NaN overflow errors

DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_BEGIN_TITLE_EDIT,
                            0) // title is about to be edited, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_END_TITLE_EDIT,
                            0) // title has been edited and changed, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_BEGIN_X_LABEL_EDIT,
                            0) // x label is about to be edited, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_END_X_LABEL_EDIT,
                            0) // x label has been edited and changed, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_BEGIN_Y_LABEL_EDIT,
                            0) // y label is about to be edited, event.Skip(false) to prevent
DECLARE_EXPORTED_EVENT_TYPE(WXDLLIMPEXP_PLOTCTRL, wxEVT_PLOT_END_Y_LABEL_EDIT,
                            0) // y label has been edited and changed, event.Skip(false) to prevent
...
#define EVT_PLOT_CURVE_SEL_CHANGING(id, fn)                                                       \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_CURVE_SEL_CHANGING, id, fn)
#define EVT_PLOT_CURVE_SEL_CHANGED(id, fn)                                                        \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_CURVE_SEL_CHANGED, id, fn)
...
#define EVT_PLOT_POINT_DOUBLECLICKED(id, fn)                                                      \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_POINT_DOUBLECLICKED, id, fn)

#define EVT_PLOT_AREA_SEL_CREATING(id, fn)                                                        \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_AREA_SEL_CREATING, id, fn)
#define EVT_PLOT_AREA_SEL_CHANGING(id, fn)                                                        \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_AREA_SEL_CHANGING, id, fn)
...
#define EVT_PLOT_BEGIN_X_LABEL_EDIT(id, fn)                                                       \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_BEGIN_X_LABEL_EDIT, id, fn)
#define EVT_PLOT_END_X_LABEL_EDIT(id, fn) wx__DECLARE_PLOTEVT(wxEVT_PLOT_END_X_LABEL_EDIT, id, fn)
#define EVT_PLOT_BEGIN_Y_LABEL_EDIT(id, fn)                                                       \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_BEGIN_Y_LABEL_EDIT, id, fn)
#define EVT_PLOT_END_Y_LABEL_EDIT(id, fn) wx__DECLARE_PLOTEVT(wxEVT_PLOT_END_Y_LABEL_EDIT, id, fn)

#define EVT_PLOT_MOUSE_FUNC_CHANGING(id, fn)                                                      \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_MOUSE_FUNC_CHANGING, id, fn)
#define EVT_PLOT_MOUSE_FUNC_CHANGED(id, fn)                                                       \
  wx__DECLARE_PLOTEVT(wxEVT_PLOT_MOUSE_FUNC_CHANGED, id, fn)
...
#define wx__DECLARE_PLOTSELECTIONEVT(evt, id, fn)                                                 \
  wx__DECLARE_EVT1(evt, id, wxPlotSelectionEventHandler(fn))

// #define EVT_PLOT_RANGE_SEL_CREATING(id, fn)
// DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CREATING, id, wxID_ANY, (wxObjectEventFunction)
// (wxEventFunction) wxStaticCastEvent( wxPlotSelectionEventFunction, & fn ), (wxObject *) NULL ),
// #define EVT_PLOT_RANGE_SEL_CREATED(id, fn)
// DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CREATED,  id, wxID_ANY, (wxObjectEventFunction)
// (wxEventFunction) wxStaticCastEvent( wxPlotSelectionEventFunction, & fn ), (wxObject *) NULL ),
// #define EVT_PLOT_RANGE_SEL_CHANGING(id, fn)
// DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CHANGING, id, wxID_ANY, (wxObjectEventFunction)
// (wxEventFunction) wxStaticCastEvent( wxPlotSelectionEventFunction, & fn ), (wxObject *) NULL ),
#define EVT_PLOT_RANGE_SEL_CHANGED(id, fn)                                                        \
  wx__DECLARE_PLOTSELECTIONEVT(wxEVT_PLOT_RANGE_SEL_CHANGED, id, fn)

/*
#define EVT_PLOT_VALUE_SEL_CREATING(id, fn)
DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CREATING, id, wxID_ANY, (wxObjectEventFunction)
(wxEventFunction) wxStaticCastEvent( wxPlotEventFunction, & fn ), (wxObject *) NULL ), #define
EVT_PLOT_VALUE_SEL_CREATED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CREATED,  id,
wxID_ANY, (wxObjectEventFunction) (wxEventFunction) wxStaticCastEvent( wxPlotEventFunction, & fn ),
(wxObject *) NULL ), #define EVT_PLOT_VALUE_SEL_CHANGING(id, fn)
DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CHANGING, id, wxID_ANY, (wxObjectEventFunction)
(wxEventFunction) wxStaticCastEvent( wxPlotEventFunction, & fn ), (wxObject *) NULL ), #define
EVT_PLOT_VALUE_SEL_CHANGED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CHANGED,  id,
wxID_ANY, (wxObjectEventFunction) (wxEventFunction) wxStaticCastEvent( wxPlotEventFunction, & fn ),
(wxObject *) NULL ), #define EVT_PLOT_AREA_SEL_CHANGED(id, fn)
DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_AREA_SEL_CHANGED,   id, wxID_ANY, (wxObjectEventFunction)
(wxEventFunction) wxStaticCastEvent( wxPlotEventFunction, & fn ), (wxObject *) NULL ),
*/
