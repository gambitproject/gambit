...
enum wxPlotDataLoad_Type {
  ...
  // stop loading datafile if there is a blank line, will continue if a line is merely #commented
  // out
...
public:
  ...
  bool Create(const wxString &filename, int x_col, int y_col,
  ...
  void SetXStepValues(int start_index, int count = -1, double x_start = 0.0, double dx = 1.0);
  ...
  int GetMinMaxAve(const wxRangeIntSelection &rangeSel, wxPoint2DDouble *minXY,
                   wxPoint2DDouble *maxXY, wxPoint2DDouble *ave, int *x_min_index,
                   int *x_max_index, int *y_min_index, int *y_max_index) const;
  ...
  double CrossCorrelation(const wxPlotData &other, int runave = 0, int min = 0,
                          int max = -1) const;
  ...
  static wxString FFTHiPassFilterFormat(double fc, wxPlotData::FFTFilter_Type filter,
                                        double n = 5);
  static wxString FFTLoPassFilterFormat(double fc, wxPlotData::FFTFilter_Type filter,
                                        double n = 5);
  static wxString FFTNotchFilterFormat(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                                       double n = 5);
  static wxString FFTBandPassFilterFormat(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                                          double n = 5);
  ...
  wxPlotData FFTBandPassFilter(double lo, double hi, wxPlotData::FFTFilter_Type filter,
                               double n = 5);
  ...
  void SetSymbol(wxPlotSymbol_Type type, wxPlotPen_Type colour_type = wxPLOTPEN_NORMAL,
                 int width = 5, int height = 5, const wxPen *pen = NULL,
  ...
  bool operator==(const wxPlotData &plotData) const { return m_refData == plotData.m_refData; }
  bool operator!=(const wxPlotData &plotData) const { return m_refData != plotData.m_refData; }
