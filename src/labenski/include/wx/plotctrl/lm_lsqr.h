...
typedef bool (*LM_LeastSquareProgressHandler_)(const wxString &WXUNUSED(text),
                                               int WXUNUSED(current), int WXUNUSED(max));
...
//  wxPlotData data(wxPlotFunction("2.5*x*x-3*x+5+3.3*log(x)+13*exp(15*x/(x+4))", "x",dummy), 0,
//  1E-4, 10000);
...
//           wxPrintf(wxT("%s=%g; "), func.GetVariableName(k).c_str(),
//           lmLeastSquare.GetVariable(k));
//
//=============================================================================
...
protected:
  ...
  void lmdif(int m, int n, double x[], double fvec[], double ftol, double xtol, double gtol,
             int maxfev, double epsfcn, double diag[], int mode, double factor, int nprint,
             int *info, int *nfev, double fjac[], int ldfjac, int ipvt[], double qtf[],
  ...
  void qrfac(int m, int n, double a[], int lda, int pivot, int ipvt[], int lipvt, double rdiag[],
             double acnorm[], double wa[]);

  void qrsolv(int n, double r[], int ldr, int ipvt[], double diag[], double qtb[], double x[],
              double sdiag[], double wa[]);

  double enorm(int n, double x[]);

  void fdjac2(int m, int n, double x[], double fvec[], double fjac[], int ldfjac, int *iflag,
