...
public:
  ...
  {
    SetOption(name,
              wxString::Format(wxT("%d %d %d %d"), value.x, value.y, value.width, value.height),
              force);
  }
  ...
  bool operator==(const wxOptionValue &optValue) const { return m_refData == optValue.m_refData; }
  bool operator!=(const wxOptionValue &optValue) const { return m_refData != optValue.m_refData; }
