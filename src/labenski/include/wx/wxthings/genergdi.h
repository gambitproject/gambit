...
#define WX_DECLARE_OBJARRAY_WITH_DECL(T, name, expmode)                                           \
  WX_DECLARE_USER_EXPORTED_OBJARRAY(T, name, WXDLLIMPEXP_THINGS)
#endif
...
public:
  ...
  wxGenericColour(unsigned char red, unsigned char green, unsigned char blue,
  ...
  void Create(unsigned char red = 0, unsigned char green = 0, unsigned char blue = 0,
              unsigned char alpha = 255);
...
public:
  ...
  wxGenericBrush(const wxColour &colour, wxBrushStyle style = wxBRUSHSTYLE_SOLID) : wxObject()
  {
    Create(colour, style);
  }
