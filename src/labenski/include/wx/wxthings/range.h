...
WX_DECLARE_OBJARRAY(wxRangeInt, wxArrayRangeInt);
WX_DECLARE_OBJARRAY(wxRangeDouble, wxArrayRangeDouble);
WX_DECLARE_OBJARRAY(wxRangeIntSelection, wxArrayRangeIntSelection);
WX_DECLARE_OBJARRAY(wxRangeDoubleSelection, wxArrayRangeDoubleSelection);
...
public:
  ...
  inline bool Intersects(const wxRangeInt &r) const { return Intersect(r).IsEmpty(); }
...
public:
  ...
  // Remove the range to the selection, returning if anything was done, false if not already
  // selected
...
public:
  ...
  // returns if the range intersects the given range
  inline bool Intersects(const wxRangeDouble &r) const { return Intersect(r).IsEmpty(); }
  // returns the intersection of the range with the other
