...
public:
  ...
  {
    return wxSheetCoords(m_row + rows, m_col + cols);
  }
  ...
  bool IsLabelCell() const
  {
    return (m_row >= -1) && (m_col >= -1) && ((m_row == -1) || (m_col == -1));
  }
  ...
  wxSheetCoords operator+(const wxSheetCoords &c) const
  {
    return wxSheetCoords(m_row + c.m_row, m_col + c.m_col);
  }
  wxSheetCoords operator-(const wxSheetCoords &c) const
  {
    return wxSheetCoords(m_row - c.m_row, m_col - c.m_col);
...
public:
  ...
  {
    SetTopCoord(lt.m_row);
    SetLeftCoord(lt.m_col);
  }
  void SetLeftBottomCoords(const wxSheetCoords &lb)
  {
    SetBottomCoord(lb.m_row);
    SetLeftCoord(lb.m_col);
  }
  void SetRightTopCoords(const wxSheetCoords &rt)
  {
    SetTopCoord(rt.m_row);
    SetRightCoord(rt.m_col);
  }
  void SetRightBottomCoords(const wxSheetCoords &rb)
  {
    SetBottomCoord(rb.m_row);
    SetRightCoord(rb.m_col);
  }
  ...
  void SetSize(const wxSheetCoords &size)
  {
    m_height = size.m_row;
    m_width = size.m_col;
  }
  ...
  {
    return (row >= m_row) && (col >= m_col) && (row <= GetBottom()) && (col <= GetRight());
  }
  ...
  {
    return !IsEmpty() && !b.IsEmpty() && (m_row <= b.m_row) && (m_col <= b.m_col) &&
           (b.GetBottom() <= GetBottom()) && (b.GetRight() <= GetRight());
  }

  // do these two blocks intersect (overlap) each other
  bool Intersects(const wxSheetBlock &b) const { return !Intersect(b).IsEmpty(); }
  //! IsEmpty() && !b.IsEmpty() &&
  ...
  int Combine(const wxSheetBlock &block, wxSheetBlock &top, wxSheetBlock &bottom,
  ...
  int Delete(const wxSheetBlock &block, wxSheetBlock &top, wxSheetBlock &bottom,
             wxSheetBlock &left, wxSheetBlock &right) const;
  ...
  {
    return (m_row == b.m_row) && (m_height == b.m_height) && (m_col == b.m_col) &&
           (m_width == b.m_width);
  }
  ...
  // returns -1 if this block is more to the right then bottom, 0 is equal, 1 if to the left and
  // higher
  ...
  bool operator<(const wxSheetBlock &b) const
  {
    ...
    else if (m_row > b.m_row) {
      return false;
    }
    if (m_col < b.m_col) {
      return true;
    }
    else if (m_col > b.m_col) {
      return false;
    }
    if (m_height < b.m_height) {
      return true;
    }
    else if (m_height > b.m_height) {
      return false;
    }
    if (m_width < b.m_width) {
      return true; // else if (m_width > b.m_width) return false;
    }
    return false;
  }
...
public:
  ...
  wxSheetSelection(const wxSheetBlock &block, int options = wxSHEET_SELECTION_NONE);
  ...
  wxSheetSelection &operator=(const wxSheetSelection &other)
  {
    Copy(other);
    return *this;
  }
