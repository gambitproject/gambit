...
#else  // !wxUSE_GRID
WX_DECLARE_OBJARRAY_WITH_DECL(wxArrayString, wxSheetStringArray, class WXDLLIMPEXP_SHEET);
#endif // wxUSE_GRID
...
public:
  ...
  {
    return (coords.m_row >= 0) && (coords.m_col >= 0) && (coords.m_row < GetNumberRows()) &&
  ...
  virtual bool HasValue(const wxSheetCoords &coords) const { return !GetValue(coords).IsEmpty(); }
...
DECLARE_PAIRED_INT_DATA_ARRAYS(wxString, wxArrayString, wxPairArrayIntSheetString,
...
WX_DECLARE_HASH_MAP(int, wxSheetStringHash, wxIntegerHash, wxIntegerEqual,
                    wxSheetStringHashStringHash);
...
public:
  ...
  virtual void SetValueAsCustom(const wxSheetCoords &coords, const wxString &typeName,
                                void *value);
  ...
  virtual wxSheetCellAttr GetAttr(const wxSheetCoords &coords, wxSheetAttr_Type kind);
