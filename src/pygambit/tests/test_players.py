...
class TestGambitPlayers(unittest.TestCase):
    def setUp(self):
        self.strategic_game = pygambit.Game.new_table([2, 2])
        self.strategic_game.players[0].label = "Alphonse"
        self.strategic_game.players[1].label = "Gaston"
        self.extensive_game = pygambit.Game.new_tree()
    ...
    def test_game_add_duplicate_player_names(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert (
                str(w[0].message) ==
                "Another player with an identical label exists"
            )
        p1 = self.strategic_game.players[0]
        assert p1.label == "Alphonse"
        p2 = self.strategic_game.players[1]
    ...
    def test_game_players_index_exception_int(self):
        ...
        self.assertRaises(
            IndexError, self.strategic_game.players.__getitem__, 3
        )

    def test_game_players_index_exception_string(self):
        "Test to verify when a player label is not in the list of players"
        self.assertRaises(
            IndexError, self.strategic_game.players.__getitem__, "None"
        )

    def test_game_players_index_exception_player(self):
        "Test to verify when a player object is not in the players"
        self.strategic_game_2 = pygambit.Game.new_table([2, 2])
        self.assertRaises(
            TypeError,
            self.strategic_game.players.__getitem__,
            self.strategic_game_2.players[0]
        )

    def test_game_players_index_exception_badindex(self):
        "Test to verify when attempting to retrieve with invalid input"
        self.assertRaises(
            TypeError, self.strategic_game.players.__getitem__, 1.3
        )

    def test_add_strategic_game_players(self):
        """Test to ensure that players can be added to a strategic game
    ...
    def test_add_extensive_game_players(self):
        """Test to ensure that players can be added to an extensive game
        with no moves
        """
