...
class TullockGame(meanstat.MeanStatisticGame):
    ...
    def payoff(self, own, others):
        try:
            ...
            return (
                p_win * (self.omega - self.cost(own) + self.prize) +
...
class GeneralTullockGame(meanstat.MeanStatisticGame):
    ...
    def __init__(self, min_choice, max_choice, step_choice, prize, omega,
                 r=1.0, cost=lambda e: e):
        # Note that this is only a mean-statistic game (trivially!)
        # for two players.
        meanstat.MeanStatisticGame.__init__(
    ...
    def title(self):
        return (
            "GeneralTullockGame with N=2, prize=%s, endowment=%s, r=%s" %
