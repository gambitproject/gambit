...
class NodeChildren:
    ...
    def __getitem__(self, action: typing.Any) -> Node:
        """Returns the successor node which is reached after the action with label
        `action` is played.

        Raises
        ------
        KeyError
            If no action with that label exists at the node's infoset, or if the node
            is terminal.
        ValueError
        ...
        .. versionchanged:: 16.7.0
            Integer indexing is no longer supported; index by the action's label, or iterate.
            A label matching no action now raises ``KeyError``.

        .. versionchanged:: 17.0.0
            No longer accepts an ``Action`` object; index by its label instead.
        """
        if isinstance(action, str):
            ...
            try:
                act = _resolve_action_handle(self.parent.deref().GetInfoset(), action)
            except KeyError:
