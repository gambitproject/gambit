    ...
    for (st, cont) in enumerate(game.choices):
        if st == 0:
            ...
            lhs[st] = (
                logprofile[st]
                - logprofile[0]
                - lam * (profile.strategy_value(st) - profile.strategy_value(0))
            )
    return lhs
...
def sym_compute_jac(game, point):
    ...
    for (st, cont) in enumerate(game.choices):
        if st == 0:
            ...
            # column wrt lambda
            matrix[-1][st] = profile.strategy_value(0) - profile.strategy_value(st)
...
class StrategicQREPathTracer(object):
    ...
    def trace_strategic_path(self, game, max_lambda=1000000.0, callback=None):
        ...
        def on_step(game, points, p, callback):
            qre = LogitQRE(
                p[-1], game.mixed_strategy_profile(point=[math.exp(x) for x in p[:-1]])
            )
            points.append(qre)
            if callback:
                callback(qre)
