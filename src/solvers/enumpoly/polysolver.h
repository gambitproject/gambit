...
/// found, a predetermined search depth is reached, or a budget on the
/// number of rectangles examined is exhausted.  The depth bound alone is
/// not a practical guarantee of termination, since the number of
/// rectangles examined can still grow very quickly with depth; the
/// rectangle budget caps the total work directly.
...
private:
  ...
  // Recursive worker for FindRoots().  `p_budget` counts down the number of
  // rectangles still allowed to be examined; when it reaches zero, sets
  // `p_budgetExceeded` and the recursion unwinds without exploring further.
  void FindRoots(std::list<Vector<double>> &rootlist, const Rectangle<double> &r,
                 const size_t max_roots, size_t &p_budget, bool &p_budgetExceeded,
