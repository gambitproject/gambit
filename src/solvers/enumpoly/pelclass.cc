...
{
  const char *q[] = {"",    "n1",  "n2",  "n3",  "n4",  "n5",  "n6",  "n7",  "n8",  "n9",  "n10",
...
void PelView::Initialize_Idf_T_Gen_node(const Gen_node &node, const char *label) const
{
  node->type = Idf_T;
...
{
  const char *q[] = {" ",   "",    "n1",  "n2",  "n3",  "n4",  "n5",  "n6",  "n7",  "n8",  "n9",
...
polynomial1 PelView::GamPolyToPelPoly(const gPoly<double> &p, const int n, const Pring ring) const
{
...
Gen_node PelView::CreatePelicanVersionOfSystem(const gPolyList<double> &input,
...
int PelView::GutsOfGetMixedVolume(node A, node norms, const Imatrix T) const
{
  ...
  while (ptr != nullptr) {
    ...
    M = aset_M(ptc, M);
    if ((Imatrix_rref(M, &v) == aset_dim(A) - 1) && (IMrows(M) == aset_dim(A) - 1)) {
...
int PelView::GetMixedVolume(const Gen_node g) const
{
  ...
  if ((nargs == 0) || (nargs == 1 && Can_Be_Aset(Gen_elt(g, 1)) != TRUE) ||
...
polynomial1 PelView::HomotopyVariableMonomialPoly(const Pring ring, const int comp) const
{
  polynomial1 P;
...
Gen_node PelView::SolutionsDerivedFromContinuation(const Pring &ring, const Gen_node &Genpoly,
                                                   const Gen_node &Solve,
                                                   const Gen_node &pel_system, int tweak) const
{
...
Gen_node PelView::SolveCheckMaybeTryAgain(const Pring &ring, const Gen_node &Genpoly,
                                          const Gen_node &Qtrig, const Gen_node &pel_system)
{
  Gen_node sols, Solve;
  bool done(false);
  for (int tweak = 0; tweak <= 3 && !done; tweak++) {
    ...
    sols = SolutionsDerivedFromContinuation(ring, Genpoly, Solve, pel_system, tweak);
#ifdef PELVIEW_DEBUG
    ...
    if (solutionsarecorrect && !HasARedundancy(GambitRootsFromPelRoots(sols))) {
      done = true;
    }
    if (!done) {
      free_Gen_list(sols);
...
Gambit::List<Gambit::Vector<gComplex>> PelView::GambitRootsFromPelRoots(const Gen_node g) const
{
...
void PelView::DisplayComplexRootList(
    const Gambit::List<Gambit::Vector<gComplex>> &complexroots) const
{
#ifdef UNUSED
...
int PelView::Dmnsn() const { return input.Dmnsn(); }
...
{
  return (input == rhs.input && complexroots == rhs.complexroots && realroots == rhs.realroots &&
          mixedvolume == rhs.mixedvolume && solutionsarecorrect == rhs.solutionsarecorrect);
}

bool PelView::operator!=(const PelView &rhs) const { return !(*this == rhs); }

Gambit::List<Gambit::Vector<gComplex>> PelView::ComplexRoots() const { return complexroots; }

Gambit::List<Gambit::Vector<double>> PelView::RealRoots() const { return realroots; }

int PelView::MixedVolume() const { return mixedvolume; }

int PelView::NumComplexRoots() const { return complexroots.Length(); }

bool PelView::FoundAllRoots() const { return (NumComplexRoots() == MixedVolume()); }

int PelView::NumRealRoots() const { return realroots.Length(); }
