                                      ...
                                      int act1, int player2) const
{
  ...
  if (player2 == player1) {
    ...
    undisturbedPayoff = aggPtr->Pr[player2].inner_prod(
      aggPtr->projectedStrat[Node][player2].begin()->first, numNei, aggPtr->projFunctions[Node],
      aggPtr->payoffs[Node]);
  }
  has = true;
}
...
void aggame::savePayoff(cmatrix &dest, int player1, int act1, int player2, int act2,
                        agg::AggNumber result,
                        agg::trie_map<agg::AggNumber> &cache, bool partial) const
{
                           ...
                           agg::trie_map<agg::AggNumber> &cache) const
{
  ...
  if (!r.second) {
    ...
    r.first->second = aggPtr->Pr[player2].inner_prod(
      aggPtr->projection[Node][player2][act2], numNei, aggPtr->projFunctions[Node],
      aggPtr->payoffs[Node]);
