...
manual and testing parts of the language.
...
A variable stores one object of the corresponding data type, and
returns it when called.  To create a user defined variable, or to give
...
If the \verb+x+ parameter is a name which is not a currently visible
variable name, a new variable of the type is created and the value of
the expression passed to \verb+y+ is ``assigned'' to \verb+x+.  If
\verb+Assign+ is called with an argument \verb+varname+ which is a
currently visible variable, it results in an error if the data type
which \verb+y+ evaluates to is different that the current data type of
\verb+x+.  On the other hand, if the data types match, then
\verb+varname+ is ``modified:'' \verb+varname+ is first deleted from
the list of visible functions and then
...
Once a variable has been assigned, it can be reassigned only if it is
assigned a value which evaluates to the same data type.  Assigning a
variable to have a new data type will result in run time error.  To
change the data type of a variable, it must be unassigned, by using
the UnAssign[] function.

A variable can be thought of as a constant function with no arguments.
However, the scoping and visibility rules for variables are different
than for functions.  Variables have local scope and visibility.  
...
There are two main classes of errors which may arise in GCL programs,
parse errors and run time errors.  
...
is trminated by a linefeed or by a semicolon.  The GCL interpreter,
...
The Include[TEXT] function can be used to insert an external file into
your GCl program.  This command is useful to avoid unneccessary typing
of often used sequences of commands or definitions of user defined
functions.  Such commands can be put in a file, and included whenever
they are needed.
...
The function NewEfg[] can be used to create a new extensive form game.
...
will create a new extensive form game \verb+e+.  The game is a trivial
game with only one node, a \verb+root node+, and one player,
\verb+chance+.  By default, the game will have its data stored in
...
Outcomes can be defined and attached:
...
where \verb+e+ is the extensive form game, and \verb+filename.efg+ is
the filename of the file to be created.  It is recommended that the
filename extension \verb+.efg+ be used when saving extensive form games, as
the Gambit Graphics user interface recognizes this extension as
containing extensive form games.  Once an extensive form game has been
...
The function NewNfg[] can be used to create a new normal form game.
In order to do anything with it, a variable should be created to be
the new extensive form game.  Thus, the command
...
will create a new normal form game \verb+n+, with three players, where
...
\subsubsection{Creationg a normal form from an extensive form}
...
For a normal form game, the game is set up as a linear complementarity
problem, and solved via the Lemke-Howson Algorithm.  (See, eg.,
...
Mixed[nfg<->NFG, list->LIST(LIST(T))] =: MIXED
...
\item
\protect \large \begin{verbatim}
ElimDom[support->NFSUPPORT, {strong->BOOL}, {time<->FLOAT} =: NFSUPPORT
\end{verbatim} \normalsize
...
contains the amount of time consumed by the elimination.
\ed
