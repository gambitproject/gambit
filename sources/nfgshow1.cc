...
if (_pl1==_pl2)
{
	if (num_players!=2)	// do nothing
	{
		...
		spread->SetRowPlayer(pl1);spread->SetColPlayer(pl2);
		return;
	}
	else 	// switch row/col
		{_pl1=pl2;_pl2=pl1;}
...
if (spread->HaveProbs()) spread->MakeProbDisp();
if (spread->HaveDom()) spread->MakeDomDisp();
if (spread->HaveVal()) spread->MakeValDisp();
// Set new title
...
spread->SetRowPlayer(pl1);spread->SetColPlayer(pl2);
// This is really odd.  If I call UpdateVals during construction, the virtual
// function table is not yet created causing a crash.  Thus, we can only
// call it if this is NOT the first time this is called
if (!first_time)
{
	UpdateVals();
	UpdateSoln();
...
{
MyDialogBox *support=new MyDialogBox(spread,"Create Support",NFG_MAKE_SUPPORT_HELP);
...
for (int i=1;i<=nf.NumPlayers();i++)
{
	...
	int j;
	for (j=0;j<num_strats;j++) strats[j]=copystring(nf.Strategies(i)[j+1]->name);
	players[i]=new wxListBox(support,0,nf.Players()[i]->GetName(),TRUE,-1,-1,80,100,
									num_strats,strats);
	for (j=0;j<num_strats;j++) players[i]->SetSelection(j,TRUE);
	for (j=0;j<num_strats;j++) delete [] strats[j]; delete [] strats;
}
support->Go();
if (support->Completed()==wxOK)  {
  ...
  for (int i=1;i<=nf.NumPlayers();i++)  {
    ...
    if (sup->NumStrats(i)==0) failed=true; // Check that each player has at least one strat
  }
  delete support;
  if (!failed)
    {
...
if (out>nf.NumOutcomes())
	MyMessageBox("This outcome is not defined yet","Outcome",NFG_OUTCOME_HELP,spread);
else
{
	...
	if (x!=-1)	// dropped an outcome at the coordinates (x,y)
	{
		...
		{
			cur_profile[pl1]=row;cur_profile[pl2]=col;
...
if (what==0)	// label game
{
	...
	MyDialogBox *nfg_label_dialog = new MyDialogBox(spread, "Label Game",NFG_LABEL_HELP);
...
if (what==1) // label strategies
{
	...
	for(i=1;i<=num_players;i++)
		if (max_strats<disp_sup->NumStrats(i)) max_strats=disp_sup->NumStrats(i);
	SpreadSheet3D *labels=new SpreadSheet3D(num_players,max_strats,1,"Label Strategies",spread);
	...
	for(i=1;i<=num_players;i++)
	{
  	int j;
		...
		for(;j<=max_strats;j++)	labels->HiLighted(i,j,1,TRUE);
		labels->SetLabelRow(i,nf.Players()[i]->GetName());
	}
	labels->Redraw();
	labels->Show(TRUE);
	while (labels->Completed()==wxRUNNING) wxYield();	// wait for ok/cancel
...
if (what==2) // label players
{
	...
	labels->Go();
	if (labels->Completed()==wxOK)
		for(i=1;i<=num_players;i++)	nf.Players()[i]->SetName(player_labels[i]);
	for(i=1;i<=num_players;i++) delete [] player_labels[i];delete [] player_labels;
}
...
{
Bool disp_probs=spread->HaveProbs(),disp_dom=spread->HaveDom(),disp_val=spread->HaveVal();

MyDialogBox *norm_options=new MyDialogBox(spread,"Normal GUI Options",NFG_FEATURES_HELP);
wxFormItem *prob_fitem=norm_options->Add(wxMakeFormBool("Display strategy probs",&disp_probs));
norm_options->Add(wxMakeFormNewLine());
wxFormItem *val_fitem=norm_options->Add(wxMakeFormBool("Display strategy values",&disp_val));
norm_options->Add(wxMakeFormNewLine());
//wxFormItem *dom_fitem=
norm_options->Add(wxMakeFormBool("Display dominance",&disp_dom));
norm_options->AssociatePanel();
if (!cur_soln && !disp_probs) ((wxCheckBox *)prob_fitem->GetPanelItem())->Enable(FALSE);
if (!cur_soln && !disp_val) ((wxCheckBox *)val_fitem->GetPanelItem())->Enable(FALSE);
// DOM if (nf.NumStratSets()==1 && !disp_dom) ((wxCheckBox *)dom_fitem->GetPanelItem())->Enable(FALSE);
norm_options->Go1();
if (norm_options->Completed()==wxOK)
{
	bool change=false;
	if (!disp_probs && spread->HaveProbs())
	{spread->RemoveProbDisp();change=true;}
	if (!disp_dom && spread->HaveDom())
	{spread->RemoveDomDisp();change=true;}
	if (!disp_val && spread->HaveVal())
	{spread->RemoveValDisp();change=true;}
	if (disp_probs && !spread->HaveProbs() && cur_soln)
	{spread->MakeProbDisp();change=true;}
/* DOM
	if (disp_dom && !spread->HaveDom() && disp_sset!=1)
	{spread->MakeDomDisp();change=true;}
*/
	if (disp_val && !spread->HaveVal() && cur_soln)
	{spread->MakeValDisp();change=true;}

	if (change) {UpdateSoln();UpdateDom();spread->Redraw();}
}
delete norm_options;
}
...
int i;
for (i=0;i<NUM_SPREAD_EVENTS;i++) events[i+1]=spread_events[i];
...
// different from the default SpreadSheet3D one to warrant an override
// of the MakeMenuBar function.  Thus, no features are selected in the
// main SpreadSheet3D constructor, except for the panel.
NormalSpread::NormalSpread(const NFSupport *sup,int _pl1,int _pl2,NfgShow *p,wxFrame *pframe) :
		...
		strat_profile(sup->Game().NumPlayers()),parent(p),pl1(_pl1),pl2(_pl2),
...
for (i=0;i<num_players;i++) delete [] player_names[i]; delete [] player_names;
row_choice->SetClientData((char *)this);col_choice->SetClientData((char *)this);
row_choice->SetSelection(_pl1-1);col_choice->SetSelection(_pl2-1);
sub_panel->NewLine();
// Create the list boxes for strategies for each player
new wxMessage(sub_panel,"Profile:");
sub_panel->NewLine();
for (i=1;i<=num_players;i++)
{
	...
	for (j=0;j<dimensionality[i];j++) delete [] strat_profile_str[j]; delete [] strat_profile_str;
}
sub_panel->Fit();
SetEditable(FALSE);
...
wxMenu *solve_menu=new wxMenu;
	...
	wxMenu *solve_settings_menu=new wxMenu;
		solve_settings_menu->Append(NFG_SOLVE_ALGORITHM_MENU,"&Algorithm","Choose current algorithm");
		solve_settings_menu->Append(NFG_SOLVE_DOMINANCE_MENU,"&Dominance","ElimDom options");
	solve_menu->Append(NFG_SOLVE_SETTINGS_MENU,"&Custom",solve_settings_menu,"Control default options");
...
if (what==1) // strategies
{
	...
	for(i=1;i<=dimensionality.Length();i++) strat_profile[i]->SetSelection(profile[i]-1);
  Redraw();
}
if (what==2) // players
{
	// the the row, col player choicebox
	const Nfg &nf=disp_sup->Game();
	row_choice->Clear();	col_choice->Clear();
	for (int i=1;i<=nf.NumPlayers();i++)
	{
	...
	// Set new title
	SetTitle(nf.GetTitle()+" : "+nf.Players()[pl1]->GetName()+" x "+nf.Players()[pl2]->GetName());
}
}
//************ extra features for displaying dominance, probs, vals **********
// these features each create an extra row and columns.  They always go in
// order: 1. Prob, 2. Domin, 3. Value.  Make sure to update the labels if
// a feature is turned on/off.  Note that if you turn on a feature that is
// already on, no new rows/cols will be created, but the labels will be updated
...
SetLabelRow(row,"Domin");SetLabelCol(col,"Domin");
features.dom=1;
}
...
case	NFG_FILE_SAVE:parent->Save();break;
case  NFG_ACCL_PAYOFF: parent->ChangePayoffs(CurRow(),CurCol()); break;
case	NFG_ACCL_NEXT_PAYOFF: parent->ChangePayoffs(CurRow(),CurCol(),true); break;
default: SpreadSheet3D::OnMenuCommand(id); break;
}
}
