				 ...
				 gOutput &tracefile, gStatus &status)
{
  ...
  if(precision == precRATIONAL) {
    ...
    for (int i = 1; i <= players.Length(); i++)   {
      ...
      any |= ComputeMixedDominated<gRational>(N, S, *newS, pl, strong,
    ...
    for (int i = 1; i <= players.Length(); i++)   {
      ...
      any |= ComputeMixedDominated<double>(N, S, *newS, pl, strong,
...
template bool 
ComputeMixedDominated(const Nfg &nfg, const NFSupport &S, 
		      NFSupport &R,int pl, bool strong, gRational junk,
		      gOutput &tracefile, gStatus &status);
template bool 
ComputeMixedDominated(const Nfg &nfg, const NFSupport &S, 
		      NFSupport &R,int pl, bool strong, double junk,
		      gOutput &tracefile, gStatus &status);
