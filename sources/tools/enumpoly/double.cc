...
#include "libgambit/gsmatrix.imp"
...
std::string ToText(gDouble d)
{
  sprintf(mygconvert_buffer, "%.*f", myprecision, d.ToDouble());
  return std::string(mygconvert_buffer);
}

// conversions from strings to numbers

gDouble TOgDouble(const std::string &s)
{ 
...
template class gbtSquareMatrix<gDouble>;
