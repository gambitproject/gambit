...
EfgOutcomeDialogC::OutcomeDragger::OutcomeDragger(EfgOutcomeDialogC *parent_,
                                                  TreeWindow *tw_)
    : parent(parent_), tw(tw_), drag_now(0)
{
#ifdef wx_msw
...
EfgOutcomeDialogC::EfgOutcomeDialogC(int rows, int cols, Efg &ef_,
    ...
    : SpreadSheet3D(rows, cols, 1, "Outcomes [S]", 
                    (wxFrame *)tw_->GetParent(), ANY_BUTTON),
...
void EfgOutcomeDialogC::OnSettings(void)
{
    MyDialogBox *options_dialog = 
        new MyDialogBox(this, "Outcome Settings", EFG_OUTCOME_HELP);
    wxStringList *opt_list =
        new wxStringList("Compact Format", "Long Entries", 0);
    ...
    if (options_dialog->Completed() == wxOK)
    {
        ...
        if (new_dialog_type != dialog_type)
        {
            ...
            wxWriteResource("Gambit", "EfgOutcome-Dialog-Type", 
...
void EfgOutcomeDialogC::SetCurOutcome(const gText &out_name)
{
    ...
    if (out_name != "")
    {
        ...
        {
            if ((ef.Outcomes()[i])->GetName() == out_name) 
                out = i;
        }
    }
                        ...
                        ef_.NumPlayers()+1, ef_, tw_, parent_)
{
    ...
    for (j = 1; j <= ef.NumPlayers(); 
         j++) DrawSettings()->SetColWidth(2+ToTextPrecision(), j);
...
void EfgOutcomeDialogLong::OnSelectedMoved(int row, int col, SpreadMoveDir how)
{
    if (col == 1)
    {
        SetCurCol(2);
        return;
    }   // player name column is off limits
    ...
    if (col == 3)  // outcome name column is special
    {
        if (how == SpreadMoveRight || how == SpreadMoveJump)
        {
            ...
            {
                row = ef.NumPlayers()*(outc_num-1)+1;
                SetCurRow(row);
            }
        }
...
{
    *row = (outc_num-1) * ef.NumPlayers() + player;
    *col = 2;
}
...
{
    *row = (outc_num-1) * ef.NumPlayers() + 1;
    *col = 3;
}
