...
void gbtNfgNavigate::OnUpdate(gbtGameView *)
{
  ...
  if (GetNumberRows() < m_doc->GetGame()->NumPlayers()) {
    AppendRows(m_doc->GetGame()->NumPlayers() - GetNumberRows());
  }
  else if (GetNumberRows() > m_doc->GetGame()->NumPlayers()) {
    DeleteRows(0, GetNumberRows() - m_doc->GetGame()->NumPlayers());
  }
