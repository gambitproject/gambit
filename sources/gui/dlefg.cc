...
//=========================================================================
//                dialogMoveAddAfterAction: Member functions
//=========================================================================

BEGIN_EVENT_TABLE(dialogMoveAddAfterAction, guiAutoDialog)
  EVT_LISTBOX(idMOVE_PLAYER_LISTBOX, dialogMoveAddAfterAction::OnPlayer)
  EVT_LISTBOX(idMOVE_INFOSET_LISTBOX, dialogMoveAddAfterAction::OnInfoset)
END_EVENT_TABLE()

dialogMoveAddAfterAction::dialogMoveAddAfterAction(wxWindow *p_parent, FullEfg &p_efg,
			     const gText &p_title, EFPlayer *p_player,
			     Infoset *p_infoset, int p_branches)
  : guiAutoDialog(p_parent, p_title),
    m_efg(p_efg), m_branches(p_branches)
{
  ...
  m_ancestorItem = new wxListBox(this, idMOVE_ANCESTOR_LISTBOX);
  for (int pl = 1; pl <= m_efg.NumPlayers(); pl++) {
    EFPlayer *player = m_efg.Players()[pl];
    for (int iset = 1; iset <= player->NumInfosets(); iset++) {
      Infoset *infoset = player->Infosets()[iset];
      for (int act = 1; act <= infoset->NumActions(); act++) {
	m_ancestorItem->Append((char *)
			       infoset->GetActionName(act));
      }
    }
  }
  m_ancestorItem->SetSelection(0);
  ...
  wxBoxSizer *ancestorSizer = new wxBoxSizer(wxVERTICAL);
  ancestorSizer->Add(new wxStaticText(this, -1, "After action"),
		     0, wxALL, 5);
  ancestorSizer->Add(m_ancestorItem, 0, wxEXPAND | wxALL, 5);

  wxBoxSizer *playerInfosetSizer = new wxBoxSizer(wxHORIZONTAL);
  playerInfosetSizer->Add(playerSizer, 1, wxALL, 5);
  playerInfosetSizer->Add(infosetSizer, 1, wxALL, 5);
  playerInfosetSizer->Add(ancestorSizer, 1, wxALL, 5);
...
void dialogMoveAddAfterAction::OnPlayer(wxCommandEvent &)
{
  int playerNumber = m_playerItem->GetSelection(); 
...
void dialogMoveAddAfterAction::OnInfoset(wxCommandEvent &)
{
  int infosetNumber = m_infosetItem->GetSelection();
...
NodeAddMode dialogMoveAddAfterAction::GetAddMode(void) const
{
  return (m_infosetItem->GetSelection() == 0) ? NodeAddNew : NodeAddIset;
}

EFPlayer *dialogMoveAddAfterAction::GetPlayer(void) const
{
  int playerNumber = m_playerItem->GetSelection();
...
Infoset *dialogMoveAddAfterAction::GetInfoset(void) const
{
  EFPlayer *player = GetPlayer();
  int infosetNumber = m_infosetItem->GetSelection();
...
Action *dialogMoveAddAfterAction::GetAncestor(void) const
{
  int actionNumber = m_ancestorItem->GetSelection();
  int count = 0;
  ...
  for (int pl = 1; pl <= m_efg.NumPlayers(); pl++) {
    ...
    for (int iset = 1; iset <= player->NumInfosets(); iset++) {
      Infoset *infoset = player->Infosets()[iset];
      for (int act = 1; act <= infoset->NumActions(); act++) {
	if (count++ == actionNumber) {
	  return infoset->Actions()[act];
	}
      }
    }
  }
...
int dialogMoveAddAfterAction::GetActions(void) const
{
  Infoset *infoset = GetInfoset();
  if (infoset)
    return infoset->NumActions();
