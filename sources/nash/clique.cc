...
int main()
{
  gbtArray<edge> edgelist1(MAXEDGES);
...
EnumCliques::EnumCliques(gbtArray<edge> &edgelist,int maxinp1,int maxinp2 ) 
...
void EnumCliques::genincidence(
  ...
  /* generates the incidence matrix  connected from the edgelist
     ...
     */
{
  gbtArray<int> newnode1(0,maxinp1-1) ;
  gbtArray<int> newnode2(0,maxinp2-1) ;
  int i,j, newi, newj ;
...
// -------------------------------------------------- 
int EnumCliques::getconnco(gbtArray<int> &firstedge,
			   gbtArray<edge> &edgelist
			   )
...
{
  int numco, newedge ;
  gbtArray<int> co1(0,maxinp1-1), co2(0,maxinp2-1);   // components of node1,2  
