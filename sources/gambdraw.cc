...
void GambitDrawSettings::PlayerColorDialog(gBlock<gString> &player_names)
{
...
wxFormItem *name_item=wxMakeFormString("Player",&name,wxFORM_SINGLE_LIST,
			new wxList(wxMakeConstraintStrings(name_list),0),NULL,wxVERTICAL);
...
wxFormItem *color_item=wxMakeFormString("Color",&color,wxFORM_SINGLE_LIST,
			new wxList(wxMakeConstraintStrings(color_list),0),NULL,wxVERTICAL);
...
player_color_struct pcs={this,color_listbox};
...
if (new_pos>=0)
{
	player_color_struct *pcs=(player_color_struct *)ob.wxWindow::GetClientData();
	if (prev_pos>=0)
