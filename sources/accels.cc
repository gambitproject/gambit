...
EditAccelDialog::EditAccelDialog(gList<Accel> &acc_, const gArray<AccelEvent> &events_, 
                                 wxFrame *parent, char *title):
    wxDialogBox(parent, title, TRUE), acc(acc_), events(events_)
{
    // Create panel items
    (void)new wxGroupBox(this, "Key", 4, 2, 212, 218, 0, "key_box");
    key_text = new wxText(this, (wxFunction)key_text_func, 
                          NULL, "", 21, 36, 28, 30, 0, "key_text");
    key_choice = new wxChoice(this, (wxFunction)key_choice_func, 
                              NULL, 68, 36, 105, -1, NUM_KEYS, key_names, 
                              0, "key_choice");
    
    char **event_names = new char *[events.Length()];
    
    for (int i = 1; i <= events.Length(); i++) 
        event_names[i-1] = events[i].name;
    
    SetLabelPosition(wxVERTICAL);
    event_choice = new wxChoice(this, (wxFunction)event_func, 
                                "Event", 240, 96, -1, -1, events.Length(), 
                                event_names, 0, "event_choice");
    SetLabelPosition(wxHORIZONTAL);
    
    wxButton *add_button    = new wxButton(this, (wxFunction)add_func, 
                                           "Add", 23, 242, -1, -1, 0, 
                                           "add_button");
    wxButton *delete_button = new wxButton(this, (wxFunction)delete_func, 
                                           "Delete", 114, 242, 80, -1, 0, 
                                           "delete_button");
    wxButton *close_button  = new wxButton(this, (wxFunction)close_func, 
                                           "Close", 214, 242, -1, -1, 0, 
                                           "close_button");
    
    (void)new wxButton(this, (wxFunction)help_func, 
                       "Help", 294, 242, -1, -1, 0, 
                       "help_button");
    
    key_ctrl  = new wxRadioBox(this, NULL, "Ctrl", 
                               23, 78, -1, -1, 3, accel_state_str, 3, wxHORIZONTAL, 
                               "ctrl_state");
    key_shift = new wxRadioBox(this, NULL, "Shift", 
                               22, 143, -1, -1, 3, accel_state_str, 3, wxHORIZONTAL, 
                               "shift_state");
...
{   
    if (key_choice->GetSelection()) 
        return Name2Key(key_choice->GetStringSelection());
    
    if (key_text->GetValue()) 
        return (int)(key_text->GetValue()[0]);
...
void EditAccelDialog::key_text_func(wxText& ob, wxCommandEvent& )
{
    ...
    if (v)
    {
        ...
        {
            char tmp_str[2];
            sprintf(tmp_str, "%c", v[0]);
            ob.SetValue(tmp_str);
        }
        
        if (strcmp(v, "") != 0) // if key_text is non-blank
        {
            wxChoice *key_choice = (wxChoice *)ob.GetClientData();
            key_choice->SetSelection(0);
        }
    }
}
...
void EditAccelDialog::OnAddFunc(void)
{
    ...
    {
        if ((acc[i].ctrl == key_ctrl->GetSelection() || 
             key_ctrl->GetSelection() == aEither) &&
            (acc[i].shift == key_shift->GetSelection() || 
             key_shift->GetSelection() == aEither) &&
