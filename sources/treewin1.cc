...
// Check for terminal nodes -- just take no action (why bother with an error?)
  ...
  try   {
    ...
    branch_num_dialog->Form()->
          Add(wxMakeFormString("Branch", &branch_name, wxFORM_CHOICE,
    ...
    if (branch_name)   delete [] branch_name;
    if (branch_num_dialog)   delete branch_num_dialog;
    throw;
  }
#endif   // USE_EXCEPTIONS
}
...
void TreeWindow::node_label(void)
{
  ...
  try   {
    ...
    if (label_dialog)   delete label_dialog;
...
void TreeWindow::infoset_switch_player(void)
{
  ...
  try  {
    ...
    if (infoset_switch_dialog)  delete infoset_switch_dialog;
    if (player_name)   delete [] player_name;
  }
#endif    // USE_EXCEPTIONS
}
...
void TreeWindow::infoset_reveal(void)
{
  ...
  try  {
    ...
    if (infoset_reveal_dialog)   delete infoset_reveal_dialog;
    if (player_names)   {
      for (int i = 1; i <= ef.NumPlayers(); i++)
        if (player_names[i - 1])  delete [] player_names[i - 1];
      if (player_names)   delete [] player_names;
    }
  }
#endif   // USE_EXCEPTIONS
}
...
void TreeWindow::action_insert(void)
{
  ...
  branch_insert_dialog->Add(wxMakeFormString("Where", &action_str,
					     wxFORM_CHOICE,
...
void TreeWindow::action_delete(void)
{
  ...
  MyDialogBox *branch_delete_dialog = new MyDialogBox(pframe, "Delete Branch",
						      EFG_ACTION_HELP);
  Infoset *iset=cursor->GetInfoset();
  wxStringList *action_list=new wxStringList;
  char *action_str=new char[30];
  for (int i=1;i<=iset->NumActions();i++) 
    action_list->Add(iset->GetActionName(i));
  branch_delete_dialog->Add(wxMakeFormString("Which", &action_str,
					     wxFORM_CHOICE,
					     new wxList(wxMakeConstraintStrings(action_list),0)));
