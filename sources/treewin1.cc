...
if (ef.GetChance()->GetName()==name) return ef.GetChance();
return 0;
}
//-----------------------------------------------------------------------
...
if (last_ef!=&ef) {player=0;infoset=0;last_ef=&ef;}
...
if (node_add_dialog.Completed()==wxOK)
{
	...
	if (set_names) {node_label();infoset_label();}
	if (player==ef.GetChance())	action_probs();
}
}
...
{wxMessageBox("Cannot delete root node!","Error",wxOK | wxCENTRE,pframe);return;}

// Check for terminal nodes -- just take no action (why bother with an error?)
if (cursor->NumChildren() == 0)  return;

MyDialogBox *branch_num_dialog=new MyDialogBox(this,"Keep Branch",EFG_NODE_HELP);
...
MyDialogBox *label_dialog = new MyDialogBox(pframe, "Label Node",EFG_NODE_HELP);
wxFormItem *label_item=wxMakeFormString("Label", &label,wxFORM_DEFAULT,
...
label_dialog->Go1();
if (label_dialog->Completed()==wxOK) cursor->SetName(label);
delete label_dialog;
delete [] label;
}
...
//***********************************************************************
#define DRAG_OUTCOME_END		7		// also defined in btreewn.cc
void TreeWindow::node_outcome(int out,int x,int y)
{
if (out>ef.NumOutcomes())
	MyMessageBox("This outcome is not defined yet","Outcome",EFG_OUTCOME_HELP,pframe);
else
{
...
{
if (!mark_node) {MyMessageBox("The mark is not set", "Error",EFG_INFOSET_HELP,pframe);return;}
if (!mark_node->GetInfoset()) {wxMessageBox("Marked node belongs to no infosets");return;}
if (!cursor->GetInfoset()) {wxMessageBox("Cursor belongs to no infosets");return;}
if (mark_node->GetSubgameRoot() != cursor->GetSubgameRoot())
	{MyMessageBox("Can not merge infosets across subgames","Error",EFG_INFOSET_HELP,pframe); return;}
if (mark_node->GetPlayer()!=cursor->GetPlayer())
	{MyMessageBox("Can not merge infosets with different players.\nChange player and retry","Error",EFG_INFOSET_HELP,pframe); return;}
...
if (!cursor->GetInfoset()) {wxMessageBox("Cursor belongs to no infosets");return;}
if (mark_node->GetSubgameRoot() != cursor->GetSubgameRoot())
{MyMessageBox("Can not join infosets across subgames","Error",EFG_INFOSET_HELP,pframe); return;}
...
{
if (!cursor->GetInfoset()) {wxMessageBox("This node belongs to no infosets");return;}
char *label=new char[MAX_LABEL_LENGTH];
...
void TreeWindow::infoset_switch_player(void)
{
  ...
  MyDialogBox *infoset_switch_dialog =
	 new MyDialogBox(this, "Set New Player", EFG_INFOSET_HELP);
  wxStringList *player_list = new wxStringList;
  char *player_name = new char[20];
  ...
  infoset_switch_dialog->Add(wxMakeFormString("Player", &player_name,
															 wxFORM_CHOICE,
...
{
if (!cursor->GetInfoset()) {wxMessageBox("This node belongs to no infosets");return;}
MyDialogBox *infoset_reveal_dialog=new MyDialogBox(this,"Reveal to Players",EFG_INFOSET_HELP);
char **player_names=new char *[ef.NumPlayers()];
int i;
for (i=1;i<=ef.NumPlayers();i++)
	player_names[i-1]=copystring(ef.Players()[i]->GetName());
wxListBox *player_item=new wxListBox(infoset_reveal_dialog,0,"Players",wxMULTIPLE,
										-1,-1,-1,-1,ef.NumPlayers(),player_names);
infoset_reveal_dialog->Go();
if (infoset_reveal_dialog->Completed()==wxOK)
{
	gBlock<EFPlayer *> players;
	for (i=1;i<=ef.NumPlayers();i++)
		if (player_item->Selected(i-1)) players.Append(ef.Players()[i]);
	ef.Reveal(cursor->GetInfoset(),players);
	infosets_changed=TRUE;
}

delete infoset_reveal_dialog;
for (i=1;i<=ef.NumPlayers();i++) delete [] player_names[i-1]; delete [] player_names;
...
{
if (!cursor->GetInfoset()) {wxMessageBox("This node belongs to no infosets");return;}
if (cursor->GetInfoset()->NumActions()<1) {wxMessageBox("No actions to label");return;}
...
{
	MyDialogBox *branch_label_dialog=new MyDialogBox (pframe, "Action Label",EFG_ACTION_HELP);
	...
	if (branch_label_dialog->Completed()==wxOK)
		for (i=1;i<=actions_now;i++)
			cursor->GetInfoset()->SetActionName(i+d*ENTRIES_PER_DIALOG,action_names[i-1]);
	delete branch_label_dialog;
	for (i=1;i<=actions_now;i++) delete [] action_names[i-1]; delete [] action_names;
}
}
...
if (num_children==0)
{MyMessageBox("Terminal node: cannot insert branch","Error",EFG_ACTION_HELP,pframe);return;}

MyDialogBox *branch_insert_dialog = new MyDialogBox(pframe, "Insert Branch",EFG_ACTION_HELP);
Infoset *iset=cursor->GetInfoset();
wxStringList *action_list=new wxStringList;char *action_str=new char[30];
for (int i=1;i<=iset->NumActions();i++) action_list->Add(iset->GetActionName(i));
branch_insert_dialog->Add(wxMakeFormString("Where", &action_str,wxFORM_CHOICE,
...
if (num_children==0)
{MyMessageBox("Terminal node: cannot delete branch","Error",EFG_ACTION_HELP,pframe);return;}

MyDialogBox *branch_delete_dialog = new MyDialogBox(pframe, "Delete Branch",EFG_ACTION_HELP);
Infoset *iset=cursor->GetInfoset();
wxStringList *action_list=new wxStringList;char *action_str=new char[30];
for (int i=1;i<=iset->NumActions();i++) action_list->Add(iset->GetActionName(i));
branch_delete_dialog->Add(wxMakeFormString("Which", &action_str,wxFORM_CHOICE,
											new wxList(wxMakeConstraintStrings(action_list),0)));
...
if (!n->GetInfoset())
{
	wxMessageBox("Probs can only be set for a CHANCE player","Error",wxOK | wxCENTRE,pframe);
	return;
}
...
MyDialogBox *tree_label_dialog = new MyDialogBox(pframe, "Label Tree",EFG_TREE_HELP);
wxFormItem *label_item=wxMakeFormString("Label", &label,wxFORM_DEFAULT,
...
{
if (cursor->NumChildren()==0)
{wxMessageBox("Terminal node: cannot delete tree","Error",wxOK | wxCENTRE,pframe);return;}

MyMessageBox *tree_delete_dialog = new MyMessageBox("Are you sure?","Delete Tree",EFG_TREE_HELP,pframe);
...
{
if (!mark_node)
{MyMessageBox("The mark is not set", "Error",EFG_TREE_HELP,pframe);return;}
if (cursor->GetSubgameRoot() != mark_node->GetSubgameRoot())
	{MyMessageBox("Can not copy across subgames","Error",EFG_TREE_HELP,pframe); return;}
nodes_changed=TRUE;
...
if (ID.GameChanged()) infosets_changed=true;
}
...
if (cursor->GetSubgameRoot()==cursor)
	{wxMessageBox("This node is already a subgame root"); return;}
if (!ef.IsLegalSubgame(cursor))
	{wxMessageBox("This node is not a root of a valid subgame"); return;}
...
{
if (cursor->GetSubgameRoot()!=cursor)
	{wxMessageBox("This node is not a subgame root");return;}
if (cursor->GetSubgameRoot()==ef.RootNode())
	{wxMessageBox("Root node is always a subgame root");return;}
ef.RemoveSubgame(cursor);
for (int i=1;i<=subgame_list.Length();i++)
	if (subgame_list[i].root==cursor) subgame_list.Remove(i);
must_recalc=true;
}
...
for (int i=1;i<=subgame_list.Length();i++)
	if (subgame_list[i].root==cursor)
		{subgame_list[i].expanded=false; must_recalc=true;;return;}
...
for (int i=1;i<=subgame_list.Length();i++)
	if (subgame_list[i].root==cursor)
		{subgame_list[i].expanded=true; must_recalc=true;return;}
wxMessageBox("This node is not a subgame root");
}
	...
	if (subgame_list[i].root==cursor)
		{
			for (int j=1;j<=subgame_list.Length();j++)
				if (ef.IsSuccessor(subgame_list[j].root,cursor))
					{subgame_list[j].expanded=true; must_recalc=true;}
			return;
		}
wxMessageBox("This node is not a subgame root");
}
...
for (int i=1;i<=subgame_list.Length();i++)
	if (subgame_list[i].root==cursor)
		{subgame_list[i].expanded=!subgame_list[i].expanded; must_recalc=true;return;}
wxMessageBox("This node is not a subgame root");
}
...
if (od.Completed()==wxOK)
	switch (od.GetMedia())
	{
		...
		default:wxMessageBox("Contact the author\negrayver@cco.caltech.edu","Internal Error");break;
	}
}
...
private:
	...
	int num_pages;
public:
	ExtensivePrintout(TreeWindow *s,wxOutputOption f,const char *title="ExtensivePrintout");
...
if (!dc->Colour) 	tree->draw_settings.SetOutcomeColor(color_outcomes);
...
ExtensivePrintout::ExtensivePrintout(TreeWindow *t,wxOutputOption f,const char *title)
...
void ExtensivePrintout::GetPageInfo(int *minPage, int *maxPage, int *selPageFrom, int *selPageTo)
{
num_pages=1;
*minPage = 0;
*maxPage = num_pages;
...
assert(fit==0 && preview==0); // suppress 'not used' message
#endif
}
//***********************************************************************
...
if (dc.Ok())
{
	...
	if (fit==wxFITTOPAGE)
	{
		...
		int w,h;
		GetSize(&w,&h);
		draw_settings.SetZoom(gmin((float)w/(float)draw_settings.MaxX(),(float)h/(float)draw_settings.MaxY()),true);
	}
	...
	dc.EndPage();
	dc.EndDoc();
	if (fit==wxFITTOPAGE)	draw_settings.SetZoom(old_zoom);
...
if (dc_mf.Ok())
{
  ...
  if (!save_mf) dc_mf.SetUserScale(2.0,2.0);
	Render(dc_mf);
	if (fit==wxFITTOPAGE)	draw_settings.SetZoom(old_zoom);
	...
	if (mf)
	{
		...
		if (!success) wxMessageBox("Copy Failed","Error",wxOK | wxCENTRE,pframe);
		delete mf;
	}
	if (save_mf)	wxMakeMetaFilePlaceable(metafile_name,0,0,(int)(dc_mf.MaxX()+10),(int)(dc_mf.MaxY()+10));
}
#else
wxMessageBox("Metafiles are not supported under X");
assert(fit==0 && save_mf==0);
#endif
}
...
if (zoom_window) // already exists, delete it
{
	...
	delete zoom_window->GetParent();zoom_window=0;
}
else
{
	zoom_window=new TreeZoomWindow(pframe,this,node_list,(const Infoset *&)hilight_infoset,
	(const Infoset *&)hilight_infoset1,(const Node *&)mark_node,(const Node *&)cursor,
	(const Node *&)subgame_node,draw_settings,GetNodeEntry(cursor));
}
...
else
{
	...
	if (s) {draw_settings.SaveOptions(copystring(s));delete [] s;}
}
}
//***********************************************************************
...
else
{
	...
	if (s) {draw_settings.LoadOptions(copystring(s));delete [] s;}
}
must_recalc=TRUE;
}
