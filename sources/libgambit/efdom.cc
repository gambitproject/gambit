...
{
  const gbtEfgInfoset *infoset = a->GetInfoset();
  ...
  const gbtEfgPlayer *player = infoset->GetPlayer();
  ...
  if (!conditional) {
    ...
    gbtList<const gbtEfgNode *> nodelist = SAct.ReachableNodesInInfoset(infoset);  
