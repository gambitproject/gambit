...
#include "wx/wxthings/filebrws.h"
...
wxFileBrowserEvent::wxFileBrowserEvent(wxEventType commandType,
                                       wxFileBrowser *fileBrowser, int Id)
...
//    m_viewToolBar->AddControl(m_viewButton);
    ...
    int s = 10;
    viewImg.Resize(wxSize(w + s, h), wxPoint(0,0));
    unsigned char r, g, b;
...
//  m_parent->m_wxwindow == NULL so nothing is done
    ...
    void GtkToolbarResizeWindow(wxWindow* win, const wxSize& size)
    {
        ...
        gtk_widget_set_usize (widget, size.x, size.y);
...
{
    wxCHECK_MSG((n>=0) && (n<m_filterCombo->GetCount()), false, wxT("Invalid filter item"));
