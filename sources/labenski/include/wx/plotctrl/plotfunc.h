...
// 
//-----------------------------------------------------------------------------
class WXDLLIMPEXP_PLOTLIB wxPlotFunction: public wxPlotCurve
{
public:
    wxPlotFunction() : wxPlotCurve() {}
    wxPlotFunction(const wxPlotFunction &curve) : wxPlotCurve() { Create(curve); }
    wxPlotFunction(const wxString &function, const wxString &vars = wxT("x")) : wxPlotCurve() 
    ...
    bool operator == (const wxPlotFunction& plotFunc)
...
private :
    DECLARE_ABSTRACT_CLASS(wxPlotFunction);
};
