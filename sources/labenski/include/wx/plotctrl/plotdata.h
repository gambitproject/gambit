/////////////////////////////////////////////////////////////////////////////
// Name:        plotdata.h
// Purpose:     wxPlotCurve, wxPlotFunction, wxPlotData for wxPlotWindow
...
//        otherwise it might not be displayed properly in a wxPlotWindow
//        
//-----------------------------------------------------------------------------

class WXDLLIMPEXP_PLOTLIB wxPlotData : public wxPlotCurve
{
public:
    ...
    wxPlotData( const wxPlotFunction &plotfunction, double x_start, double dx, int points ):wxPlotCurve()
        { Create( plotfunction, x_start, dx, points ); }
        
    virtual ~wxPlotData() {}
        
    // Ref the source plotdata
    bool Create( const wxPlotData& plotData );
    // create from a wxPlotFunction, starting at x-start, with dx increment, for number of points
    bool Create( const wxPlotFunction &plotFunction, double x_start, double dx, int points );
    // allocate memory and if zero then initialize it to zeroes
    //   don't use uninitialized data since trying to plot it will cause problems
    ...
    // assign the malloc(ed) data sets to this plotdata, if !static_data they'll be free(ed) on destruction
    bool Create( double *x_data, double *y_data, int points, bool static_data = false );

    // Make true (not refed) copy of this, if copy_all = true then copy header, filename, pens, etc
    bool Copy( const wxPlotData &source, bool copy_all = false ); 
    // Only copy the header, filename, pens, etc...
    bool CopyExtra( const wxPlotData &source );
        
    // Resize the data by appending or cropping off a number of samples to the end
    //   if samples > 0 then append samples with a timestep of the first two points and of y_value
    bool Resize( int samples, double y_value = 0.0 );
    ...
    // Get a sub-section of this curve from index of size count points, if count < 0 then to end
    wxPlotData GetSubPlotData(int index, int count = -1) const;

    // Delete or unref the data
    void Destroy();
    ...
    // especially if reording X quantities and using the wxPlotWindow
    ...
    // Get a pointer to the data (call CalcBoundingRect after if values change)
    ...
    //   doesn't fail just retuns ends if out of bounds
    ...
    //   returns empty plotdata is ranges don't match
    ...
    bool operator == (const wxPlotData& plotData)
...
private:
    DECLARE_ABSTRACT_CLASS(wxPlotData)
};
...
class WXDLLIMPEXP_PLOTLIB wxPlotDataObject : public wxTextDataObject
{
public:
    wxPlotDataObject();
