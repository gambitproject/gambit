...
#ifndef _WX_PLOTWIN_H_
#define _WX_PLOTWIN_H_

#if defined(__GNUG__) && !defined(NO_GCC_PRAGMA)
    #pragma interface "plotwin.h"
#endif
...
class WXDLLIMPEXP_PLOTLIB wxPlotArea;
class WXDLLIMPEXP_PLOTLIB wxPlotAxis;
class WXDLLIMPEXP_PLOTLIB wxPlotWindow;
class WXDLLIMPEXP_PLOTLIB wxPlotEvent;

class WXDLLIMPEXP_PLOTLIB wxPlotCurve;
...
#include "wx/dynarray.h"
    ...
    #define WX_DECLARE_OBJARRAY_WITH_DECL(T, name, expmode) WX_DECLARE_USER_EXPORTED_OBJARRAY(T, name, WXDLLIMPEXP_PLOTLIB)
#endif
WX_DECLARE_OBJARRAY_WITH_DECL(wxPoint2DDouble, wxArrayPoint2DDouble, class WXDLLIMPEXP_PLOTLIB);
WX_DECLARE_OBJARRAY_WITH_DECL(wxRect2DDouble, wxArrayRect2DDouble, class WXDLLIMPEXP_PLOTLIB);
WX_DECLARE_OBJARRAY_WITH_DECL(wxPlotCurve, wxArrayPlotCurve, class WXDLLIMPEXP_PLOTLIB);
...
enum wxPlotSelection_Type
{
    ...
    wxPLOT_SELECT_MUTIPLE       // multiple curves may have multiple selections
};

// Redraw parts or all of the windows
enum wxPlotRedraw_Type
{
    wxPLOT_REDRAW_NONE       = 0x000,
    ...
    wxPLOT_REDRAW_WINDOW     = 0x008,  // wxPlotWindow container window 
...
//-----------------------------------------------------------------------------
// wxPlotArea - window where the plot is drawn - privately used in wxPlotWindow
//-----------------------------------------------------------------------------

class WXDLLIMPEXP_PLOTLIB wxPlotArea : public wxWindow
{
public:
    ...
    // Get the owner (parent) wxPlotWindow
    wxPlotWindow *GetOwner() const { return m_owner; }
...
//-----------------------------------------------------------------------------
// wxPlotAxis - X or Y axis window (privately used in wxPlotWindow)
//-----------------------------------------------------------------------------

class WXDLLIMPEXP_PLOTLIB wxPlotAxis : public wxWindow
{
public:
...
//-----------------------------------------------------------------------------
// wxPlotWindow - window to display wxPlotCurves, public interface
//
// notes:
...
class WXDLLIMPEXP_PLOTLIB wxPlotWindow : public wxWindow
{
public:
    wxPlotWindow() : wxWindow() { Init(); }
    
    wxPlotWindow( wxWindow *parent, wxWindowID win_id = wxID_ANY, 
                  const wxPoint &pos = wxDefaultPosition, 
                  const wxSize &size = wxDefaultSize, 
                  wxPlotAxis_Type flags = wxPLOT_DEFAULT,
                  const wxString& name = wxT("wxPlotWindow") ) 
    {
        Init();
    ...
    // Total number of curves associated with the plotwindow
    ...
    const wxPoint2DDouble& GetCursorPoint();
    ...
    // Draw the data curve symbols on the plotwindow
    ...
    int GetCursorSize() const { return m_cursor_size; }
    void SetCursorSize(int size) { m_cursor_size = size; }
    
    // Get/Set the axis numbers font and colour, default normal & black
    wxFont   GetAxisFont() const   { return m_xAxis->GetFont(); }
    wxColour GetAxisColour() const { return m_xAxis->GetForegroundColour(); }
    ...
    wxFont   GetAxisLabelFont() const   { return GetFont(); }
    wxColour GetAxisLabelColour() const { return GetForegroundColour(); }
    ...
    wxFont   GetKeyFont() const { return m_area->GetFont(); }
    ...
    const wxPoint& GetKeyPosition() const { return m_keyPosition; }
    bool GetKeyInside() const { return m_key_inside; }
    void SetKeyPosition(const wxPoint &pos, bool stay_inside=true) 
        { m_keyPosition = pos; m_key_inside = stay_inside; Redraw(wxPLOT_REDRAW_PLOT); }
    ...
    enum wxPlotWindowTextCtrl_Type
    {
        wxPLOT_EDIT_TITLE = 1,
        wxPLOT_EDIT_XAXIS,
        wxPLOT_EDIT_YAXIS,
    ...
    //  which can be vetoed
    void ShowTextCtrl(wxPlotWindowTextCtrl_Type type, bool send_event = false);
    ...
    // Redraw parts of the plotwindow using combinations of wxPlot_Redraw_Type
    ...
    // Draw the wxPlotWindow (this window)
    virtual void DrawPlotWindow( wxDC *dc );    
    ...
    // Draw the cursor marking a single point in a curve wxPlotWindow::GetCursorPoint
    ...
    // Draw the plot axes and plotwindow on this wxDC for printing, sort of WYSIWYG
    ...
    enum
    {
        ID_PLOTWIN_X_AXIS = 100,
        ID_PLOTWIN_Y_AXIS,
        ID_PLOTWIN_AREA,
        ID_PLOTWIN_X_SCROLLBAR,  
        ID_PLOTWIN_Y_SCROLLBAR
    };
    ...
    wxArrayPoint2DDouble m_markers; // unused
...
protected:
    ...
    wxPlotCurve *m_activeCurve;                 // these two point to same curve
    int          m_active_index;

    // cursor variables
    wxPoint2DDouble m_cursorPt;
    int m_cursor_index;
    int m_cursor_curve;
    long m_cursor_colour;
    int m_cursor_size;

    // fonts and colours, rest of them are the window's fore/back colours and font
    wxFont   m_titleFont;
    ...
    bool m_key_inside;
    bool m_show_key;
    wxString m_keyString;
    int m_key_line_width;
    int m_key_line_margin;   
    ...
    // rects of the positions of each window - remember for DrawPlotWindow
...
private:
    void Init();
    DECLARE_ABSTRACT_CLASS(wxPlotWindow)
    DECLARE_EVENT_TABLE()
};

//-----------------------------------------------------------------------------
// wxPlotPrintout - simple mechanism to print the contents of the wxPlotWindow
//                  on a single page
//-----------------------------------------------------------------------------

class WXDLLIMPEXP_PLOTLIB wxPlotPrintout : public wxPrintout
{
public:
    wxPlotPrintout(wxPlotWindow* plotWin, const wxString &title = wxEmptyString);
    ...
    wxPlotWindow *GetPlotWindow() const { return m_plotWin; }
...
class WXDLLIMPEXP_PLOTLIB wxPlotEvent : public wxNotifyEvent
{
public:
    wxPlotEvent(wxEventType commandType = wxEVT_NULL, int id = wxID_ANY, 
                wxPlotWindow *window = NULL);
    ...
    // index of the curve in wxPlotWindow::GetCurve(index)
    ...
    wxPlotWindow *GetPlotWindow() const 
        { return wxDynamicCast(GetEventObject(),wxPlotWindow); }
...
class WXDLLIMPEXP_PLOTLIB wxPlotSelectionEvent : public wxPlotEvent
{
public:
...
// wxPlotEvent 
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_ADD_CURVE,          0) // a curve has been added
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_DELETING_CURVE,     0) // a curve is about to be deleted, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_DELETED_CURVE,      0) // a curve has been deleted

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CURVE_SEL_CHANGING, 0) // curve selection changing, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CURVE_SEL_CHANGED,  0) // curve selection has changed

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_MOUSE_MOTION,       0) // mouse moved
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CLICKED,            0) // mouse left or right clicked
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_DOUBLECLICKED,      0) // mouse left or right doubleclicked
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_POINT_CLICKED,      0) // clicked on a plot point (+-2pixels) 
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_POINT_DOUBLECLICKED,0) // dclicked on a plot point (+-2pixels) 

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CREATING,  0) // mouse left down and drag begin
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CHANGING,  0) // mouse left down and dragging
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CREATED,   0) // mouse left down and drag end

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VIEW_CHANGING,      0) // zoom or origin of plotwindow is about to change
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VIEW_CHANGED,       0) // zoom or origin of plotwindow has changed

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CURSOR_CHANGING,    0) // cursor point/curve is about to change, event.Skip(false) to prevent
                                                           // if the cursor is invalidated since
                                                           // the curve is gone you cannot prevent it
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_CURSOR_CHANGED,     0) // cursor point/curve changed

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_ERROR,              0) // an error has occured, see event.GetString()
                                                           // usually nonfatal NaN overflow errors 

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_BEGIN_TITLE_EDIT,   0) // title is about to be edited, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_END_TITLE_EDIT,     0) // title has been edited and changed, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_BEGIN_X_LABEL_EDIT, 0) // x label is about to be edited, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_END_X_LABEL_EDIT,   0) // x label has been edited and changed, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_BEGIN_Y_LABEL_EDIT, 0) // y label is about to be edited, event.Skip(false) to prevent
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_END_Y_LABEL_EDIT,   0) // y label has been edited and changed, event.Skip(false) to prevent

DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_MOUSE_FUNC_CHANGING,0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_MOUSE_FUNC_CHANGED, 0)

// wxPlotSelectionEvent 
//DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_RANGE_SEL_CREATING,0)
//DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_RANGE_SEL_CREATED, 0)
//DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_RANGE_SEL_CHANGING,0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_RANGE_SEL_CHANGED,   0)

// unused
/*
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VALUE_SEL_CREATING, 0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VALUE_SEL_CREATED,  0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VALUE_SEL_CHANGING, 0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_VALUE_SEL_CHANGED,  0)
DECLARE_LOCAL_EVENT_TYPE(wxEVT_PLOT_AREA_SEL_CHANGED,   0)
*/
...
#define EVT_PLOT_ADD_CURVE(id, fn)           DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_ADD_CURVE,           id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_DELETING_CURVE(id, fn)      DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_DELETING_CURVE,      id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_DELETED_CURVE(id, fn)       DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_DELETED_CURVE,       id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_CURVE_SEL_CHANGING(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_CURVE_SEL_CHANGING,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_CURVE_SEL_CHANGED(id, fn)   DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_CURVE_SEL_CHANGED,   id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_MOUSE_MOTION(id, fn)        DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_MOUSE_MOTION,        id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_CLICKED(id, fn)             DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_CLICKED,             id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_DOUBLECLICKED(id, fn)       DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_DOUBLECLICKED,       id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_POINT_CLICKED(id, fn)       DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_POINT_CLICKED,       id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_POINT_DOUBLECLICKED(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_POINT_DOUBLECLICKED, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_AREA_SEL_CREATING(id, fn)   DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_AREA_SEL_CREATING,   id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_AREA_SEL_CHANGING(id, fn)   DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_AREA_SEL_CHANGING,   id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_AREA_SEL_CREATED(id, fn)    DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VIEW_CREATED,        id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_VIEW_CHANGING(id, fn)       DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VIEW_CHANGING,       id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_VIEW_CHANGED(id, fn)        DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VIEW_CHANGED,        id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_CURSOR_CHANGING(id, fn)     DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_CURSOR_CHANGING,     id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_CURSOR_CHANGED(id, fn)      DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_CURSOR_CHANGED,      id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_ERROR(id, fn)               DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_ERROR,               id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_BEGIN_TITLE_EDIT(id, fn)    DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_BEGIN_TITLE_EDIT,    id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_END_TITLE_EDIT(id, fn)      DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_END_TITLE_EDIT,      id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_BEGIN_X_LABEL_EDIT(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_BEGIN_X_LABEL_EDIT,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_END_X_LABEL_EDIT(id, fn)    DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_END_X_LABEL_EDIT,    id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_BEGIN_Y_LABEL_EDIT(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_BEGIN_Y_LABEL_EDIT,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_END_Y_LABEL_EDIT(id, fn)    DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_END_Y_LABEL_EDIT,    id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

#define EVT_PLOT_MOUSE_FUNC_CHANGING(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_MOUSE_FUNC_CHANGING, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_MOUSE_FUNC_CHANGED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_MOUSE_FUNC_CHANGED,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),

typedef void (wxEvtHandler::*wxPlotSelectionEventFunction)(wxPlotSelectionEvent&);
//#define EVT_PLOT_RANGE_SEL_CREATING(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CREATING, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
//#define EVT_PLOT_RANGE_SEL_CREATED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CREATED,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
//#define EVT_PLOT_RANGE_SEL_CHANGING(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CHANGING, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_RANGE_SEL_CHANGED(id, fn)   DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_RANGE_SEL_CHANGED,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotSelectionEventFunction) & fn, (wxObject *) NULL ),

/*
#define EVT_PLOT_VALUE_SEL_CREATING(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CREATING, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_VALUE_SEL_CREATED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CREATED,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_VALUE_SEL_CHANGING(id, fn) DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CHANGING, id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_VALUE_SEL_CHANGED(id, fn)  DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_VALUE_SEL_CHANGED,  id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
#define EVT_PLOT_AREA_SEL_CHANGED(id, fn)   DECLARE_EVENT_TABLE_ENTRY(wxEVT_PLOT_AREA_SEL_CHANGED,   id, wxID_ANY, (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) (wxNotifyEventFunction) (wxPlotEventFunction) & fn, (wxObject *) NULL ),
*/

#endif // _WX_PLOTWIN_H_
