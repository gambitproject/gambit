...
NodeSolnShow::NodeSolnShow(int num_players,const EfgShow *parent_):
							SpreadSheet3D(2,1,1,"Inspect Node",(wxFrame *)parent_,0),parent(parent_),
...
for (i=1;i<=NUM_FEATURES;i++)
	if (features[i]) SetLabelRow(Pos(i),feature_names[i]);
...
{
int pos=0;
for (int i=1;i<=feature;i++) if (features[i]) pos++;
if (pos==0) pos=1;
return pos;
}
...
for (int i=1;i<=NUM_FEATURES;i++)
{
	...
	if (i%2==0) options_dialog->Add(wxMakeFormNewLine());
}
...
if (options_dialog->Completed()==wxOK)
{
	...
	for (i=NUM_FEATURES;i>=1;i--)
		if (features[i] && !new_features[i]) {DelRow(Pos(i));features[i]=0;}
	// now check if we turned anything on
	for (i=1;i<=NUM_FEATURES;i++)
		if (new_features[i] && !features[i])
			{AddRow(Pos(i)+1);SetLabelRow(Pos(i)+1,feature_names[i]);features[i]=1;}
	// save as default, if desired
	if (save_def)
		for (i=1;i<=NUM_FEATURES;i++)
			wxWriteResource(NODESOLN_SECTION,feature_names[i],features[i],"gambit.ini");
	Redraw();
}
}
...
switch(id)
{
	case EFG_NODE_INSP_OPTIONS: SetOptions();break;
...
{
	for (int i=1;i<=NUM_FEATURES;i++) if (features[i]) SetCell(Pos(i),1,"N/A");
	return;
}
...
for (int feature=1;feature<=NUM_FEATURES;feature++)
	if (features[feature])
		switch(feature)
		{
			...
			case ISETPROB   : SetCell(Pos(feature),1,parent->AsString(tIsetProb,n)); break;
			case BELIEFPROB : SetCell(Pos(feature),1,parent->AsString(tBeliefProb,n)); break;
			case NODEVALUE  :	SetCell(Pos(feature),1,parent->AsString(tNodeValue,n)); break;
			case ISETVALUE  :	SetCell(Pos(feature),1,parent->AsString(tIsetValue,n)); break;
			...
			default: assert(0 && "Invalid feature for node solndisp"); break;
		}
Repaint();
}
...
sortby = new wxRadioBox(this, 0, "Sort By", -1, -1, -1, -1, 8, sort_by_str, 2,wxVERTICAL, "sortby");
NewLine();
NewLine();
new wxGroupBox(this, "Filter By", 12, 95, 464, 150, 0, "gbox");
filter_creator = new wxListBox(this, 0, "Creator", wxMULTIPLE, 55,-1,140,85, NUM_BCREATORS, options.filter_cr_str+1);
filter_nash = new wxListBox(this, 0, "Nash", wxMULTIPLE, -1,-1,44,85, 3, options.filter_tri_str+1);
filter_perfect = new wxListBox(this, 0, "Perfect", wxMULTIPLE, -1,-1,56,85, 3, options.filter_tri_str+1);
filter_seq = new wxListBox(this, 0, "Sequential", wxMULTIPLE, -1,-1,78,85, 3, options.filter_tri_str+1);
...
// but in wx_motif, they fail to appear.  So, we create them later by
...
case BSOLN_ID: return 1;
case BSOLN_PLAYER: return 2;
case BSOLN_ISET: return 3;
// The data is always the LAST column
case BSOLN_DATA: return (FeaturePos(BSOLN_NUM_FEATURES-1)+1);
// The rest of the features are optional and may change location
default:
	{
	int pos=3;
	for (int i=0;i<=feature;i++)
		if (features[i] && feature_width[i]) pos++;
	return pos;
	}
}
}
...
MyDialogBox *options_dialog=new MyDialogBox(this,"Settings",EFG_SOLVE_INSPECT_OPTIONS_HELP);
...
if (options_dialog->Completed()==wxOK)
{
	...
	for (i=BSOLN_NUM_FEATURES-1;i>=0;i--)
		if (features[i] && !new_features[i])
			{if (feature_width[i]) DelCol(FeaturePos(i)); features[i]=0;}
	// now check if we turned anything on
	for (i=0;i<BSOLN_NUM_FEATURES;i++)
		if (new_features[i] && !features[i])
			{
...
int new_soln;
if (col!=-1) new_soln=SolnNum(row); else	new_soln=row;
if (new_soln!=cur_soln)
{
	...
	HiLighted(SolnPos(cur_soln),1,0,TRUE);
	// inform the parent
	if (parent) parent->ChangeSolution(cur_soln);
	Repaint();
}
}
...
if (col==FeaturePos(BSOLN_ISET)) // hilight infoset
{
	...
	else
	{
		...
		row=row%num_isets;
		if (row==0)
			{pl=num_players;iset=dim.Lengths()[pl];}
		else
			while (row-dim.Lengths()[pl]>0) {row-=dim.Lengths()[pl];pl++;iset=row;}
		if (row==dim.Lengths()[pl]) iset=row;
	}
...
if (col==FeaturePos(BSOLN_ID))
{
	...
	if (row!=prow) {row=prow;SetCurRow(row);}
}

if (features[BSOLN_DYNAMIC] && parent) UpdateSoln(row,col);
}
...
{
if (solns.Length()==0) {wxMessageBox("Solution list is empty");return;}
if (!all)
{
	int row=CurRow();
	if (row==1) {wxMessageBox("Remove what?");return;}
	int soln_num=SolnNum(row);

	solns.Remove(soln_num);
	if (soln_num==cur_soln) UpdateSoln(1,1);	// unselect all solutions
	for (int i=num_isets;i>=1;i--) DelRow(2+(soln_num-1)*num_players+(i-1));
	num_solutions--;
}
else
{
	UpdateSoln(1,1);	// unselect all solutions
	while (solns.Length()) solns.Remove(1);    // del all solutions
	while (GetRows()>1) DelRow(2);  // del all rows after row 1
	num_solutions=0;
}
...
if (add_dialog->Completed()==wxOK)
{
	...
	SortFilter(false);
	if (cur_soln==soln_num) parent->ChangeSolution(cur_soln);
}
delete add_dialog;
}
...
SetCell(1,1,"Player");Bold(1,1,0,TRUE);
SetCell(1,2,"Iset");Bold(1,2,0,TRUE);
int cur_pos=2;
...
int i;
for (i=l;i>=1;i--)	if (picked[i]) ok=true;
if (!ok)
{
	wxMessageBox("Must pick at least one solution","Pick a solution",wxOK|wxCENTRE,this);
	return;
}
for (i=l;i>=1;i--)	if (!picked[i]) solns.Remove(i);
SetCompleted(wxOK);
}
...
if (new_soln!=picked)
{
	if (picked) HiLighted(SolnPos(picked),1,0,FALSE); // un-highlight this solution
	if (new_soln) HiLighted(SolnPos(new_soln),1,0,TRUE); // highlight this solution
	picked=new_soln;
}
Repaint();
}
