...
*/
  ...
  if (quickie.FindCertainNumberOfRoots(Cube,100000,0))
    ...
    gout << "The system\n" << quickie.UnderlyingEquations()
	 << " could not be resolved by FindCertainNumberOfRoots.\n";

  timer.Stop();
  gout << "The QuikSolv computation of roots using FindCertainNumberOfRoots took " 
    << (int)timer.Elapsed() << " seconds.\n\n";


  timer.Start();

  if (quickie.FindCertainNumberOfRoots(Cube,200000,0))
    gout << "\nThe first test system is\n" 
	 << quickie.UnderlyingEquations() 
	 << "It has the following roots in [-4,4]^4:\n"
	 << quickie.RootList();
...
*/
  ...
  if (quickie2.FindCertainNumberOfRoots(Cube,1000000,0))
  ...
  if (quickie2.FindCertainNumberOfRoots(Cube,2000000,0))
    gout << "The system, for totally mixed equilibrium of a 2x2x2x2 game\n"
	 << "without cubic terms, is\n" 
	 << quickie2.UnderlyingEquations() 
	 << "It has the following roots in [-4,4]^4:\n"
...
*/
  ...
  if (quickie3.FindCertainNumberOfRoots(Cube,1000000,0))
    gout << "The system, for totally mixed equilibrium of a 2x2x2x2 game "
	 << "is\n" 
