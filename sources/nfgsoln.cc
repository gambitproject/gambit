...
filter_creator = new wxListBox(this, 0, "Creator", wxMULTIPLE, 55,-1,140,85, NUM_MCREATORS, options.filter_cr_str+1);
...
filter_proper = new wxListBox(this, 0, "Proper", wxMULTIPLE, -1,-1,78,85, 3, options.filter_tri_str+1);
NewLine();
...
int	NfgSolnShow::feature_width[MSOLN_NUM_FEATURES]={0,0,-1,8,2,2,2,7,7,7};
...
if (opts&MSOLN_O_OPTIONS)
	...
	for (i=0;i<MSOLN_NUM_FEATURES;i++) features[i]=0;
...
case MSOLN_ID: return 1;
case MSOLN_PLAYER: return 2;
...
MyDialogBox *options_dialog=new MyDialogBox(this,"Settings",NFG_SOLVE_INSPECT_OPTIONS_HELP);
...
if (options_dialog->Completed()==wxOK)
{
	...
	if (save_def)
		for (i=0;i<MSOLN_NUM_FEATURES;i++)
			wxWriteResource(MSOLN_SHOW_SECT,feature_names[i],features[i],defaults_file);
...
for (int i=1;i<=num_solutions;i++)
{
	...
	for (j=1;j<=num_players;j++)
	{
		...
		for (; k1<=cur_vector.Lengths()[j];k1++) SetCell(cur_pos+(j-1),sp+k1,"");
	}
}
}
...
int row=CurRow();
if (row==1) return;	// first row=no solution selected
int new_soln=(row-1)/num_players+1;
if ((row-1)%num_players==0) new_soln-=1;
if (new_soln<1) new_soln=1;if (new_soln>solns.Length()) new_soln=solns.Length();
...
if (!all)
{
	...
	for (int i=num_players;i>=1;i--) DelRow(2+(soln_num-1)*num_players+(i-1));
	num_solutions--;
}
else
{
...
public:
	...
	{
	if (row==1) {SetCurRow(2);row=2;}
	if (col-1>dim[row-1]) SetCurCol(dim[row-1]+1);
...
Nfg1SolnPicker::Nfg1SolnPicker(gSortList<MixedSolution > &soln,int num_players,int max_strats,
						...
						:NfgSolnShow(soln,num_players,max_strats,cur_soln_,ds,sf_options,parent_,parent_frame,MSOLN_O_EDIT)
{
