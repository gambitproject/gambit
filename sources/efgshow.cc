...
if (what==SOLVE_SETUP_CUSTOM)
{
	...
	if (ESD.GetResult()==SD_CANCEL) return;	// CANCEL
...
{
	int completed=wxMessageBox("This game is not perfect recall\nDo you wish to continue?","Solve Warning",wxOK|wxCANCEL|wxCENTRE,this);
	if (completed!=wxOK) return;
}

EfgSolveSettings ESS(ef);
Enable(FALSE);	// do not want users doing anything while solving
if (ESS.MarkSubgames()) tw->subgame_solve();
if (!ESS.MarkSubgames() && ESS.UseStandard()) tw->subgame_clear_all(); // for standard if not mark, must clear
...
{
if (soln_show) delete soln_show;soln_show=0;
cur_soln=0;
solns.Flush();
Nodes(ef,all_nodes);
...
if (!IsPerfectRecall(ef,bad1,bad2))
{
	int completed=wxMessageBox("This game is not perfect recall\nDo you wish to continue?","Efg -> Nfg",wxOK|wxCANCEL|wxCENTRE,this);
	if (completed!=wxOK) return false;
}
...
solve_normal_dialog->Add(wxMakeFormString("Type",&normal_str,wxFORM_RADIOBOX,
