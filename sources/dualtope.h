...
struct ind_pair {  // For recording the endpoints of an edge
    int head;
    int tail;
  };
...
public:
  ...
  bool               is_edge(const ind_pair&)             const;
  gList<ind_pair*> ind_pairs_of_edges()                 const;

  // Input-Output 
  void print_edge(const ind_pair edge) const;
  void list_edges(const int no_edges, const ind_pair* edges) const;
       // Before using, consider rewriting to use gList<ind_pair>
