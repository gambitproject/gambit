...
wxMenu *solve_menu=new wxMenu;
	...
	solve_menu->Append(SOLVE_SETTINGS,"&Custom",solve_settings_menu,"Control default options");
...
// Node menus
	...
	case NODE_INSERT:tw->node_add();break;
	case NODE_LABEL:tw->node_label();break;
	case NODE_SET_MARK:tw->node_set_mark();break;
	case NODE_GOTO_MARK:tw->node_goto_mark();break;
// Branch menu
	case ACTION_DELETE:tw->action_delete();break;
	case ACTION_INSERT:tw->action_insert();break;
	case ACTION_LABEL:tw->action_label();break;
	case ACTION_PROBS:tw->action_probs();	break;
// Tree menu
	case TREE_DELETE:tw->tree_delete();break;
...
// Infoset menu
	...
	case INFOSET_SPLIT:tw->infoset_split();break;
	case INFOSET_JOIN:tw->infoset_join();break;
	case INFOSET_LABEL:tw->infoset_label();break;
	case INFOSET_SWITCH_PLAYER:tw->infoset_switch_player();break;
	case INFOSET_REVEAL:tw->infoset_reveal();break;
// File menu
	case FILE_OUTPUT:tw->output();break;
	case FILE_SAVE:tw->file_save();break;
	case FILE_CLOSE:Close();break;
	case FILE_LOG:GetMenuBar()->Check(log_item,tw->logging());break;
// Inspect menu
...
// Subgames menu
	...
	case SUBGAME_CLEARALL:tw->subgame_clear_all();break;
	case SUBGAME_CLEARONE:tw->subgame_clear_one();break;
	case SUBGAME_COLLAPSEALL:tw->subgame_collapse_all();break;
	case SUBGAME_COLLAPSEONE:tw->subgame_collapse_one();break;
	case SUBGAME_EXPANDALL:tw->subgame_expand_all();break;
	case SUBGAME_EXPANDBRANCH:tw->subgame_expand_branch();break;
...
// Solve menu
	...
	case SOLVE_STANDARD:	SolveSetup(SOLVE_SETUP_STANDARD); break;
...
// Display menu
	...
	case DISPLAY_ZOOM_WIN:GetMenuBar()->Check(zoom_win_item,tw->display_zoom_win());break;
	case DISPLAY_INC_ZOOM:tw->display_set_zoom(tw->display_get_zoom()+ZOOM_DELTA);break;
	case DISPLAY_DEC_ZOOM:tw->display_set_zoom(tw->display_get_zoom()-ZOOM_DELTA);break;
	case DISPLAY_LEGENDS:tw->display_legends();break;
	case DISPLAY_SET_OPTIONS:tw->display_options();break;
	case DISPLAY_COLORS:tw->display_colors(); break;
	case DISPLAY_SAVE_DEFAULT:tw->display_save_options();break;
	case DISPLAY_LOAD_DEFAULT:tw->display_load_options();break;
	case DISPLAY_SAVE_CUSTOM:tw->display_save_options(FALSE);break;
	case DISPLAY_LOAD_CUSTOM:tw->display_load_options(FALSE);break;
	case DISPLAY_REDRAW: /* redraws automatically after switch */ break;
	case DISPLAY_ACCELS:
...
// Help menu
	...
	case GAMBIT_HELP_CONTENTS:wxHelpContents(EFG_GUI_HELP);break;
	default:
...
for (int i=0;i<NUM_EFG_EVENTS;i++) events[i+1]=efg_events[i];
return events;
}
...
if (options_dialog->Completed()==wxOK)
{
	if (old_features.iset_hilight && !features.iset_hilight)
		{HilightInfoset(0,0,1);HilightInfoset(0,0,2);}
...
{
if (!features.iset_hilight) return;
if (who==1)
...
for (int i=1;i<=ef.NumPlayers();i++)
{
	...
	players[i]=new wxListBox(support,0,ef.Players()[i]->GetName(),TRUE,-1,-1,80,100);
	int j;
...
if (support->Completed()==wxOK)  {
  ...
  for (int i=ef.NumPlayers();i>=1;i--) // going from bottom up.
      ...
      for (int j=1;j<=ef.Players()[i]->NumInfosets();j++)
	{
	  ...
	  if (sup->NumActions(i,j)==0) failed=true;
	}
    }
  delete support;
  if (!failed)
...
if (EDPD.Completed()==wxOK)
{
	...
	if (!EDPD.DomMixed())
	{
		if (EDPD.FindAll())
		{
			while ((sup=ComputeDominated(*sup,EDPD.DomStrong(),EDPD.Players(),gnull,status)))
				supports.Append(sup);
		}
		else
		{
			if ((sup=ComputeDominated(*sup,EDPD.DomStrong(),EDPD.Players(),gnull,status)))
				supports.Append(sup);
		}
	}
	else
...
while (supports.Length()) delete supports.Remove(1);
...
if (!ef)	// must create a new extensive form from scratch or file
{
	...
	else												// from data file
	{
		...
		if (!infile.IsValid())
      	{wxMessageBox("ReadFailed::Check the file");return;}
		ReadEfgFile(infile,ef);
		if (!ef) wxMessageBox("ReadFailed::Check the file");
	}
}
EfgShow *ef_show=0;
if (ef)
