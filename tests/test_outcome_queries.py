...
def test_get_outcome():
    ...
    assert (
        games.node_at_history(game, ("U1", "D2", "U3")).outcome
        == game.outcomes["Outcome 1"]
    )
    assert not games.node_at_history(game, ()).outcome


def test_make_outcome_null():
    """Resetting a node's outcome to null leaves the node's outcome view falsy."""
    game = games.read_from_file("basic_extensive_game.efg")
    node = games.node_at_history(game, ("U1", "U2", "U3"))
...
def test_node_outcome_subscript_tracks_mutation():
    ...
    game = games.read_from_file("basic_extensive_game.efg")
    node = games.node_at_history(game, ("U1", "D2", "U3"))
    proxy = node.outcome
    player = "Player 1"
    proxy[player] = 7
...
def test_null_outcome_label_is_none():
    ...
    game = games.read_from_file("basic_extensive_game.efg")
    assert games.node_at_history(game, ()).outcome.label is None
...
def test_null_outcome_compares_unequal_to_itself():
    ...
    assert (root.outcome == root.outcome) is False
...
def test_null_outcome_reads_zero_payoffs():
    ...
    for player in game.players:
        assert root.outcome[player] == 0


def test_null_outcome_payoff_write_raises():
    game = games.read_from_file("basic_extensive_game.efg")
    with pytest.raises(gbt.UndefinedOperationError):
        games.node_at_history(game, ()).outcome["Player 1"] = 1


def test_null_outcome_label_write_raises():
    game = games.read_from_file("basic_extensive_game.efg")
    with pytest.raises(ValueError):
        games.node_at_history(game, ()).outcome.label = "Outcome 4"
...
def test_null_outcome_number_is_none():
    ...
    game = games.read_from_file("basic_extensive_game.efg")
    assert games.node_at_history(game, ()).outcome.number is None
