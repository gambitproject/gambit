...
def create_efg_corresponding_to_bimatrix_game_as_ndarrays(
...
def create_efg_corresponding_to_bimatrix_game(g: gbt.Game) -> gbt.Game:
    """
    There is no direct pygambit method to create an EFG from a stategic-form game.
    Here we create an EFG corresponding to a bimatrix game, given as a Gambit game.
    """
    assert len(g.players) == 2
    A, B = g.to_arrays()
    title = f"EFG for: {g.title}"
    return create_efg_corresponding_to_bimatrix_game_as_ndarrays(A, B, title)


def create_efg_corresponding_to_catalog_bimatrix_game(slug: str) -> gbt.Game:
    g = gbt.catalog.load(slug)
    return create_efg_corresponding_to_bimatrix_game(g)
...
def create_2x2_zero_sum_efg(variant: None | str = None) -> gbt.Game:
    ...
    A = np.eye(2)
    B = -A
    g = create_efg_corresponding_to_bimatrix_game_as_ndarrays(A, B, title)
