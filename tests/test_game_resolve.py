...
def _test_valid_resolutions(collection: list, resolver: typing.Callable) -> None:
    """Generic function to exercise resolving objects via their existing labels."""
...
def test_resolve_node_rejects_node():
    """A bare `Node` -- even one belonging to this same game -- is not accepted;
    `_resolve_node` only resolves a `Selector`, `History` tuple, or label `str`."""
    game = games.read_from_file("sample_extensive_game.efg")
    with pytest.raises(TypeError):
        game._resolve_node(games.node_at_history(game, ()), "test_resolve_node_rejects_node")
