...
def test_make_outcome_attaches_to_all_given_nodes():
    ...
    game.append_move(games.root_node(game), "Alice", ["U", "M", "D"])
    up, middle, down = games.root_node(game).children
...
def test_make_outcome_absorbs_fully_covered_outcome_and_reuses_label():
    ...
    game.append_move(games.root_node(game), "Alice", ["U", "D"])
    up, down = games.root_node(game).children
...
def test_make_outcome_bad_label_raises_and_leaves_game_unchanged(bad_label: str):
    ...
    game.append_move(games.root_node(game), "A", ["win", "lose"])
    win_node, lose_node = games.root_node(game).children
...
def test_make_outcome_incomplete_payoffs_raises():
    ...
    game.append_move(games.root_node(game), "Alice", ["U", "D"])
    with pytest.raises(ValueError):
        game.make_outcome(next(iter(games.root_node(game).children)), {"Alice": 1}, "w")
...
def test_make_outcome_payoffs_naming_player_twice_raises():
    ...
    with pytest.raises(ValueError):
        game.make_outcome(next(iter(games.root_node(game).children)), payoffs, "w")
...
def test_make_outcome_null_on_already_null_node_is_a_no_op():
    game = gbt.Game.new_tree(["Alice"])
    game.append_move(games.root_node(game), "Alice", ["U", "D"])
    up, _ = games.root_node(game).children
    outcome_count = len(game.outcomes)
    game.make_outcome_null(up)
